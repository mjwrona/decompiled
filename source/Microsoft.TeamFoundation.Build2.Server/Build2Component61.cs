...
// Type: Microsoft.TeamFoundation.Build2.Server.Build2Component61
...
{
  internal class Build2Component61 : Build2Component60
  {
    protected static readonly SqlMetaData[] typ_ScheduleAddTable2 = new SqlMetaData[6]
    {
      ...
      string comment = null)
    {
      ...
      using (ResultCollection rc = new ResultCollection((IDataReader) this.ExecuteReader(), this.ProcedureName, this.RequestContext))
      {
        ...
        if (definition != null)
          this.UpdateScheduleTrigger(rc, definition);
    ...
    {
      Build2Component61 build2Component61 = this;
      build2Component61.TraceEnter(0, nameof (GetDefinitionAsync));
      build2Component61.PrepareStoredProcedure("Build.prc_GetDefinition");
      build2Component61.BindInt("@dataspaceId", build2Component61.GetDataspaceId(projectId));
      build2Component61.BindInt("@definitionId", definitionId);
      build2Component61.BindNullableInt32("@definitionVersion", definitionVersion);
      build2Component61.BindBoolean("@includeDeleted", includeDeleted);
      build2Component61.BindNullableUtcDateTime2("@minMetricsTime", minMetricsTime);
      build2Component61.BindBoolean("@includeLatestBuilds", includeLatestBuilds);
      BuildDefinition definitionAsync;
      using (ResultCollection rc = new ResultCollection((IDataReader) await build2Component61.ExecuteReaderAsync(), build2Component61.ProcedureName, build2Component61.RequestContext))
      {
        rc.AddBinder<BuildDefinition>(build2Component61.GetBuildDefinitionBinder());
        rc.AddBinder<BuildDefinitionMetric>(build2Component61.GetBuildDefinitionMetricBinder());
        rc.AddBinder<DefinitionTagData>(build2Component61.GetDefinitionTagDataBinder());
        if (!definitionVersion.HasValue)
          rc.AddBinder<DesignerSchedule>(build2Component61.GetDesignerScheduleBinder());
        rc.AddBinder<BuildData>(build2Component61.GetBuildDataBinder());
        rc.AddBinder<BuildTagData>(build2Component61.GetBuildTagDataBinder());
        rc.AddBinder<BuildOrchestrationData>(build2Component61.GetBuildOrchestrationDataBinder());
        BuildDefinition definition = rc.GetCurrent<BuildDefinition>().SingleOrDefault<BuildDefinition>();
        if (definition != null)
        {
          rc.NextResult();
          ...
          if (!definitionVersion.HasValue)
            build2Component61.UpdateScheduleTrigger(rc, definition);
          if (includeLatestBuilds)
            build2Component61.BindLatestBuilds(rc, definition);
        }
        build2Component61.TraceLeave(0, nameof (GetDefinitionAsync));
    ...
    public override async Task<List<BuildDefinition>> GetDefinitionsByIdsAsync(
      ...
      bool includeDrafts = false)
    {
      Build2Component61 component = this;
      ...
      using (ResultCollection rc = new ResultCollection((IDataReader) await component.ExecuteReaderAsync(), component.ProcedureName, component.RequestContext))
      {
        ...
        rc.AddBinder<DesignerSchedule>(component.GetDesignerScheduleBinder());
    ...
    public override void UpdateCronSchedules(
      BuildDefinition definition,
      string branchName,
    ...
    {
      using (this.TraceScope(method: nameof (UpdateCronSchedules)))
      {
        this.PrepareStoredProcedure("Build.prc_UpdateCronSchedules");
        if (branchName.Length > 2048)
          throw new ArgumentException("Branch name for schedule exceeds maximum length and will be truncated.");
        this.BindInt("@dataspaceId", this.GetDataspaceId(definition.ProjectId));
        this.BindCronScheduleTable("@scheduleTable", (IEnumerable<PipelineSchedule>) schedules, branchName);
        this.BindInt("@definitionId", definition.Id);
        this.BindString("@branchName", branchName, 2048, BindStringBehavior.EmptyStringToNull, SqlDbType.NVarChar);
        using (ResultCollection resultCollection = new ResultCollection((IDataReader) this.ExecuteReader(), this.ProcedureName, this.RequestContext))
        {
          resultCollection.AddBinder<CronSchedule>(this.GetCronScheduleBinder());
          resultCollection.AddBinder<CronSchedule>(this.GetCronScheduleBinder());
          deletedSchedules = resultCollection.GetCurrent<CronSchedule>().Items.Distinct<CronSchedule>().ToList<CronSchedule>();
          resultCollection.NextResult();
          addedSchedules = resultCollection.GetCurrent<CronSchedule>().Items.Distinct<CronSchedule>().ToList<CronSchedule>();
        }
      }
    }

    public override List<CronSchedule> GetCronSchedules(Guid projectId, Guid scheduleJobId = default (Guid))
    {
      List<CronSchedule> cronScheduleList = new List<CronSchedule>();
      using (this.TraceScope(method: nameof (GetCronSchedules)))
      {
        this.PrepareStoredProcedure("Build.prc_GetCronSchedules");
        this.BindInt("@dataspaceId", this.GetDataspaceId(projectId));
        this.BindGuid("@scheduleJobId", scheduleJobId);
        using (ResultCollection resultCollection = new ResultCollection((IDataReader) this.ExecuteReader(), this.ProcedureName, this.RequestContext))
        {
          resultCollection.AddBinder<CronSchedule>(this.GetCronScheduleBinder());
          return resultCollection.GetCurrent<CronSchedule>().Items.Distinct<CronSchedule>().ToList<CronSchedule>();
        }
      }
    }

    protected List<DesignerSchedule> ExtractDesignerSchedulesFromDefinition(
      BuildDefinition definition)
    {
      List<DesignerSchedule> schedulesFromDefinition = new List<DesignerSchedule>();
      if (definition.Triggers.FirstOrDefault<BuildTrigger>((System.Func<BuildTrigger, bool>) (t => t.TriggerType == DefinitionTriggerType.Schedule)) is ScheduleTrigger scheduleTrigger)
      {
        ...
        {
          DesignerSchedule designerSchedule = new DesignerSchedule(0, schedule.BranchFilters, new DesignerScheduleDetails(schedule.TimeZoneId, schedule.StartHours, schedule.StartMinutes, schedule.DaysToBuild), schedule.ScheduleJobId, schedule.ScheduleOnlyWithChanges, false, 0);
          schedulesFromDefinition.Add(designerSchedule);
        }
      }
      return schedulesFromDefinition;
    }

    protected override void UpdateScheduleTrigger(ResultCollection rc, BuildDefinition definition)
    {
      rc.NextResult();
      ...
      foreach (DesignerSchedule designerSchedule in rc.GetCurrent<DesignerSchedule>())
        scheduleList.Add(new Schedule()
        {
          BranchFilters = designerSchedule.BranchDetails.ToList<string>(),
          TimeZoneId = designerSchedule.ScheduleDetails.TimeZoneId,
          StartHours = designerSchedule.ScheduleDetails.StartHours,
          StartMinutes = designerSchedule.ScheduleDetails.StartMinutes,
          DaysToBuild = designerSchedule.ScheduleDetails.DaysToBuild,
          ScheduleJobId = designerSchedule.ScheduleJobId,
          ScheduleOnlyWithChanges = designerSchedule.ScheduleOnlyWithChanges
        });
    ...
    protected override void UpdateScheduleTriggers(
      ResultCollection rc,
      Dictionary<int, BuildDefinition> definitions)
    {
      rc.NextResult();
      List<DesignerSchedule> list = rc.GetCurrent<DesignerSchedule>().Items.Distinct<DesignerSchedule>().ToList<DesignerSchedule>();
      List<Schedule> scheduleList = new List<Schedule>();
      foreach (KeyValuePair<int, BuildDefinition> keyValuePair in definitions.ToArray<KeyValuePair<int, BuildDefinition>>())
      {
        ...
        scheduleList.Clear();
        foreach (DesignerSchedule designerSchedule in list.Where<DesignerSchedule>((System.Func<DesignerSchedule, bool>) (ns => ns.DefinitionId == buildDefinition.Value.Id)))
          scheduleList.Add(new Schedule()
          {
            BranchFilters = designerSchedule.BranchDetails.ToList<string>(),
            TimeZoneId = designerSchedule.ScheduleDetails.TimeZoneId,
    ...
    protected virtual ObjectBinder<DesignerSchedule> GetDesignerScheduleBinder() => (ObjectBinder<DesignerSchedule>) new DesignerScheduleBinder(this.RequestContext, (BuildSqlComponentBase) this);

    protected void BindCronScheduleTable(
      string parameterName,
      IEnumerable<PipelineSchedule> schedules,
      string branchName)
    {
      schedules = schedules ?? Enumerable.Empty<PipelineSchedule>();
      System.Func<PipelineSchedule, SqlDataRecord> selector = (System.Func<PipelineSchedule, SqlDataRecord>) (sch2 =>
      {
        SqlDataRecord record = new SqlDataRecord(Build2Component61.typ_ScheduleAddTable2);
        record.SetString(0, sch2.DisplayName, BindStringBehavior.Unchanged);
        record.SetString(1, branchName, BindStringBehavior.Unchanged);
        record.SetString(2, sch2.ScheduleDetails, BindStringBehavior.Unchanged);
        record.SetGuid(3, sch2.ScheduleJobId);
        record.SetBoolean(4, sch2.ScheduleOnlyWithChanges);
        record.SetBoolean(5, sch2.BatchSchedules);
        return record;
      });
      this.BindTable(parameterName, "Build.typ_ScheduleAddTable2", schedules.Select<PipelineSchedule, SqlDataRecord>(selector));
    }

    protected virtual ObjectBinder<CronSchedule> GetCronScheduleBinder() => (ObjectBinder<CronSchedule>) new CronScheduleBinder(this.RequestContext, (BuildSqlComponentBase) this);

    protected void BindDesignerScheduleTable(
      string parameterName,
      IEnumerable<DesignerSchedule> schedules)
    {
      schedules = schedules ?? Enumerable.Empty<DesignerSchedule>();
      System.Func<DesignerSchedule, SqlDataRecord> selector = (System.Func<DesignerSchedule, SqlDataRecord>) (sch2 =>
      {
        SqlDataRecord record = new SqlDataRecord(Build2Component61.typ_ScheduleAddTable2);
        string str1 = JsonUtility.ToString((object) sch2.BranchDetails);
        ...
        record.SetString(1, str1, BindStringBehavior.Unchanged);
        record.SetString(2, str2, BindStringBehavior.Unchanged);
        record.SetGuid(3, sch2.ScheduleJobId);
        record.SetBoolean(4, sch2.ScheduleOnlyWithChanges);
        record.SetBoolean(5, sch2.Batch);
        return record;
      });
      this.BindTable(parameterName, "Build.typ_ScheduleAddTable2", schedules.Select<DesignerSchedule, SqlDataRecord>(selector));
    }
