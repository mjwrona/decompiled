...
// Type: Microsoft.VisualStudio.Services.BlobStore.WebApi.DedupNodeChildrenNeedAction
...
namespace Microsoft.VisualStudio.Services.BlobStore.WebApi
{
  ...
  public struct DedupNodeChildrenNeedAction
  {
    ...
    public DedupNodeChildrenNeedAction(
      DedupIdentifier[] missing,
    ...
    {
      DedupIdentifier[] dedupIdentifierArray;
      int index;
      if (this.InsufficientKeepUntil != null)
      {
        dedupIdentifierArray = this.InsufficientKeepUntil;
        for (index = 0; index < dedupIdentifierArray.Length; ++index)
          yield return dedupIdentifierArray[index];
        dedupIdentifierArray = (DedupIdentifier[]) null;
      }
      if (this.Missing != null)
      {
        dedupIdentifierArray = this.Missing;
        for (index = 0; index < dedupIdentifierArray.Length; ++index)
          yield return dedupIdentifierArray[index];
        dedupIdentifierArray = (DedupIdentifier[]) null;
      }
