...
// Type: Microsoft.TeamFoundation.Git.Server.SshFileBufferedStream
...
{
  internal class SshFileBufferedStream : FileBufferedStreamBase
  {
      ...
      : base(sizeLimit)
    {
      ...
      this.m_readStream = FileBufferedStreamBase.CreateFileWithTemporaryAttribute(this.m_filename, 65536);
      this.m_writeHandle = Microsoft.TeamFoundation.Common.Internal.NativeMethods.CreateFile(this.m_filename, Microsoft.TeamFoundation.Common.Internal.NativeMethods.FileAccess.GenericWrite, Microsoft.TeamFoundation.Common.Internal.NativeMethods.FileShare.Read | Microsoft.TeamFoundation.Common.Internal.NativeMethods.FileShare.Delete, IntPtr.Zero, Microsoft.TeamFoundation.Common.Internal.NativeMethods.CreationDisposition.OpenExisting, Microsoft.TeamFoundation.Common.Internal.NativeMethods.FileAttributes.Temporary, IntPtr.Zero);
      if (this.m_writeHandle.IsInvalid)
        throw new Win32Exception();
      this.m_buffer = new ByteArray(65536);
      // ISSUE: reference to a compiler-generated field
      // ISSUE: reference to a compiler-generated field
      ThreadPool.QueueUserWorkItem(SshFileBufferedStream.\u003C\u003EO.\u003C0\u003E__WriteCallback ?? (SshFileBufferedStream.\u003C\u003EO.\u003C0\u003E__WriteCallback = new WaitCallback(SshFileBufferedStream.WriteCallback)), (object) this);
    }
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
      ...
      lock (this.m_lock)
      {
        while (!this.m_eof && this.m_bytes <= this.m_position)
          Monitor.Wait(this.m_lock);
        if (this.m_eof)
        {
          if (this.m_bytes < this.m_position + (long) count)
          {
      ...
      int num = this.m_readStream.Read(buffer, offset, count);
    ...
    {
      SshFileBufferedStream fileBufferedStream = (SshFileBufferedStream) state;
      bool flag = false;
      try
      {
        while (!flag)
        {
          ...
          else
          {
            ...
            lock (fileBufferedStream.m_lock)
            {
              flag = fileBufferedStream.m_disposed;
      ...
      {
        if (!fileBufferedStream.m_leaveOpen)
          fileBufferedStream.m_inputStream.Close();
        if (fileBufferedStream.m_buffer != null)
        {
          fileBufferedStream.m_buffer.Dispose();
          fileBufferedStream.m_buffer = (ByteArray) null;
        }
        fileBufferedStream.m_writeHandle.Dispose();
      }
    }
