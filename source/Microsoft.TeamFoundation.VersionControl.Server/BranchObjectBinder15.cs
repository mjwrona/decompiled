...
// Type: Microsoft.TeamFoundation.VersionControl.Server.BranchObjectBinder15
...
{
  internal class BranchObjectBinder15 : BranchObjectBinder
  {
    public BranchObjectBinder15(VersionControlSqlResourceComponent component)
      : base(component)
    {
    }
    ...
    protected override BranchObject Bind()
    {
      ...
      branchObject.Properties.RootItem.ItemPathPair = this.GetItemPathPair(this.fullPath.GetServerItem(this.Reader, false));
        ...
        using (StringReader input = new StringReader(s1))
        {
          using (XmlReader reader = XmlReader.Create((TextReader) input, settings))
          {
            ...
            foreach (XmlNode selectNode in xmlDocument.SelectNodes("/items/i"))
            {
              ...
              itemIdentifier.ItemPathPair = this.GetItemPathPair(this.GetNodeAttributeServerPath(selectNode.Attributes["f"].Value));
        ...
        using (StringReader input = new StringReader(s2))
        {
          using (XmlReader reader = XmlReader.Create((TextReader) input, settings))
          {
            ...
            foreach (XmlNode selectNode in xmlDocument.SelectNodes("/folders/f"))
              ...
              {
                ItemPathPair = this.GetItemPathPair(this.GetNodeAttributeServerPath(selectNode.Attributes["s"].Value)),
