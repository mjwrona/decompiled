...
// Type: Microsoft.TeamFoundation.Server.Core.Catalog.Objects.ReportingFolder
...
namespace Microsoft.TeamFoundation.Server.Core.Catalog.Objects
{
  ...
  public class ReportingFolder : CatalogObject
  {
    ...
    public Uri GetReportManagerFolderUrl()
    {
      Uri reportManagerUrl = (Uri) null;
      ReportingServer reportServer = this.GetReportServer();
      if (reportServer != null)
        reportManagerUrl = reportServer.ReportsManagerServiceLocation;
      return !(reportManagerUrl != (Uri) null) ? (Uri) null : ReportingUtilities.GetReportManagerItemUrl(reportManagerUrl, this.FullPath);
    }
