...
// Type: Microsoft.TeamFoundation.Server.Core.Catalog.Objects.CatalogObject
...
namespace Microsoft.TeamFoundation.Server.Core.Catalog.Objects
{
  ...
  public class CatalogObject : INotifyPropertyChanged
  {
    ...
    public void Preload()
    {
      ...
      ICollection<CatalogNode> nodes;
      using (this.Context.GetRequestContext(out requestContext))
        nodes = (ICollection<CatalogNode>) this.CatalogNode.QueryChildren(requestContext, (IEnumerable<Guid>) knownDescendantTypes, true, this.Context.QueryOptions);
      this.Preload(new CatalogBulkData(nodes, knownDescendantTypes));
    ...
    public void Refresh(bool preload)
    {
      ...
      {
        IVssRequestContext requestContext;
        ICollection<CatalogNode> source;
        using (this.Context.GetRequestContext(out requestContext))
          source = (ICollection<CatalogNode>) this.Context.GetCatalogService(requestContext).QueryNodes(requestContext, (IEnumerable<string>) new string[1]
          {
            this.CatalogNode.FullPath
          }, (IEnumerable<Guid>) null, this.Context.QueryOptions);
        this.CatalogNode = source.FirstOrDefault<CatalogNode>() ?? throw new Exception("Refresh failed, node was no longer found");
      }
    ...
    public T CreateChild<T>(string displayName) where T : CatalogObject
    {
      ...
      using (this.Context.GetRequestContext(out requestContext))
      {
        T catalogObject = this.CreateCatalogObject<T>(this.CatalogNode.CreateChild(requestContext, CatalogObjectUtilities.GetResourceTypeIdentifier<T>(), displayName));
    ...
    protected Uri LocationAsUrl(ServiceDefinition service)
    {
      ...
      if (service != null)
      {
        ...
        using (this.Context.GetRequestContext(out requestContext))
        {
          ILocationService locationService = this.Context.GetLocationService(requestContext);
          if (locationService != null)
          {
            AccessMapping defaultAccessMapping = this.Context.DefaultAccessMapping;
            string uriString = locationService.LocationForAccessMapping(requestContext, service, defaultAccessMapping);
            if (uriString == null && !object.Equals((object) defaultAccessMapping, (object) this.Context.PublicAccessMapping))
              uriString = locationService.LocationForAccessMapping(requestContext, service, this.Context.PublicAccessMapping);
    ...
    protected void PreloadChild<T>(CatalogBulkData bulkData, ref object result) where T : CatalogObject
    {
      ...
      if (bulkData.QueriedForResourceType(resourceTypeId))
      {
        obj = this.CreateCatalogObject<T>(bulkData.Nodes.FirstOrDefault<CatalogNode>((Func<CatalogNode, bool>) (node => node.Resource.ResourceType.Identifier == resourceTypeId && node.ParentPath == this.CatalogNode.FullPath)));
        if ((object) obj != null)
        {
    ...
    protected ICollection<T> GetDependencyCollection<T>(string dependencyName) where T : CatalogObject
    {
      ...
      else
      {
        ...
        if (this.CatalogNode.Dependencies.Sets.ContainsKey(dependencyName))
        {
          IEnumerable<CatalogNode> set = (IEnumerable<CatalogNode>) this.CatalogNode.Dependencies.Sets[dependencyName];
          dependencyCollection = (ICollection<T>) new DependencyCollection<T>(this, dependencyName, (ICollection<T>) set.Select<CatalogNode, T>((Func<CatalogNode, T>) (node => this.CreateCatalogObject<T>(node))).ToList<T>());
        }
        else
          dependencyCollection = (ICollection<T>) new DependencyCollection<T>(this, dependencyName, (ICollection<T>) new List<T>());
    ...
    protected T GetDependency<T>(string dependencyName) where T : CatalogObject
    {
      ...
      {
        this.EnsureDependenciesExpanded();
        obj = !this.CatalogNode.Dependencies.Singletons.ContainsKey(dependencyName) ? CatalogObject.NullObject : (object) this.CreateCatalogObject<T>(this.CatalogNode.Dependencies.Singletons[dependencyName]) ?? CatalogObject.NullObject;
    ...
    protected void SetDependency<T>(string dependencyName, T value) where T : CatalogObject
    {
      ...
      if (catalogNode != null)
      {
        this.CatalogNode.Dependencies.Singletons[dependencyName] = catalogNode;
        this.SingletonDependencies[dependencyName] = (object) value;
      }
      else
      {
        this.CatalogNode.Dependencies.Singletons.Remove(dependencyName);
    ...
    private void EnsureDependenciesExpanded()
    {
      ...
      IVssRequestContext requestContext;
      using (this.Context.GetRequestContext(out requestContext))
        this.CatalogNode.ExpandDependencies(requestContext);
    }
    ...
    protected ICollection<T> QueryChildren<T>(Dictionary<string, string> filters, bool recurse) where T : CatalogObject
    {
      ...
      if (!string.IsNullOrEmpty(this.CatalogNode.FullPath))
      {
        ...
        ICollection<CatalogNode> source;
        using (this.Context.GetRequestContext(out requestContext))
          source = (ICollection<CatalogNode>) this.CatalogNode.QueryChildren(requestContext, (IEnumerable<Guid>) CatalogObjectUtilities.GetResourceTypeIdentifierFilter<T>(), (IEnumerable<KeyValuePair<string, string>>) filters, recurse, this.Context.QueryOptions);
        objs = (ICollection<T>) source.Select<CatalogNode, T>((Func<CatalogNode, T>) (c => this.Context.CreateCatalogObject<T>(c))).ToList<T>();
      }
      return objs;
    }
    ...
    protected ICollection<T> QueryDependents<T>(bool includeParents, bool expandDependencies) where T : CatalogObject
    {
      ...
      ICollection<CatalogNode> source;
      using (this.Context.GetRequestContext(out requestContext))
        source = (ICollection<CatalogNode>) this.CatalogNode.QueryDependents(requestContext, queryOptions);
      Guid resourceTypeId;
    ...
    protected T QueryParent<T>(bool recurse) where T : CatalogObject
    {
      ...
      if (catalogNode == null)
      {
        ...
        ICollection<CatalogNode> source;
        using (this.Context.GetRequestContext(out requestContext))
          source = (ICollection<CatalogNode>) this.CatalogNode.QueryParents(requestContext, (IEnumerable<Guid>) CatalogObjectUtilities.GetResourceTypeIdentifierFilter<T>(), recurse, this.Context.QueryOptions);
        catalogNode = source.FirstOrDefault<CatalogNode>();
      }
    ...
    protected T QueryChild<T>(Dictionary<string, string> filters, bool recurse) where T : CatalogObject
    {
      IVssRequestContext requestContext;
      ICollection<CatalogNode> source;
      using (this.Context.GetRequestContext(out requestContext))
        source = (ICollection<CatalogNode>) this.CatalogNode.QueryChildren(requestContext, (IEnumerable<Guid>) CatalogObjectUtilities.GetResourceTypeIdentifierFilter<T>(), (IEnumerable<KeyValuePair<string, string>>) filters, recurse, this.Context.QueryOptions);
      return this.CreateCatalogObject<T>(source.FirstOrDefault<CatalogNode>());
    }
    ...
    protected ICollection<T> QueryDependents<T>() where T : CatalogObject
    {
      ...
      {
        IVssRequestContext requestContext;
        ICollection<CatalogNode> catalogNodes;
        using (this.Context.GetRequestContext(out requestContext))
          catalogNodes = (ICollection<CatalogNode>) this.CatalogNode.QueryDependents(requestContext, this.Context.QueryOptions);
        Guid resourceTypeIdentifier = CatalogObjectUtilities.GetResourceTypeIdentifier<T>();
        foreach (CatalogNode catalogNode in (IEnumerable<CatalogNode>) catalogNodes)
        {
