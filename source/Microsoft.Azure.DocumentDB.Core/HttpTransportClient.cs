...
namespace Microsoft.Azure.Documents
{
  internal sealed class HttpTransportClient : TransportClient
  {
      ...
      using (HttpRequestMessage requestMessage = this.PrepareHttpMessage(activityId, physicalAddress, resourceOperation, request))
      {
        ...
        catch (Exception ex)
        {
          ...
          if (WebExceptionUtility.IsWebExceptionRetriable(ex))
          {
            ...
            throw new GoneException(string.Format((IFormatProvider) CultureInfo.CurrentUICulture, RMResources.ExceptionMessage, (object) RMResources.Gone), ex, localIpAddress: physicalAddress.ToString());
          }
          ...
          ServiceUnavailableException unavailableException = new ServiceUnavailableException(string.Format((IFormatProvider) CultureInfo.CurrentUICulture, RMResources.ExceptionMessage, (object) RMResources.ServiceUnavailable), ex, (HttpResponseHeaders) null, physicalAddress);
    ...
    private static Uri GetRootOperationUri(Uri baseAddress, OperationType operationType) => new Uri(baseAddress, PathsHelper.GenerateRootOperationPath(operationType));
      ...
      DocumentServiceRequest request)
    {
      using (response)
      {
        ...
        if (response.Headers.TryGetValues("lsn", out values1))
          long.TryParse(values1.FirstOrDefault<string>(), NumberStyles.Integer, (IFormatProvider) CultureInfo.InvariantCulture, out result1);
        ...
        switch (statusCode)
        {
          ...
          case HttpStatusCode.NotFound:
            ...
            {
              GoneException goneException = new GoneException(string.Format((IFormatProvider) CultureInfo.CurrentUICulture, RMResources.ExceptionMessage, (object) RMResources.Gone), response.RequestMessage.RequestUri);
              goneException.LSN = result1;
          ...
          case HttpStatusCode.Gone:
            ...
            try
            {
              ...
              if (values3 != null)
              {
                if (values3.Any<string>())
                {
                  if (!uint.TryParse(values3.First<string>(), NumberStyles.Integer, (IFormatProvider) CultureInfo.InvariantCulture, out result2))
                  {
            ...
            switch (result2)
            {
              ...
              case 1008:
                ...
                break;
              default:
                documentClientException = (DocumentClientException) new GoneException(string.Format((IFormatProvider) CultureInfo.CurrentUICulture, RMResources.ExceptionMessage, (object) RMResources.Gone), response.Headers, response.RequestMessage.RequestUri);
                documentClientException.Headers.Set("x-ms-activity-id", activityId);
                break;
            }
            break;
        ...
        documentClientException.LSN = result1;
      ...
      bool includeContent = true)
    {
      ...
      using (responseMessage)
      {
        ...
        {
          stringList1.Add(header.Key);
          if (string.Compare(header.Key, "x-ms-alt-content-path", StringComparison.Ordinal) == 0)
            stringList2.Add(Uri.UnescapeDataString(header.Value.SingleOrDefault<string>()));
          else
            stringList2.Add(header.Value.SingleOrDefault<string>());
        }
        response.ResponseHeaderNames = stringList1.ToArray();
        response.ResponseHeaderValues = stringList2.ToArray();
