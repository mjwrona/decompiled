...
namespace Microsoft.Azure.Documents
{
  internal class RntbdConnection : IConnection
  {
      ...
      Guid activityId)
    {
      ...
      {
        task1,
        this.SendRequestAsyncInternal(requestPayload, activityId)
      };
      Task task2 = await Task.WhenAny(awaitTasks);
      if (task2 == awaitTasks[0])
      {
        ...
        DefaultTrace.TraceError("Throwing RequestTimeoutException while awaiting request send. Task start time {0}. Task end time {1}. Request message size: {2}", (object) requestStartTime, (object) now, (object) requestPayload.Length);
        if (!awaitTasks[0].IsFaulted)
        {
    ...
    protected virtual byte[] BuildContextRequest(Guid activityId) => TransportSerialization.BuildContextRequest(activityId, this.userAgent, RntbdConstants.CallerId.Anonymous);
    ...
    {
      byte[] buffer1 = this.BuildContextRequest(activityId);
      await negotiatingStream.WriteAsync(buffer1, 0, buffer1.Length);
      Tuple<byte[], byte[]> tuple = await this.ReadHeaderAndMetadata(8000, true, activityId, state);
      byte[] src = tuple.Item1;
      byte[] buffer2 = tuple.Item2;
      ...
      using (MemoryStream readStream = new MemoryStream(buffer2))
      {
        ...
        this.serverAgent = Encoding.UTF8.GetString(response.serverAgent.value.valueBytes);
        this.serverVersion = Encoding.UTF8.GetString(response.serverVersion.value.valueBytes);
        ...
        if ((uint) status < 200U || (uint) status >= 400U)
        {
          using (MemoryStream memoryStream = new MemoryStream(await this.ReadBody(true, responseActivityId, state)))
          {
            ...
            Microsoft.Azure.Documents.Trace.CorrelationManager.ActivityId = responseActivityId;
      ...
      RntbdResponseState state)
    {
      ...
      if (response.payloadPresent.value.valueByte != (byte) 0)
      {
        ...
        try
        {
          buffer2 = await this.ReadBody(false, responseActivityId, state);
        }
        catch (Exception ex)
        {
          if (!(ex is DocumentClientException))
        ...
        body = new MemoryStream(buffer2);
      }
      state.SetState(RntbdResponseStateEnum.Done);
      return TransportSerialization.MakeStoreResponse(status, responseActivityId, response, (Stream) body, this.serverVersion);
    }
