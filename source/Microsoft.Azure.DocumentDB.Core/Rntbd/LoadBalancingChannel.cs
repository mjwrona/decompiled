...
namespace Microsoft.Azure.Documents.Rntbd
{
  internal sealed class LoadBalancingChannel : IChannel, IDisposable
  {
    ...
    public LoadBalancingChannel(Uri serverUri, ChannelProperties channelProperties)
    {
      ...
      if (channelProperties.PartitionCount > 1)
      {
        ...
        for (int index = 0; index < this.partitions.Length; ++index)
          this.partitions[index] = new LoadBalancingPartition(serverUri, channelProperties1);
      }
      else
        this.singlePartition = new LoadBalancingPartition(serverUri, channelProperties);
    }
    ...
    {
      this.ThrowIfDisposed();
      return this.singlePartition != null ? this.singlePartition.RequestAsync(request, physicalAddress, resourceOperation, activityId) : this.partitions[((long) activityId.GetHashCode() & 2415919103L) % (long) this.partitions.Length].RequestAsync(request, physicalAddress, resourceOperation, activityId);
    }
