...
// Type: Microsoft.Azure.Documents.Client.SessionContainer
...
namespace Microsoft.Azure.Documents.Client
{
  internal sealed class SessionContainer : ISessionContainer
  {
    ...
    private static void AddSessionToken(
      ...
      ISessionToken token)
    {
      self.sessionTokensRIDBased.AddOrUpdate(rid, (Func<ulong, ConcurrentDictionary<string, ISessionToken>>) (ridKey => new ConcurrentDictionary<string, ISessionToken>()
      {
        [partitionKeyRangeId] = token
      }), (Func<ulong, ConcurrentDictionary<string, ISessionToken>, ConcurrentDictionary<string, ISessionToken>>) ((ridKey, tokens) =>
      {
        tokens.AddOrUpdate(partitionKeyRangeId, token, (Func<string, ISessionToken, ISessionToken>) ((existingPartitionKeyRangeId, existingToken) => existingToken.Merge(token)));
        return tokens;
      }));
    }
      ...
      ConcurrentDictionary<string, ISessionToken> partitionKeyRangeIdToTokenMap)
    {
      ...
      return partitionKeyRangeIdToTokenMap.TryGetValue("0", out sessionToken) ? string.Format((IFormatProvider) CultureInfo.InvariantCulture, "{0}", (object) sessionToken) : string.Empty;
    }
    ...
    private sealed class SessionContainerSnapshot
    {
      ...
      public override bool Equals(object obj)
      {
        if (obj == null || (object) this.GetType() != (object) obj.GetType())
          return false;
        SessionContainer.SessionContainerSnapshot containerSnapshot = (SessionContainer.SessionContainerSnapshot) obj;
        return SessionContainer.SessionContainerSnapshot.AreDictionariesEqual<string, ulong>(this.collectionNameByResourceId, containerSnapshot.collectionNameByResourceId, (Func<ulong, ulong, bool>) ((x, y) => (long) x == (long) y)) && SessionContainer.SessionContainerSnapshot.AreDictionariesEqual<ulong, string>(this.collectionResourceIdByName, containerSnapshot.collectionResourceIdByName, (Func<string, string, bool>) ((x, y) => x == y)) && SessionContainer.SessionContainerSnapshot.AreDictionariesEqual<ulong, Dictionary<string, ISessionToken>>(this.sessionTokensRIDBased, containerSnapshot.sessionTokensRIDBased, (Func<Dictionary<string, ISessionToken>, Dictionary<string, ISessionToken>, bool>) ((x, y) => SessionContainer.SessionContainerSnapshot.AreDictionariesEqual<string, ISessionToken>(x, y, (Func<ISessionToken, ISessionToken, bool>) ((a, b) => a.Equals(b)))));
      }
        ...
        Func<U, U, bool> areEqual)
      {
        ...
        {
          if (!right.ContainsKey(key) || !areEqual(left[key], right[key]))
            return false;
        }
        return true;
      }
