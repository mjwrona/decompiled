...
// Type: Microsoft.Azure.Documents.SecureStringHMACSHA256Helper
...
namespace Microsoft.Azure.Documents
{
  internal sealed class SecureStringHMACSHA256Helper : IComputeHash, IDisposable
  {
    ...
    private void Dispose(bool disposing)
    {
      if (!disposing)
        return;
      IntPtr algorithmHandle = this.algorithmHandle;
    ...
    private void AddData(IntPtr hashHandle, byte[] data)
    {
      ...
      try
      {
        int error = SecureStringHMACSHA256Helper.NativeMethods.BCryptHashData(hashHandle, gcHandle.AddrOfPinnedObject(), (uint) data.Length, 0U);
        if (error != 0)
