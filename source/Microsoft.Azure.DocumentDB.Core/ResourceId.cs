...
namespace Microsoft.Azure.Documents
{
  internal sealed class ResourceId : IEquatable<ResourceId>
  {
    ...
    public bool IsDocumentCollectionId => this.Database != 0U && this.DocumentCollection != 0U && this.Document == 0UL && this.StoredProcedure == 0UL && this.Trigger == 0UL && this.UserDefinedFunction == 0UL;
    ...
    public static bool TryParse(string id, out ResourceId rid)
    {
      ...
      try
      {
        ...
        if (!ResourceId.Verify(id, out buffer) || buffer.Length % 4 != 0 && buffer.Length != 3 && buffer.Length != 7)
          return false;
        rid = new ResourceId();
        if (buffer.Length == 3)
        {
          ...
          for (int index = 0; index < 3; ++index)
            rid.Offer |= (uint) buffer[index] << index * 8;
          return true;
        }
        if (buffer.Length == 7)
        {
          ...
          for (int index = 0; index < 7; ++index)
            rid.Snapshot |= (ulong) ((uint) buffer[index] << index * 8);
          return true;
        }
        if (buffer.Length >= 4)
    ...
    public static bool Verify(string id, out byte[] buffer)
    {
      ...
      if (buffer != null && buffer.Length <= (int) ResourceId.Length)
        return true;
      buffer = (byte[]) null;
      return false;
    }
    ...
    private static bool HasNonHierarchicalResourceId(ResourceType eResourceType) => eResourceType == ResourceType.MasterPartition || eResourceType == ResourceType.ServerPartition || eResourceType == ResourceType.RidRange;
