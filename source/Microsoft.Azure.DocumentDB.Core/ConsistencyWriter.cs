...
namespace Microsoft.Azure.Documents
{
  ...
  internal sealed class ConsistencyWriter
  {
      ...
      bool useMultipleWriteLocations)
    {
      ...
      this.storeReader = new StoreReader(transportClient, addressSelector, (ISessionContainer) null, connectionStateListener);
    }
      ...
      bool forceRefresh)
    {
      ...
      if (request.RequestContext.GlobalStrongWriteStoreResult == null)
      {
        ...
        Tuple<PerProtocolPartitionAddressInformation, AddressCacheToken> tuple = await this.addressSelector.ResolveAddressesAsync(request, forceRefresh);
        ...
        request.RequestContext.ClientRequestStatistics.ContactedReplicas = tuple.Item1.ReplicaUris.ToList<Uri>();
        Uri primaryUri = tuple.Item1.GetPrimaryUri(request);
        this.LastWriteAddress = primaryUri.ToString();
        this.connectionStateListener?.UpdateConnectionState(primaryUri, tuple.Item2);
        ...
        try
        {
          storeResult = StoreResult.CreateStoreResult(await this.transportClient.InvokeResourceOperationAsync(primaryUri, request), (Exception) null, true, false, primaryUri);
        }
        catch (Exception ex)
        {
          storeResult = StoreResult.CreateStoreResult((StoreResponse) null, ex, true, false, primaryUri);
          if (ex is DocumentClientException)
          {
            DocumentClientException documentClientException = (DocumentClientException) ex;
            if (!string.IsNullOrWhiteSpace(documentClientException.Headers["x-ms-write-request-trigger-refresh"]))
            {
              int result;
              if (int.TryParse(documentClientException.Headers.GetValues("x-ms-write-request-trigger-refresh")[0], NumberStyles.Integer, (IFormatProvider) CultureInfo.InvariantCulture, out result))
              {
                if (result == 1)
        ...
        request.RequestContext.ClientRequestStatistics.RecordResponse(request, storeResult);
    ...
    private void StartBackgroundAddressRefresh(DocumentServiceRequest request)
    {
      try
      {
        this.addressSelector.ResolvePrimaryUriAsync(request, true).ContinueWith((Action<Task<Tuple<Uri, AddressCacheToken>>>) (task =>
        {
          if (!task.IsFaulted)
            return;
          DefaultTrace.TraceWarning("Background refresh of the primary address failed with {0}", (object) task.Exception.ToString());
        }));
      }
      catch (Exception ex)
      {
        DefaultTrace.TraceWarning("Background refresh of the primary address failed with {0}", (object) ex.ToString());
      }
    }
  }
}
