      ...
      public int GetHashCode(Index index)
      {
        ...
        switch (index.Kind)
        {
          case IndexKind.Hash:
            ...
            int? nullable1 = precision1.HasValue ? new int?((int) precision1.GetValueOrDefault()) : new int?();
            return ((nullable1.HasValue ? new int?(num2 ^ nullable1.GetValueOrDefault()) : new int?()) ?? 0) ^ ((HashIndex) index).DataType.GetHashCode();
          case IndexKind.Range:
            int num3 = num1;
            ...
            return ((nullable2.HasValue ? new int?(num3 ^ nullable2.GetValueOrDefault()) : new int?()) ?? 0) ^ ((RangeIndex) index).DataType.GetHashCode();
          case IndexKind.Spatial:
