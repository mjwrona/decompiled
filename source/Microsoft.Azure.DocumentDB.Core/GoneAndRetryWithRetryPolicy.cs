...
namespace Microsoft.Azure.Documents
{
    ...
    IRetryPolicy<Tuple<bool, bool, TimeSpan, int, int, TimeSpan>>
  {
        ...
        if (val2 <= 0)
        {
          switch (exception)
          {
            ...
            case PartitionKeyRangeIsSplittingException _:
              string str = string.Format("Received {0} after backoff/retry", (object) ((object) exception).GetType().Name);
              ...
              exception1 = !this.detectConnectivityIssues || this.request.RequestContext.ClientRequestStatistics == null || !this.request.RequestContext.ClientRequestStatistics.IsCpuOverloaded ? (!this.detectConnectivityIssues || this.request.RequestContext.ClientRequestStatistics == null || this.request.RequestContext.ClientRequestStatistics.FailedReplicas.Count < 3 ? (Exception) new ServiceUnavailableException(exception) : (Exception) new ServiceUnavailableException(string.Format(RMResources.ClientUnavailable, (object) this.request.RequestContext.ClientRequestStatistics.FailedReplicas.Count, (object) (this.request.RequestContext.ClientRequestStatistics.RegionsContacted.Count == 0 ? 1 : this.request.RequestContext.ClientRequestStatistics.RegionsContacted.Count)), exception)) : (Exception) new ServiceUnavailableException(string.Format(RMResources.ClientCpuOverload, (object) this.request.RequestContext.ClientRequestStatistics.FailedReplicas.Count, (object) (this.request.RequestContext.ClientRequestStatistics.RegionsContacted.Count == 0 ? 1 : this.request.RequestContext.ClientRequestStatistics.RegionsContacted.Count)));
              break;
            default:
      ...
      switch (exception)
      {
        ...
        case InvalidPartitionException _:
          ...
          if (this.attemptCountInvalidPartition++ > 2)
          {
            ...
            return Task.FromResult<ShouldRetryResult<Tuple<bool, bool, TimeSpan, int, int, TimeSpan>>>(ShouldRetryResult<Tuple<bool, bool, TimeSpan, int, int, TimeSpan>>.NoRetry((Exception) new ServiceUnavailableException(exception)));
          }
    ...
    private void ClearRequestContext()
    {
      ...
      this.request.RequestContext.QuorumSelectedStoreResponse = (StoreResult) null;
    }
  }
}
