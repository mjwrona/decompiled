...
// Type: Microsoft.VisualStudio.Services.Cloud.ResourceUtilization21Service
...
{
  internal class ResourceUtilization21Service : 
    VssBaseService,
    ...
    IVssFrameworkService
  {
    ...
    private ResourceUtilization21Service m_deploymentService;
    ...
    private const string c_modulationKey = "Modulation";
    ...
    public void ServiceStart(IVssRequestContext requestContext)
    {
      ...
      {
        this.m_deploymentService = (ResourceUtilization21Service) null;
        requestContext.GetService<IVssRegistryService>().RegisterNotification(requestContext, new RegistrySettingsChangedCallback(this.OnSettingsChanged), false, in ResourceUtilization21Service.s_serviceSettingsRegistryQuery);
        ...
        this.m_deploymentService = requestContext.To(TeamFoundationHostType.Deployment).GetService<ResourceUtilization21Service>();
    ...
    public async Task ThrottleRequestAsync(IVssRequestContext requestContext, RUStage ruStage)
    {
      ...
      else
      {
        ...
        switch (ruStage)
        {
          case RUStage.BeginRequest:
          case RUStage.RequestReady:
            flag2 = true;
            break;
          default:
            flag2 = false;
        ...
        if (flag2)
        {
          if (ruStage == RUStage.BeginRequest)
          {
            if (this.EnsureRulesAndThresholdsSynced(requestContext))
              this.UpdateResourceCache();
            this.EnsureSettingsSynced(requestContext);
          }
          this.CheckRequestThrottleInfo(requestContext, ruStage, ref throttleInfo);
          this.IncrementResources(requestContext, ruStage, ref throttleInfo, out requestCounters);
          if (ruStage == RUStage.RequestReady)
            this.AdjustRequestReadyDelay((int) requestContext.RequestTimer.DelaySpan.TotalMilliseconds, ref throttleInfo);
        }
        else
        {
          ...
          ResourceUtilization21Service.GetOrCreateResourceUtilizationEvents(requestContext)["ExecutionTime"] = (object) requestExecutionTime_ms;
          double totalMilliseconds = requestContext.RequestTimer.DelaySpan.TotalMilliseconds;
          flag1 = requestContext.RootContext.TryGetItem<ThrottleInfo>(RequestContextItemsKeys.ThrottleInfo2, out throttleInfo);
          this.CheckRequestThrottleInfo(requestContext, ruStage, ref throttleInfo);
          this.AdjustAndCapPostRequestDelay(requestContext, (int) totalMilliseconds, requestExecutionTime_ms, ref throttleInfo);
        }
        if (!flag1 && throttleInfo != null)
          requestContext.RootContext.Items[RequestContextItemsKeys.ThrottleInfo2] = (object) throttleInfo;
        this.BlockRequestAndThrowIfNeeded(requestContext, throttleInfo);
        if (throttleInfo != null && !this.Settings.DoNotTarpit && throttleInfo.IsTarpit())
          await this.TarpitRequest(requestContext, throttleInfo, ruStage);
        if (ruStage != RUStage.RequestReady)
        {
          throttleInfo = (ThrottleInfo) null;
          requestCounters = (IList<IRequestResourceCounter>) null;
        }
        else if (throttleInfo.IsBlock())
        {
          throttleInfo = (ThrottleInfo) null;
          requestCounters = (IList<IRequestResourceCounter>) null;
        }
        ...
        {
          SemaphoreWaitStatus[] source = await this.AwaitRequestCounters(requestContext, requestCounters);
          if (source.Length == 0)
          {
            throttleInfo = (ThrottleInfo) null;
            requestCounters = (IList<IRequestResourceCounter>) null;
          ...
          {
            this.AddTimeSpentInConcurrencySemaphore(requestContext, ((IEnumerable<SemaphoreWaitStatus>) source).Max<SemaphoreWaitStatus>((Func<SemaphoreWaitStatus, long>) (s => s.TicksSpentInConcurrencySemaphore)));
    ...
    {
      ThrottleInfo throttleInfo;
      if (requestContext.RootContext.TryGetItem<ThrottleInfo>(RequestContextItemsKeys.ThrottleInfo2, out throttleInfo) && throttleInfo.IsBlock())
        return;
    ...
    public static void DecrementRequestCounters(IVssRequestContext requestContext)
    {
      ...
      {
        ResourceUtilization21Service.m_perfCounter_ServiceCallsPerSec.Increment();
      }
    }
        ...
        foreach (KeyValuePair<Guid, ProcessedRURule> keyValuePair in this.m_rulesAndThresholds.ProcessedRules.Where<KeyValuePair<Guid, ProcessedRURule>>((Func<KeyValuePair<Guid, ProcessedRURule>, bool>) (p => p.Value.Resource is RUResource_Concurrency)))
        {
          ...
          if ((rule.WhenToCheckIfGoverned & ruStage) != RUStage.Null && rule.ShouldGovernRequest(requestContext))
          {
            ...
            if (this.m_localResourceCache != null && this.m_localResourceCache.TryGetValue(key, out concurrentDictionary))
            {
              ...
              {
                IDictionary<string, object> utilizationEvents = ResourceUtilization21Service.GetOrCreateResourceUtilizationEvents(requestContext);
      ...
      ref ThrottleInfo throttleInfo)
    {
      try
      {
        ...
        foreach (KeyValuePair<Guid, ProcessedRURule> keyValuePair in this.m_rulesAndThresholds.ProcessedRules.Where<KeyValuePair<Guid, ProcessedRURule>>((Func<KeyValuePair<Guid, ProcessedRURule>, bool>) (p => !(p.Value.Resource is RUResource_Concurrency))))
        {
          ...
          bool flag1 = processedRuRule.WhenToCheckIfGoverned == ruStage;
          if (flag1)
          {
            bool flag2;
            switch (ruStage)
          ...
          if (((flag1 ? 1 : 0) | (ruStage != RUStage.PostLog ? (false ? 1 : 0) : (processedRuRule.Resource.WhenToMeasureResourceConsumption == RUStage.EndRequest ? 1 : 0))) != 0 && processedRuRule.ShouldGovernRequest(requestContext))
          {
        ...
        if (redisResult != null)
        {
          ...
          if (forRuleAndEntity != null)
          {
            ...
            if (state > ResourceState2.DPFlag)
            {
              ...
              ThrottleInfo throttleInfo1 = new ThrottleInfo(rule, ruleId, (int) (dpImpact + 1.0), entity, state, throttlingThreshold, usage, (DateTimeOffset) reset, retryAfterTimestamp);
              if ((state & ResourceState2.Tarpit) > ResourceState2.Normal)
              {
                ...
                ThrottleInfo throttleInfo2 = throttleInfo1;
                bool flag;
                switch (ruStage)
                {
                  case RUStage.BeginRequest:
                ...
                throttleInfo2.DelayForNextAwait = flag ? this.GetRequestReadyDelayForNextAwait(requestContext, factorAboveThreshold) : this.GetPostLogDelayForNextAwait(requestContext, factorAboveThreshold);
                if (dpImpact > this.Settings.DPImpactNotificationThreshold && !this.Settings.DoNotTarpit)
                  throttleInfo1.ShouldNotifyUser = true;
              }
              bool flag1;
              switch (ruStage)
              {
              ...
              if (flag1 && (state & (ResourceState2.Flag | ResourceState2.Tarpit)) > ResourceState2.Normal)
                rule.Resource.FlagThrottleInfo(requestContext, ref throttleInfo1);
              return throttleInfo1;
            }
          }
        }
      }
    ...
    private static void QueueThrottlingEventForPotentialEmailNotification(
      ...
      ThrottleInfo throttleInfo)
    {
      if (!throttleInfo.ShouldNotifyUser || !requestContext.IsFeatureEnabled("VisualStudio.FrameworkService.ThrottlingNotification.Email") || throttleInfo.IsBlock() || requestContext.Status is UnauthorizedRequestException)
        return;
        ...
        foreach (KeyValuePair<Guid, ProcessedRURule> processedRule in this.m_rulesAndThresholds.ProcessedRules)
        {
          ...
          if (processedRuRule.Resource.WhenToMeasureResourceConsumption == whenToMeasure && processedRuRule.ShouldGovernRequest(requestContext))
          {
            ...
            if (requestCost > 0L || processedRuRule.Resource.ShouldQueueZeroIncrement())
            {
              ...
              ResourceUtilization21Service.GetOrCreateResourceUtilizationEvents(requestContext)[processedRuRule.GetResourceName()] = (object) requestCost.ToString();
            }
          }
        }
        if (source.Count <= 0)
    ...
    private void UpdateResourceCache()
    {
      ...
      this.m_localResourceCache = (IReadOnlyDictionary<Guid, ConcurrentDictionary<string, ResourceUtilization2Service.ConcurrentCounter>>) new ReadOnlyDictionary<Guid, ConcurrentDictionary<string, ResourceUtilization2Service.ConcurrentCounter>>((IDictionary<Guid, ConcurrentDictionary<string, ResourceUtilization2Service.ConcurrentCounter>>) dictionary);
      if (this.m_resourceCache != null)
      {
        foreach (Guid key in this.m_resourceCache.Keys.Where<Guid>((Func<Guid, bool>) (x => !temp.Keys.Contains<Guid>(x))))
          this.m_resourceCache.Remove(key);
        foreach (Guid key in temp.Keys.Where<Guid>((Func<Guid, bool>) (x => !this.m_resourceCache.Keys.Contains(x))))
          this.m_resourceCache.Add(key, new ConcurrentDictionary<string, ResourceUtilization2Service.RedisResult>((IEqualityComparer<string>) StringComparer.OrdinalIgnoreCase));
      }
      else
        this.m_resourceCache = (IDictionary<Guid, ConcurrentDictionary<string, ResourceUtilization2Service.RedisResult>>) temp;
    }
    ...
    private void IncrementResourcesTask(IVssRequestContext requestContext, object taskArgs)
    {
      ...
      {
        stopwatch.Stop();
        VssPerformanceCounter performanceCounter = ResourceUtilization21Service.m_perfCounter_AverageTaskExecutionTime;
        performanceCounter.IncrementTicks(stopwatch);
        performanceCounter = ResourceUtilization21Service.m_perfCounter_AverageTaskExecutionTimeBase;
      ...
      ConcurrentStack<ResourceUtilization2Service.IncrementResourceTaskArgs> taskArgsToProcess)
    {
      ...
      ResourceUtilization21Service.m_perfCounter_AverageQueueSize.IncrementBy((long) count);
      ResourceUtilization21Service.m_perfCounter_AverageQueueSizeBase.Increment();
      DateTimeOffset utcNow = (DateTimeOffset) DateTime.UtcNow;
    ...
    internal void AdjustRequestReadyDelay(int beginRequestDelay, ref ThrottleInfo throttleInfo)
    {
      if (!throttleInfo.IsTarpit())
        return;
      throttleInfo.DelayForNextAwait = Math.Max(0, throttleInfo.DelayForNextAwait - beginRequestDelay);
    }
      ...
      ref ThrottleInfo throttleInfo)
    {
      ...
      if ((double) throttleInfo.DelayForNextAwait <= this.Settings.PostRequestTarpittingAbsoluteCap.TotalMilliseconds)
        return;
      throttleInfo.DelayForNextAwait = (int) this.Settings.PostRequestTarpittingAbsoluteCap.TotalMilliseconds;
    }

    private double GetModulationMultiplier(IVssRequestContext requestContext, RUStage ruStage)
    {
      double modulationMultiplier1 = 1.0;
      IDictionary<string, object> utilizationEvents = ResourceUtilization21Service.GetOrCreateResourceUtilizationEvents(requestContext);
      object modulationMultiplier2;
      if (utilizationEvents.TryGetValue("Modulation", out modulationMultiplier2))
        return (double) modulationMultiplier2;
      bool flag;
      switch (ruStage)
      {
        case RUStage.BeginRequest:
      ...
      if (flag)
      {
        switch ((uint) TimeSpan.FromMinutes((double) (requestContext.StartTime().Ticks / 3000000000L)).GetHashCode() % 5U)
        {
          case 0:
          case 1:
            modulationMultiplier1 = 1.0 - this.Settings.ModulationFactor;
            break;
          case 2:
          case 3:
            modulationMultiplier1 = 1.0 + this.Settings.ModulationFactor;
            break;
          default:
            modulationMultiplier1 = 1.0;
            break;
        }
        utilizationEvents["Modulation"] = (object) modulationMultiplier1;
      }
      return modulationMultiplier1;
    }
      ...
      RUStage ruStage)
    {
      if (throttleInfo.DelayForNextAwait <= 0)
        return;
      if (ruStage == RUStage.PostLog)
        ResourceUtilization21Service.QueueThrottlingEventForPotentialEmailNotification(requestContext, throttleInfo);
    ...
    private void BlockRequestAndThrowIfNeeded(
      IVssRequestContext requestContext,
      ThrottleInfo throttleInfo)
    {
      if (!throttleInfo.IsBlock() || this.Settings.DoNotBlockDistributed)
        return;
      this.BlockRequestAndThrow(requestContext, throttleInfo);
    }

    private void BlockRequestAndThrow(IVssRequestContext requestContext, ThrottleInfo throttleInfo)
    {
      string resourceHeader = throttleInfo.GetResourceHeader(requestContext);
      Dictionary<string, string> headersToAdd = new Dictionary<string, string>()
      {
      ...
      utilizationEvents["ShouldOutputTelemetry"] = (object) true;
      ResourceUtilization21Service.BlockRequestException(requestContext, throttleInfo);
    }
