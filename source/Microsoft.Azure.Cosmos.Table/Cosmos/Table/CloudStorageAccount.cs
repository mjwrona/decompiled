...
// Type: Microsoft.Azure.Cosmos.Table.CloudStorageAccount
...
namespace Microsoft.Azure.Cosmos.Table
{
  public sealed class CloudStorageAccount
  {
      ...
      bool useHttps)
    {
      ...
      if (!string.IsNullOrEmpty(storageCredentials.AccountName))
      {
        if (string.IsNullOrEmpty(accountName))
          ...
          throw new ArgumentException(string.Format((IFormatProvider) CultureInfo.CurrentCulture, "Account names do not match.  First account name is {0}, second is {1}.", new object[2]
          {
            (object) storageCredentials.AccountName,
            (object) accountName
          }));
      }
      CommonUtility.AssertNotNull("AccountName", (object) accountName);
      this.TableStorageUri = CloudStorageAccount.ConstructTableEndpoint(useHttps ? "https" : "http", accountName, endpointSuffix);
      this.Credentials = storageCredentials;
    ...
    public string GetSharedAccessSignature(SharedAccessAccountPolicy policy)
    {
      ...
      string hash = SharedAccessSignatureHelper.GetHash(policy, this.Credentials.AccountName, "2017-07-29", this.Credentials.Key);
      return SharedAccessSignatureHelper.GetSignature(policy, hash, this.Credentials.KeyName, "2017-07-29").ToString();
    }
    ...
    public string ToString(bool exportSecrets)
    {
      if (this.Settings == null)
      {
        ...
        if (this.DefaultEndpoints && this.EndpointSuffix != null)
          this.Settings.Add("EndpointSuffix", this.EndpointSuffix);
        else if (this.TableEndpoint != (Uri) null)
    ...
    private static CloudStorageAccount GetDevelopmentStorageAccount(Uri proxyUri)
    {
      ...
      CloudStorageAccount developmentStorageAccount = new CloudStorageAccount(new StorageCredentials("devstoreaccount1", "Eby8vdM02xNOcqFlqUwJPLlmEtlCDXJ1OUzFT50uSRZ6IFsuFq2UVErCz4I6tq/K1SZFPTOtr/KBHBeksoGMGw=="), new StorageUri(uri1, uri2))
      {
      ...
      Action<string> error)
    {
      ...
      if (matchesAutomaticEndpointsSpec | flag)
      {
        ...
        {
          accountInformation = new CloudStorageAccount(CloudStorageAccount.GetCredentials(settings), func8(str3, str7, new Func<IDictionary<string, string>, StorageUri>(CloudStorageAccount.ConstructTableEndpoint)))
          {
