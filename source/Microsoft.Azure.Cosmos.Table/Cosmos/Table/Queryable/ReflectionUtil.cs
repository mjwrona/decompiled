...
// Type: Microsoft.Azure.Cosmos.Table.Queryable.ReflectionUtil
...
namespace Microsoft.Azure.Cosmos.Table.Queryable
{
  internal static class ReflectionUtil
  {
    ...
    {
      Dictionary<string, SequenceMethod> dictionary = new Dictionary<string, SequenceMethod>((IEqualityComparer<string>) EqualityComparer<string>.Default);
      ...
      ReflectionUtil.StaticMethodMap = new Dictionary<MethodInfo, SequenceMethod>((IEqualityComparer<MethodInfo>) EqualityComparer<MethodInfo>.Default);
      ReflectionUtil.StaticInverseMap = new Dictionary<SequenceMethod, MethodInfo>((IEqualityComparer<SequenceMethod>) EqualityComparer<SequenceMethod>.Default);
      foreach (MethodInfo allLinqOperator in ReflectionUtil.GetAllLinqOperators())
      {
        ...
        SequenceMethod key;
        if (dictionary.TryGetValue(methodDescription, out key))
        {
          ReflectionUtil.StaticMethodMap.Add(allLinqOperator, key);
          ReflectionUtil.StaticInverseMap[key] = allLinqOperator;
        }
      }
      ...
      ReflectionUtil.ProjectMethodInfo = ((IEnumerable<MethodInfo>) typeof (TableQuery).GetMethods(BindingFlags.Static | BindingFlags.Public)).First<MethodInfo>((Func<MethodInfo, bool>) (m => m.Name == "Project"));
    }
    ...
    {
      SequenceMethod sequenceMethod1;
      return ReflectionUtil.TryIdentifySequenceMethod(method, out sequenceMethod1) && sequenceMethod1 == sequenceMethod;
    }
      ...
      else
      {
        ...
        foreach (Type genericArgument in type.GetGenericArguments())
        {
          if (flag)
            ...
            description.Append(", ");
          Dictionary<Type, int> genericArgumentOrdinals1 = genericArgumentOrdinals;
          StringBuilder description1 = description;
          ReflectionUtil.AppendCanonicalTypeDescription(genericArgument, genericArgumentOrdinals1, description1);
        }
        description.Append(">");
      }
    }

    internal static IEnumerable<MethodInfo> GetAllLinqOperators() => ((IEnumerable<MethodInfo>) typeof (System.Linq.Queryable).GetMethods(BindingFlags.Static | BindingFlags.Public)).Concat<MethodInfo>((IEnumerable<MethodInfo>) typeof (Enumerable).GetMethods(BindingFlags.Static | BindingFlags.Public));
  }
}
