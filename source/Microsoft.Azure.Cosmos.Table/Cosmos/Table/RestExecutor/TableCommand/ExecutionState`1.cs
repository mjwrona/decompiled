...
// Type: Microsoft.Azure.Cosmos.Table.RestExecutor.TableCommand.ExecutionState`1
  ...
  internal class ExecutionState<T> : IDisposable
  {
      ...
      OperationContext operationContext)
    {
      ...
      if (!(this.OperationContext.StartTime == DateTimeOffset.MinValue))
        return;
      this.OperationContext.StartTime = DateTimeOffset.UtcNow;
    }

    internal void Init()
    {
      this.Req = (StorageRequestMessage) null;
    ...
    internal RESTCommand<T> Cmd { get; private set; }

    internal StorageLocation CurrentLocation { get; set; }

    internal RESTCommand<T> RestCMD => this.Cmd;
    ...
    internal TimeSpan RemainingTimeout
    {
      get
      {
        ...
        TimeSpan timeSpan = this.OperationExpiryTime.Value - DateTime.UtcNow;
        return !(timeSpan <= TimeSpan.Zero) ? timeSpan : throw StorageExceptionTranslator.GenerateTimeoutException(this.Cmd.CurrentResult, (Exception) null);
      }
    }
    ...
    {
      get => this.cancellationLockerObject;
