...
// Type: Microsoft.VisualStudio.Services.Common.CommandLine.Validation.OptionsAreMutuallyExclusive
...
{
  public sealed class OptionsAreMutuallyExclusive : OptionValidation
  {
    private Collection<string> dependencyOptions;

    public OptionsAreMutuallyExclusive(IEnumerable<string> dependencyOptions)
    {
      this.SetDependencyOptions(dependencyOptions);
      this.DependencyFailedAction = (Action<OptionValidation>) (dependency =>
      {
        throw new OptionValidationException(CommonResources.ErrorOptionsAreMutuallyExclusive((object) string.Join(", ", (IEnumerable<string>) this.dependencyOptions)));
      });
    }

    public OptionsAreMutuallyExclusive(
    ...
    public override void Validate(IEnumerable<Option> options, IEnumerable<Argument> arguments)
    {
      ...
      try
      {
        ...
        foreach (string dependencyOption in this.dependencyOptions)
        {
          ...
          if (option != null)
          {
            collection.Add(option.Name);
            if (OptionsAreMutuallyExclusive.IsOptionDefined(option, arguments))
              ++num;
          }
        }
        if (num <= 1)
