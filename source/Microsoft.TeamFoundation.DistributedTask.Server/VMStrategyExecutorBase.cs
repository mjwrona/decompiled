...
// Type: Microsoft.TeamFoundation.DistributedTask.Server.VMStrategyExecutorBase
...
{
  internal abstract class VMStrategyExecutorBase : IStrategyExecutor
  {
    ...
    public virtual TaskResult? GetDeploymentResult(out IList<TimelineRecord> timelineRecords)
    {
      ...
      foreach (VirtualMachineState virtualMachineState in (IEnumerable<VirtualMachineState>) this.m_virtualMachineGroupState.VirtualMachines.Values)
      {
        ...
        if (virtualMachineState.DeploymentAttempted)
        {
          ...
          aggregatedResult = new TaskResult?(VMStrategyExecutorBase.MergeResult(aggregatedResult, jobResult));
        }
      }
    ...
    protected JobInstance CreateJob()
    {
      ...
      if (this.m_virtualMachineGroupState.DeploymentPool != null)
        agentPoolTarget.Pool = new AgentPoolReference()
        {
          Id = this.m_virtualMachineGroupState.DeploymentPool.Id
        };
      else
        agentPoolTarget.Pool = new AgentPoolReference()
        {
      ...
      DeploymentStrategyBaseAction strategyBaseAction = this.m_actions.First<DeploymentStrategyBaseAction>((Func<DeploymentStrategyBaseAction, bool>) (a => a.Type == DeploymentStrategyActionType.Deploy));
