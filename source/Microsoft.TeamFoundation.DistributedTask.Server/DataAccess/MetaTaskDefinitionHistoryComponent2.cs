...
// Type: Microsoft.TeamFoundation.DistributedTask.Server.DataAccess.MetaTaskDefinitionHistoryComponent2
...
{
  internal class MetaTaskDefinitionHistoryComponent2 : MetaTaskDefinitionHistoryComponent
  {
    ...
    public override void DeleteTaskGroupHistory(Guid projectId, Guid taskGroupId)
    {
      this.TraceEnter(0, nameof (DeleteTaskGroupHistory));
      this.PrepareStoredProcedure("Task.prc_DeleteMetaTaskDefinitionHistory", projectId);
      this.BindGuid("@definitionId", taskGroupId);
      this.ExecuteNonQuery();
      this.TraceLeave(0, nameof (DeleteTaskGroupHistory));
    }
