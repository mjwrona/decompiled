...
// Type: Microsoft.TeamFoundation.DistributedTask.Server.DataAccess.MetaTaskDefinitionComponent4
...
{
  internal class MetaTaskDefinitionComponent4 : MetaTaskDefinitionComponent3
  {
    ...
    public override MetaTaskDefinitionData PublishTaskGroup(
      Guid projectId,
      PublishTaskGroupMetadata taskGroupMetadata,
      Guid taskGroupId,
      Guid modifiedBy)
    {
      using (new TaskSqlComponentBase.SqlMethodScope((TaskSqlComponentBase) this, nameof (PublishTaskGroup)))
      {
        this.PrepareStoredProcedure("Task.prc_PublishTaskGroup", projectId);
        this.BindGuid("@sourceDefinitionId", taskGroupMetadata.TaskGroupId);
        this.BindInt("@sourceDefinitionRevision", taskGroupMetadata.TaskGroupRevision);
        this.BindGuid("@targetDefinitionId", taskGroupId);
        this.BindInt("@targetDefinitionRevision", taskGroupMetadata.ParentDefinitionRevision);
        this.BindBoolean("@preview", taskGroupMetadata.Preview);
