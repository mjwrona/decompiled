...
// Type: Microsoft.TeamFoundation.WorkItemTracking.Server.DalGetWorkItemDetailsByReferencingUriElement
...
{
  internal class DalGetWorkItemDetailsByReferencingUriElement : DalGetWorkItemDetailsElementBase
  {
    public void JoinBatch(string uri, int artifactType, IVssIdentity user)
    {
      this.m_outputs = 1;
      this.m_index = this.SqlBatch.AddExpectedReturnedDataTables(this.m_outputs);
      this.SqlBatch.AppendSql("exec dbo.");
      this.SqlBatch.AppendSql("GetWorkItemDetailByReferenceUri");
