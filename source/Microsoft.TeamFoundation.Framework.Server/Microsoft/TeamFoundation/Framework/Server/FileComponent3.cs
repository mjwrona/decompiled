...
// Type: Microsoft.TeamFoundation.Framework.Server.FileComponent3
...
{
  internal class FileComponent3 : FileComponent2
  {
    public virtual QueryMD5ContextResult QueryMD5Context(OwnerId ownerId, long fileId)
    {
      this.PrepareStoredProcedure("prc_QueryMD5Context");
      this.BindInt("@fileId", (int) fileId);
      this.BindByte("@ownerId", (byte) ownerId);
      object md5Context = this.ExecuteScalar();
      return md5Context != null ? new QueryMD5ContextResult((byte[]) md5Context) : (QueryMD5ContextResult) null;
    }
