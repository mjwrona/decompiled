...
// Type: Microsoft.TeamFoundation.Framework.Server.DataTierInfoBinder2
...
{
  internal class DataTierInfoBinder2 : DataTierInfoBinder
  {
    protected SqlColumnBinder UserId = new SqlColumnBinder(nameof (UserId));
    protected SqlColumnBinder Password = new SqlColumnBinder("PasswordEncrypted");
    protected ISqlConnectionInfo m_connectionInfo;

    internal DataTierInfoBinder2(ISqlConnectionInfo connectionInfo) => this.m_connectionInfo = connectionInfo;
    ...
    protected override DataTierInfo Bind()
    {
      ...
      if (!SqlConnectionInfoFactory.TryCreate(connectionString, userName, bytes, out connectionInfo))
      {
        ...
        if (!string.IsNullOrEmpty(userName) && bytes != null && bytes.Length != 0)
          password = TeamFoundationSigningService.DecryptRaw(this.m_connectionInfo, frameworkSigningKey, bytes, SigningAlgorithm.SHA256).ToSecureString();
        connectionInfo = SqlConnectionInfoFactory.Create(connectionString, userName, password, bytes);
      }
      return new DataTierInfo(connectionInfo, state, tags, frameworkSigningKey);
    }
  }
}
