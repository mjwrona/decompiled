-- Parameter: @dbSessionName NVARCHAR(MAX) - Name of the session to start. Note this parameter is only used for SQL Azure
-- Parameter: @enforceReadScaleOut BIT - If 1 and stmt is used to start readScaleOutSession then the check if the query is running in readScaleOut is enforced.
SET XACT_ABORT ON
...
-- start the session if it is currently stopped
IF (@sessionStarted = 0 AND (@dbSessionName != @readScaleOutSessionName OR @enforceReadScaleOut != 1 OR @isRunningInReadScaleOut = 1))
BEGIN
    SET @statement = N'
            ALTER EVENT SESSION ' + QUOTENAME(@sessionName) + ' ON ' + @sessionType + '
            STATE=START'
