// Decompiled with JetBrains decompiler
// Type: Microsoft.Azure.Pipelines.Checks.WebApi.CheckDefinitionData
// Assembly: Microsoft.Azure.Pipelines.Checks.WebApi, Version=19.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a
// MVID: 381241F9-9196-42AF-BB4C-5187E3EFE32E
// Assembly location: C:\Program Files\Azure DevOps Server 2022\Application Tier\Web Services\bin\Microsoft.Azure.Pipelines.Checks.WebApi.dll

using Newtonsoft.Json.Linq;
using System;
using System.Runtime.Serialization;

namespace Microsoft.Azure.Pipelines.Checks.WebApi
{
  [DataContract]
  public class CheckDefinitionData
  {
    [DataMember]
    public Guid DefinitionRefId { get; set; }

    [DataMember]
    public string UIContributionType { get; set; }

    [DataMember]
    public string[] UIContributionDependencies { get; set; }

    [DataMember]
    public string Name { get; set; }

    [DataMember]
    public string Description { get; set; }

    [DataMember(EmitDefaultValue = false)]
    public CheckEvaluationOrder CheckEvaluationOrder { get; set; }

    [DataMember]
    public CheckIcon Icon { get; set; }

    [DataMember]
    public bool AllowMultipleConfigurations { get; set; }

    [DataMember]
    public JObject CheckDefinition { get; set; }
  }
}
