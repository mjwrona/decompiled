...
// Type: Microsoft.VisualStudio.Services.Gallery.Web.Models.IVSGallery
...
{
  public interface IVSGallery
  {
    Task<List<VSSearchResult>> GetFeaturedExtensions();

    Task<List<VSSearchResult>> GetMostPopularExtensions();

    Task<List<VSSearchResult>> GetTopRatedExtensions();

    Task<List<VSSearchResult>> GetRecentlyAddedExtensions();

    Task<VSCategory[]> GetCategories();
  }
}
