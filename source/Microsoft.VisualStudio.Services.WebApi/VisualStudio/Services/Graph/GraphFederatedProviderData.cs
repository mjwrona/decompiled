// Decompiled with JetBrains decompiler
// Type: Microsoft.VisualStudio.Services.Graph.GraphFederatedProviderData
// Assembly: Microsoft.VisualStudio.Services.WebApi, Version=19.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a
// MVID: 7B264323-C592-4F23-AB6B-55AEDC85864F
// Assembly location: C:\Program Files\Azure DevOps Server 2022\Application Tier\Web Services\bin\Plugins\Microsoft.VisualStudio.Services.WebApi.dll

using Microsoft.VisualStudio.Services.Common;
using Microsoft.VisualStudio.Services.WebApi;
using System.ComponentModel;
using System.Runtime.Serialization;

namespace Microsoft.VisualStudio.Services.Graph
{
  [ClientInternalUseOnly(true)]
  [EditorBrowsable(EditorBrowsableState.Never)]
  [DataContract]
  public class GraphFederatedProviderData
  {
    public SubjectDescriptor SubjectDescriptor { get; private set; }

    [DataMember(Name = "SubjectDescriptor")]
    private string SubjectDescriptorString
    {
      get => this.SubjectDescriptor.ToString();
      set => this.SubjectDescriptor = SubjectDescriptor.FromString(value);
    }

    [DataMember]
    public string ProviderName { get; private set; }

    [DataMember]
    public string AccessToken { get; private set; }

    [DataMember]
    public long Version { get; private set; }

    public GraphFederatedProviderData(
      SubjectDescriptor subjectDescriptor,
      string providerName,
      string accessToken,
      long version)
    {
      this.SubjectDescriptor = subjectDescriptor;
      this.ProviderName = providerName;
      this.AccessToken = accessToken;
      this.Version = version;
    }
  }
}
