...
// Type: Microsoft.TeamFoundation.Dashboards.Controllers.Dashboardv3Controller
...
{
  [VersionedApiControllerCustomName("Dashboard", "Dashboards", 3)]
  [ControllerApiVersion(6.0)]
  [ResolveTfsProjectAndTeamFilter(RequireExplicitTeam = true)]
  public class Dashboardv3Controller : DashboardApiv2ControllerBase
  {
    ...
    [ClientExample("GET_dashboards_by_project.json", null, null, null)]
    [TraceFilter(10017240, 10017249)]
    [HttpGet]
    [ClientResourceOperation(ClientResourceOperationName.List)]
    [PublicProjectRequestRestrictions]
    public List<Dashboard> GetDashboardsByProject()
    {
      List<Dashboard> dashboards = this.DashboardService.GetDashboards(this.TfsRequestContext, this.ProjectId);
      List<Dashboard> dashboardsByProject = dashboards != null ? dashboards : throw new DashboardsDoNotExistException();
      if (this.TeamId != Guid.Empty)
        dashboardsByProject = dashboards.Where<Dashboard>((Func<Dashboard, bool>) (d => d.GroupId == this.TeamId)).ToList<Dashboard>();
      dashboardsByProject.ForEach((Action<Dashboard>) (dashboard => dashboard.AddLinks(this.TfsRequestContext, this.Url, this.ProjectId, this.TeamId != Guid.Empty, dashboard.GroupId)));
      return dashboardsByProject;
    }
    ...
    {
      ArgumentUtility.CheckForNull<CopyDashboardOptions>(copyDashboardOptions, nameof (copyDashboardOptions), "Dashboards");
      DashboardExtension.ValidateCopyOptions(copyDashboardOptions);
      CopyDashboardResponse dashboardResponse = this.DashboardService.CopyDashboard(this.TfsRequestContext, dashboardId, DashboardConsumerFactory.CopyDashboardConsumer(this.TfsRequestContext, this.ProjectId, new Guid?(this.TeamId)), DashboardConsumerFactory.CopyDashboardConsumer(this.TfsRequestContext, copyDashboardOptions.ProjectId, copyDashboardOptions.TeamId), copyDashboardOptions);
      Dashboard copiedDashboard = dashboardResponse.CopiedDashboard;
      IVssRequestContext tfsRequestContext = this.TfsRequestContext;
      UrlHelper url = this.Url;
      Guid projectId = copyDashboardOptions.ProjectId;
      int num = copyDashboardOptions.CopyDashboardScope == DashboardScope.Project_Team ? 1 : 0;
      Guid teamId1;
      if (copyDashboardOptions.TeamId.HasValue)
      {
        Guid? teamId2 = copyDashboardOptions.TeamId;
        Guid empty = Guid.Empty;
        if ((teamId2.HasValue ? (teamId2.HasValue ? (teamId2.GetValueOrDefault() == empty ? 1 : 0) : 1) : 0) == 0)
        {
          teamId2 = copyDashboardOptions.TeamId;
          teamId1 = teamId2.Value;
          goto label_4;
        }
      }
      teamId1 = new Guid();
label_4:
      copiedDashboard.AddLinksForCopy(tfsRequestContext, url, projectId, num != 0, teamId1);
