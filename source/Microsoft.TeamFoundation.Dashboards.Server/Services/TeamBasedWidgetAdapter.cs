...
// Type: Microsoft.TeamFoundation.Dashboards.Services.TeamBasedWidgetAdapter
...
{
  internal class TeamBasedWidgetAdapter : WidgetAdapterBase
  {
      ...
      CopyDashboardParameterHandler copyDashboardParameterHandler = null)
    {
      ...
      if (targetDashboardConsumer.GetScope() == DashboardScope.Project_Team)
      {
        ...
        if (JsonUtilities.TryDeserialize<Dictionary<string, string>>(widget1.Settings, out dictionary, true))
        {
          dictionary["teamId"] = targetDashboardConsumer.GetGroupId().ToString();
          widget1.Settings = dictionary.Serialize<Dictionary<string, string>>();
        }
      }
