...
// Type: Microsoft.TeamFoundation.Dashboards.Services.SprintOverviewWidgetAdapter
...
{
  public class SprintOverviewWidgetAdapter : WidgetAdapterBase
  {
      ...
      CopyDashboardParameterHandler copyDashboardParameterHandler = null)
    {
      ...
      {
        Dictionary<string, string> dictionary2 = JsonUtilities.Deserialize<Dictionary<string, string>>(dictionary1["team"].ToString(), true);
        if (targetDashboardConsumer.GetScope() == DashboardScope.Project_Team && sourceDashboardConsumer.GetGroupId() != targetDashboardConsumer.GetGroupId())
        {
          dictionary2["id"] = targetDashboardConsumer.GetGroupId().ToString();
          dictionary2.Remove("name");
        }
        ...
        dictionary1["team"] = (object) dictionary2;
