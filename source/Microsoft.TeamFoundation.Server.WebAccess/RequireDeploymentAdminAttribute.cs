...
// Type: Microsoft.TeamFoundation.Server.WebAccess.RequireDeploymentAdminAttribute
...
namespace Microsoft.TeamFoundation.Server.WebAccess
{
  ...
  {
    private const string s_area = "WebAccessFilterAttribute";
    private const string s_layer = "RequireDeploymentAdminAttribute";

    public RequireDeploymentAdminAttribute()
      : this(false)
    {
    }

    public RequireDeploymentAdminAttribute(bool allowTroubleshoot) => this.AllowTroubleshoot = allowTroubleshoot;
    ...
    public override void OnActionExecuting(ActionExecutingContext filterContext)
    {
      ...
      {
        if (!controller.TfsRequestContext.IsDeploymentAdmin() && !this.AllowTroubleshootPermission(controller))
          throw new HttpException(404, PlatformResources.PageNotFound);
      }
      else
      {
        TeamFoundationTracingService.TraceRaw(520041, TraceLevel.Error, "WebAccessFilterAttribute", nameof (RequireDeploymentAdminAttribute), "No TfsController associated with filter context. Defaulting to 404 response.");
        throw new HttpException(404, PlatformResources.PageNotFound);
      }
    }
