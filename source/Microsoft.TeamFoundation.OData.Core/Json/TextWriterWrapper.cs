// Decompiled with JetBrains decompiler
// Type: Microsoft.OData.Json.TextWriterWrapper
// Assembly: Microsoft.TeamFoundation.OData.Core, Version=7.6.3.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a
// MVID: 6619C7F6-E44A-4143-AE77-6D570F968D9A
// Assembly location: C:\Program Files\Azure DevOps Server 2022\Application Tier\Web Services\bin\Microsoft.TeamFoundation.OData.Core.dll

using System;
using System.IO;
using System.Text;

namespace Microsoft.OData.Json
{
  internal abstract class TextWriterWrapper : TextWriter
  {
    protected TextWriter writer;

    protected TextWriterWrapper(IFormatProvider formatProvider)
      : base(formatProvider)
    {
    }

    public override Encoding Encoding => this.writer.Encoding;

    public override string NewLine => this.writer.NewLine;

    public virtual void IncreaseIndentation()
    {
    }

    public virtual void DecreaseIndentation()
    {
    }

    public override void Flush() => this.writer.Flush();

    protected static void InternalCloseOrDispose() => throw new NotImplementedException();
  }
}
