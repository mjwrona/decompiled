...
// Type: Microsoft.VisualStudio.Services.DevSecOps.Common.LcsDiff`1
      ...
      out bool quitEarly)
    {
      ...
      for (int numDifferences = 1; numDifferences <= num3 / 2 + 1; ++numDifferences)
      {
        ...
        for (int index = sourceIndex1; index <= num1; index += 2)
        {
          ...
          int num9 = num7;
          for (; num7 < originalEnd && num8 < modifiedEnd && this.ElementsAreEqual(num7 + 1, num8 + 1); ++num8)
            ++num7;
        ...
        for (int index = sourceIndex2; index <= num2; index += 2)
        {
          ...
          for (; originalIndex2 > originalStart && modifiedIndex > modifiedStart && this.ElementsAreEqual(originalIndex2, modifiedIndex); --modifiedIndex)
            --originalIndex2;
