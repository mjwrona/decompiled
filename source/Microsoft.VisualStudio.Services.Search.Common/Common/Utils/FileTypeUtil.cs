...
// Type: Microsoft.VisualStudio.Services.Search.Common.Utils.FileTypeUtil
...
{
  internal static class FileTypeUtil
  {
    private static readonly object s_lock = new object();
    private static Encoding s_utf8;
    private static Encoding s_utf8NoBom;
    private static Encoding s_utf16Little;
    private static Encoding s_utf16Big;
    private static Encoding s_utf32Little;
    private static Encoding s_utf32Big;
    private static readonly byte[] s_pdfMagic = new byte[5]
    {
      (byte) 37,
      (byte) 80,
      (byte) 68,
      ...
      out bool containsPreamble)
    {
      ...
      if (FileTypeUtil.s_utf8 == null)
      {
        lock (FileTypeUtil.s_lock)
        {
          if (FileTypeUtil.s_utf8 == null)
          {
            FileTypeUtil.s_utf8 = (Encoding) new UTF8Encoding(true);
            FileTypeUtil.s_utf8NoBom = (Encoding) new UTF8Encoding(false);
            FileTypeUtil.s_utf16Little = (Encoding) new UnicodeEncoding(false, true);
            FileTypeUtil.s_utf16Big = (Encoding) new UnicodeEncoding(true, true);
            FileTypeUtil.s_utf32Little = (Encoding) new UTF32Encoding(false, true);
            FileTypeUtil.s_utf32Big = (Encoding) new UTF32Encoding(true, true);
          }
        }
      }
      ...
      if (encoding == null)
      {
        ...
        if ((result & utF8Flags) == UTF8Flags.NONASCII)
          encoding = FileTypeUtil.s_utf8NoBom;
      }
    ...
    private static Encoding FindMatchingEncoding(byte[] buffer, int length, out bool isPdf)
    {
      ...
      if (length >= 2)
      {
        if (buffer[0] == (byte) 254 && buffer[1] == byte.MaxValue)
          return FileTypeUtil.s_utf16Big;
        if (buffer[0] == byte.MaxValue && buffer[1] == (byte) 254)
          return length >= 4 && buffer[2] == (byte) 0 && buffer[3] == (byte) 0 ? FileTypeUtil.s_utf32Little : FileTypeUtil.s_utf16Little;
        if (length >= 3 && buffer[0] == (byte) 239 && buffer[1] == (byte) 187 && buffer[2] == (byte) 191)
          return FileTypeUtil.s_utf8;
        if (length >= 4 && buffer[0] == (byte) 0 && buffer[1] == (byte) 0 && buffer[2] == (byte) 254 && buffer[3] == byte.MaxValue)
          return FileTypeUtil.s_utf32Big;
        if (length >= FileTypeUtil.s_pdfMagic.Length && ArrayUtil.Equals(FileTypeUtil.s_pdfMagic, buffer, FileTypeUtil.s_pdfMagic.Length))
        {
          isPdf = true;
          return (Encoding) null;
        }
