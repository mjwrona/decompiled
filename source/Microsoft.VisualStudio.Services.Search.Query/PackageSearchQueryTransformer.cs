...
// Type: Microsoft.VisualStudio.Services.Search.Query.PackageSearchQueryTransformer
...
{
  internal static class PackageSearchQueryTransformer
  {
    ...
    private static readonly CompositeTermCorrector s_compositeTermCorrector = new CompositeTermCorrector(new ICorrector[4]
    {
      ...
      (ICorrector) new PackageInlineFieldCorrector(),
    ...
    {
      IExpression expression = CorrectorTraverser.CorrectTerms(requestContext, queryParseTree, (ICorrector) PackageSearchQueryTransformer.s_compositeTermCorrector);
      return PackageSearchQueryTransformer.s_emptyExpressionCorrector.Correct(requestContext, expression);
    }

    public static IExpression Parse(string searchText) => searchText != null ? new QueryParser(PackageSearchQueryTransformer.s_supportedOperators).Parse(searchText) : throw new ArgumentNullException(nameof (searchText));
      ...
      IEnumerable<RelevanceRule> rules)
    {
      ...
      {
        foreach (IRelevanceRuleCorrector relevanceRuleCorrector in PackageSearchQueryTransformer.s_relevanceRulesCorrector)
