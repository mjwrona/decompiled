...
// Type: Microsoft.VisualStudio.Services.Search.Query.Correctors.DoubleQuotesCorrector
...
{
  internal class DoubleQuotesCorrector : TermCorrector
  {
      ...
      TermExpression termExpression)
    {
      ...
      if (termExpression1.IsOfType("*") || termExpression1.IsOfType(CodeFileContract.CodeContractQueryableElement.FileName))
      {
      ...
      {
        if (termExpression1.Type.StartsWith("\"", StringComparison.Ordinal) && termExpression1.Type.EndsWith("\"", StringComparison.Ordinal))
          termExpression1.Type = this.TrimWrappingDoubleQuotesForNonDefaultExpression(termExpression1.Type.CorrectEscapedDoubleQuotes());
        if (termExpression1.Value.StartsWith("\"", StringComparison.Ordinal) && termExpression1.Value.EndsWith("\"", StringComparison.Ordinal))
