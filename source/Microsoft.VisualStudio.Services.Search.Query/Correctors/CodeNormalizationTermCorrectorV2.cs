...
// Type: Microsoft.VisualStudio.Services.Search.Query.Correctors.CodeNormalizationTermCorrectorV2
...
{
  internal class CodeNormalizationTermCorrectorV2 : TermCorrector
  {
      ...
      TermExpression termExpression)
    {
      ...
      termExpression.Value = termExpression.IsOfType(CodeFileContract.CodeContractQueryableElement.FilePath) || termExpression.IsOfType(CodeFileContract.CodeContractQueryableElement.BranchName) ? termExpression.Value.NormalizePath() : (CodeSearchFilters.CEFilterNameToIdMap.ContainsKey(termExpression.Type) || termExpression.IsOfType(CodeFileContract.CodeContractQueryableElement.Default) && !termExpression.Value.ContainsWhitespaceOrSpecialCharacters() && !termExpression.Value.Contains("?") && !termExpression.Value.Contains("*") ? termExpression.Value.NormalizeStringAndReplaceTurkishDottedI() : termExpression.Value.NormalizeString());
