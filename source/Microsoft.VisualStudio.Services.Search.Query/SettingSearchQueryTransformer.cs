...
// Type: Microsoft.VisualStudio.Services.Search.Query.SettingSearchQueryTransformer
...
{
  internal static class SettingSearchQueryTransformer
  {
    ...
    {
      IExpression expression = CorrectorTraverser.CorrectTerms(requestContext, queryParseTree, (ICorrector) SettingSearchQueryTransformer.s_compositeTermCorrector);
      return SettingSearchQueryTransformer.s_emptyExpressionCorrector.Correct(requestContext, expression);
    }

    public static IExpression Parse(string searchText) => searchText != null ? new QueryParser(SettingSearchQueryTransformer.s_supportedOperators).Parse(searchText) : throw new ArgumentNullException(nameof (searchText));
      ...
      IEnumerable<RelevanceRule> rules)
    {
      ...
      {
        foreach (IRelevanceRuleCorrector relevanceRuleCorrector in SettingSearchQueryTransformer.s_relevanceRulesCorrector)
