...
// Type: Microsoft.TeamFoundation.WorkItemTracking.Server.DataAccess.WorkItemTypeExtensionComponent17
...
{
  internal class WorkItemTypeExtensionComponent17 : WorkItemTypeExtensionComponent16
  {
      ...
      bool? isDisabled = null)
    {
      ...
      WorkItemTypeletRecord workItemTypelet = this.ReadExtensions((ObjectBinder<WorkItemTypeletRecord>) new WorkItemTypeletRecordBinder4()).FirstOrDefault<WorkItemTypeletRecord>();
    ...
    public override List<WorkItemTypeletRecord> GetWorkItemTypelets(Guid processId)
    {
      ...
      this.BindGuid("@processId", processId);
      return this.ReadTypelets((ObjectBinder<WorkItemTypeletRecord>) new WorkItemTypeletRecordBinder4(), (ObjectBinder<WorkItemTypeExtensionBehaviorRecord>) new WorkItemTypeExtensionBehaviorRecordBinder());
    }
    ...
    public override WorkItemTypeletRecord UpdateBehavior(
      Guid extensionId,
      Guid processId,
      string refName,
      string name,
      ...
      Guid changedBy)
    {
      this.PrepareStoredProcedure("prc_UpdateBehavior");
      this.BindGuid("@id", extensionId);
      this.BindGuid("@processId", processId);
      ...
      if (!string.IsNullOrWhiteSpace(color))
        this.BindString("@color", color, 10, BindStringBehavior.Unchanged, SqlDbType.NVarChar);
      this.BindBoolean("@overridden", overridden);
      return this.ReadExtensions((ObjectBinder<WorkItemTypeletRecord>) new WorkItemTypeletRecordBinder4()).FirstOrDefault<WorkItemTypeletRecord>();
    }

    public override void DeleteBehavior(
      Guid extensionId,
      Guid processId,
      string refName,
      Guid changedBy)
    {
      this.PrepareStoredProcedure("prc_DeleteBehavior");
      this.BindGuid("@processId", processId);
      this.BindGuid("@id", extensionId);
      this.BindString("@refName", refName, 386, BindStringBehavior.Unchanged, SqlDbType.NVarChar);
      this.BindBoolean("@deleteWitReference", true);
      this.BindGuid("@changedBy", changedBy);
      this.ExecuteNonQueryEx();
    }

    public override List<int> GetExtendedWorkItemIds(Guid typeletId, int markedField)
    {
      this.PrepareStoredProcedure("prc_GetExtendedWorkItemIds");
      this.BindNullableGuid("@typeletId", typeletId);
    ...
    internal override void SetReconciliationWatermarks(
      IEnumerable<Guid> extensionIds,
      Guid reconciliationWatermark)
    {
      this.PrepareStoredProcedure("prc_SetWorkItemTypeExtensionReconciliationWatermarks");
