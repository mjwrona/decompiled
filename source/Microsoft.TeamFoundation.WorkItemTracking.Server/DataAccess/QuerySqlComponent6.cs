...
// Type: Microsoft.TeamFoundation.WorkItemTracking.Server.DataAccess.QuerySqlComponent6
...
{
  internal class QuerySqlComponent6 : QuerySqlComponent5
  {
      ...
      bool getRowCount)
    {
      ...
      this.BindBoolean("@isQueryHistorySamplingEnabled", true);
      if (getRowCount)
      {
    ...
    public override void SaveQueryHistories(
      IEnumerable<QueryExecutionHistory> queryExecutionHistories)
    {
      this.PrepareStoredProcedure("prc_SaveQueryHistories");
      this.BindQueryExecutionHistoryTable("@executionHistories", queryExecutionHistories);
      ...
      DateTime cacheLoadCutOffTime)
    {
      ...
      this.BindBoolean("@isUseHistoryForOptimization", true);
      using (ResultCollection resultCollection = new ResultCollection(this.ExecuteReader(), this.ProcedureName, this.RequestContext))
      {
        resultCollection.AddBinder<QueryOptimizationInstance>((ObjectBinder<QueryOptimizationInstance>) new QuerySqlComponent.QueryOptimizationInstanceBinder());
        resultCollection.AddBinder<QueryExecutionHistory>((ObjectBinder<QueryExecutionHistory>) new QuerySqlComponent.QueryExecutionHistorysBinder());
