// Decompiled with JetBrains decompiler
// Type: Microsoft.TeamFoundation.WorkItemTracking.Server.DataAccess.WorkItemTrackingObjectBinder`1
// Assembly: Microsoft.TeamFoundation.WorkItemTracking.Server, Version=19.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a
// MVID: B0AE48DA-B6D2-466C-91D8-D0BF0F05DE87
// Assembly location: C:\Program Files\Azure DevOps Server 2022\Application Tier\Web Services\bin\Plugins\Microsoft.TeamFoundation.WorkItemTracking.Server.dll

using Microsoft.TeamFoundation.Framework.Server;
using System.Collections.Generic;
using System.Data;

namespace Microsoft.TeamFoundation.WorkItemTracking.Server.DataAccess
{
  public abstract class WorkItemTrackingObjectBinder<T> : ObjectBinder<T>
  {
    protected override sealed T Bind() => this.Bind(this.Reader);

    public abstract T Bind(IDataReader reader);

    protected IDataReader Reader => this.BaseReader;

    public IEnumerable<T> BindAll(IDataReader reader)
    {
      while (reader.Read())
        yield return this.Bind(reader);
    }
  }
}
