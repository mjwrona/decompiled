...
// Type: Microsoft.TeamFoundation.WorkItemTracking.Server.QueryItems.QueryItemComponent11
...
{
  internal class QueryItemComponent11 : QueryItemComponent10
  {
    protected override WorkItemTrackingObjectBinder<QueryItemEntry> GetDrilldownQueryItemEntryBinder() => (WorkItemTrackingObjectBinder<QueryItemEntry>) new QueryItemComponent.DrilldownQueryDataBinder6((QueryItemComponent) this);

    public override void BindIncludeExecutionInfo(bool includeExecutionInfo) => this.BindBoolean("@includeExecutionInfo", includeExecutionInfo);
      ...
      Guid? filterUnderProjectId = null)
    {
      ...
      this.BindIncludeExecutionInfo(includeExecutionInfo);
      try
      {
    ...
    public override IList<QueryItemEntry> SearchQueries(
      Guid projectId,
      Guid teamFoundationId,
      int maxResultCount,
      bool includeWiql,
      ...
      bool includeExecutionInfo = false)
    {
      this.PrepareStoredProcedure("prc_SearchQueryItems");
      this.BindProjectId(projectId);
      this.BindGuid("@teamFoundationId", teamFoundationId);
      this.BindBoolean("@includeWiql", includeWiql);
      this.BindString("@searchText", searchText, (int) byte.MaxValue, false, SqlDbType.NVarChar);
      this.BindIncludeDeleted(includeDeleted);
      this.BindMaxResultCount(maxResultCount);
      this.BindIncludeExecutionInfo(includeExecutionInfo);
      try
      {
        return (IList<QueryItemEntry>) this.ExecuteUnknown<IEnumerable<QueryItemEntry>>((System.Func<IDataReader, IEnumerable<QueryItemEntry>>) (reader => this.GetDrilldownQueryItemEntryBinder().BindAll(reader))).ToList<QueryItemEntry>();
      }
      catch (SqlException ex)
      {
        this.MapException(ex);
    ...
    public override void UpdateQueryHash(IList<QueryHash> queryHashInfo)
    {
      this.PrepareStoredProcedure("prc_PopulateQueryItemsHash");
      new QueryItemComponent.QueryHashTable("@queryHashInfo", (IEnumerable<QueryHash>) queryHashInfo).BindTable(this);
      this.ExecuteNonQuery();
    }

    public override IList<QueryItemEntry> FetchTopQueryItemsWithoutExecutionInfo(int top = 1000)
    {
      this.PrepareStoredProcedure("prc_FetchTopQueryItemsWithoutExecutionInfo");
