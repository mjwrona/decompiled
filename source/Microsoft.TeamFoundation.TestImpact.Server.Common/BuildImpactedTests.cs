...
// Type: Microsoft.TeamFoundation.TestImpact.Server.Common.BuildImpactedTests
...
namespace Microsoft.TeamFoundation.TestImpact.Server.Common
{
  ...
  [CLSCompliant(false)]
  public class BuildImpactedTests
  {
    private List<CodeChange> _changes;
    ...
    [XmlArray]
    [XmlArrayItem(typeof (CodeChange))]
    [ClientProperty(ClientVisibility.Internal, ClientVisibility.Internal, PropertyName = "CodeChangesInternal", UseClientDefinedProperty = true)]
    public List<CodeChange> CodeChanges
    {
      get
      {
        if (this._changes == null)
          this._changes = new List<CodeChange>();
