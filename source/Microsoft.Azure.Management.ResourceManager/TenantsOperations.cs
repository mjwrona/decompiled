...
// Type: Microsoft.Azure.Management.ResourceManager.TenantsOperations
...
{
  internal class TenantsOperations : IServiceOperations<SubscriptionClient>, ITenantsOperations
  {
    internal TenantsOperations(SubscriptionClient client) => this.Client = client != null ? client : throw new ArgumentNullException(nameof (client));

    public SubscriptionClient Client { get; private set; }

    public async Task<AzureOperationResponse<IPage<TenantIdDescription>>> ListWithHttpMessagesAsync(
      ...
      CancellationToken cancellationToken = default (CancellationToken))
    {
      ...
      string _url = new Uri(new Uri(_baseUrl + (_baseUrl.EndsWith("/") ? "" : "/")), "tenants").ToString();
      ...
      AzureOperationResponse<IPage<TenantIdDescription>> _result = new AzureOperationResponse<IPage<TenantIdDescription>>();
      ...
      if (_statusCode == HttpStatusCode.OK)
      {
        ...
        try
        {
          _result.Body = (IPage<TenantIdDescription>) SafeJsonConvert.DeserializeObject<Page1<TenantIdDescription>>(_responseContent, this.Client.DeserializationSettings);
        }
    ...
    public async Task<AzureOperationResponse<IPage<TenantIdDescription>>> ListNextWithHttpMessagesAsync(
      string nextPageLink,
      Dictionary<string, List<string>> customHeaders = null,
      CancellationToken cancellationToken = default (CancellationToken))
    {
