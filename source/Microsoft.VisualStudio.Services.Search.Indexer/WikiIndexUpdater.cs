...
// Type: Microsoft.VisualStudio.Services.Search.Indexer.WikiIndexUpdater
...
{
  internal sealed class WikiIndexUpdater : AbstractIndexUpdater
  {
    public WikiIndexUpdater(
    ...
    public override IndexOperationsResponse DeleteDocumentsByQuery(
      ...
      bool leniant = false)
    {
      return this.SearchIndex.BulkDeleteByQuery<WikiContract>((ExecutionContext) this.IndexingExecutionContext, new BulkDeleteByQueryRequest<WikiContract>(query, this.DocumentContractType)
      {
        Lenient = leniant
      }, forceComplete);
    }
