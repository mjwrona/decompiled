...
// Type: Microsoft.VisualStudio.Services.Search.Indexer.Routing.RoutingProviders.WikiEntityRoutingProvider
...
{
  internal class WikiEntityRoutingProvider : IRoutingDataProvider
  {
    ...
    [StaticSafe]
    public static IEntityType EntityType => (IEntityType) WikiEntityType.GetInstance();

    internal WikiEntityRoutingProvider(string routing)
      : this(new CreateSearchIndexHelper(), EntityProvisionerFactory.GetIndexProvisioner(WikiEntityRoutingProvider.EntityType), routing)
    {
    }

    internal WikiEntityRoutingProvider(
      ...
      List<IndexingUnitWithSize> indexingUnitsWithSizeEstimates)
    {
      ...
      long maxDocsPerSharedIndex = indexingExecutionContext.GetConfigValue<long>("/Service/ALMSearch/Settings/MaxWikiDocsPerSharedIndexShard") * (long) indexingExecutionContext.RequestContext.GetConfigValue<int>("/Service/ALMSearch/Settings/WikiSharedIndexPrimaries", TeamFoundationHostType.Deployment, 1);
      ...
      int shardId)
    {
      ...
      foreach (IndexingUnitWithSize indexingUnitWithSize in indexingUnitsWithSize)
      {
        ...
        WikiEntityType instance = WikiEntityType.GetInstance();
