...
namespace Microsoft.Azure.Documents.Collections
{
  internal sealed class DictionaryNameValueCollection : INameValueCollection, IEnumerable
  {
      ...
      : this(c.Count)
    {
      ...
      {
        string[] values = c.GetValues(str1);
        if (values != null)
        {
    ...
    public void Add(string key, string value)
    {
      if (key == null)
      {
        DictionaryNameValueCollection.CompositeValue nullValue;
        if ((nullValue = this.nullValue) == null)
    ...
    public void Add(INameValueCollection c)
    {
      ...
      if (c is DictionaryNameValueCollection nameValueCollection)
      {
        ...
        DictionaryNameValueCollection.CompositeValue nullValue;
        if ((nullValue = nameValueCollection.nullValue) == null)
          return;
        DictionaryNameValueCollection.CompositeValue compositeValue;
        if ((compositeValue = this.nullValue) == null)
          compositeValue = this.nullValue = new DictionaryNameValueCollection.CompositeValue();
        compositeValue.Add(nullValue);
      }
      else
      {
