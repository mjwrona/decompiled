...
namespace Microsoft.Azure.Documents
{
  ...
  internal class DocumentClientException : Exception
  {
      ...
      : base(DocumentClientException.MessageWithActivityId(errorResource.Message, responseHeaders))
    {
      ...
      this.responseHeaders = (INameValueCollection) new StoreResponseNameValueCollection();
      ...
      if (this.responseHeaders.Get("x-ms-activity-id") == null)
        this.responseHeaders.Set("x-ms-activity-id", System.Diagnostics.Trace.CorrelationManager.ActivityId.ToString());
      this.LSN = -1L;
      ...
      objArray[0] = (object) statusCode1.GetValueOrDefault();
      ...
      : base(DocumentClientException.MessageWithActivityId(message, responseHeaders), innerException)
    {
      ...
      statusCode1 = this.StatusCode;
      objArray[0] = (object) statusCode1.GetValueOrDefault();
      objArray[1] = (object) message;
      objArray[2] = innerException != null ? (object) innerException.ToString() : (object) "null";
      objArray[3] = (object) DocumentClientException.SerializeHTTPResponseHeaders(responseHeaders);
      DefaultTrace.TraceError("DocumentClientException with status code {0}, message: {1}, inner exception: {2}, and response headers: {3}", objArray);
    }
    ...
    {
      this.StatusCode = statusCode;
      if (responseHeaders is StoreResponseNameValueCollection nameValueCollection)
      {
        this.responseHeaders = nameValueCollection.Clone();
      }
      else
      {
        this.responseHeaders = (INameValueCollection) new StoreResponseNameValueCollection();
      ...
      : base(info, context)
    {
      ...
      if (statusCode1.GetValueOrDefault() == httpStatusCode & statusCode1.HasValue)
        return;
      DefaultTrace.TraceError("DocumentClientException with status code {0}, and serialization info: {1}", (object) this.StatusCode.GetValueOrDefault(), (object) info.ToString());
    }
    ...
    internal INameValueCollection Headers => this.responseHeaders;
    ...
    internal string RawErrorMessage => this.rawErrorMessage ?? base.Message;
    ...
    private static string MessageWithActivityId(string message, string activityIdFromHeaders = null)
    {
      ...
      else
      {
        if (!(System.Diagnostics.Trace.CorrelationManager.ActivityId != Guid.Empty))
          return message;
        str = System.Diagnostics.Trace.CorrelationManager.ActivityId.ToString();
      }
    ...
    {
      SubStatusCodes forGoneRetryPolicy = SubStatusCodes.Unknown;
      if (exception is DocumentClientException documentClientException)
      {
        switch (documentClientException)
        {
          case PartitionIsMigratingException _:
            forGoneRetryPolicy = SubStatusCodes.Server_CompletingPartitionMigrationExceededRetryLimit;
            break;
          case InvalidPartitionException _:
            forGoneRetryPolicy = SubStatusCodes.Server_NameCacheIsStaleExceededRetryLimit;
            break;
          case PartitionKeyRangeIsSplittingException _:
            forGoneRetryPolicy = SubStatusCodes.Server_CompletingSplitExceededRetryLimit;
            break;
          case PartitionKeyRangeGoneException _:
            forGoneRetryPolicy = SubStatusCodes.Server_PartitionKeyRangeGoneExceededRetryLimit;
            break;
          default:
            forGoneRetryPolicy = documentClientException.GetSubStatus();
            break;
        }
      }
      return forGoneRetryPolicy;
    }
