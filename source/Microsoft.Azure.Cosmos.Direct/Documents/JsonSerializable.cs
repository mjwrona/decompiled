...
namespace Microsoft.Azure.Documents
{
  internal abstract class JsonSerializable
  {
      ...
      SerializationFormattingPolicy formattingPolicy = SerializationFormattingPolicy.None)
    {
      ...
      this.OnSave();
      if (typeof (Document).IsAssignableFrom(this.GetType()) && !this.GetType().Equals(typeof (Document)) || typeof (Attachment).IsAssignableFrom(this.GetType()) && !this.GetType().Equals(typeof (Attachment)))
    ...
    internal TEnum? GetEnumValue<TEnum>(string propertyName) where TEnum : struct
    {
      if (!typeof (TEnum).IsEnum())
        throw new ArgumentException(string.Format("{0} is not an Enum.", (object) typeof (TEnum)));
      string str = this.GetValue<string>(propertyName);
      if (string.IsNullOrWhiteSpace(str))
        return new TEnum?();
      TEnum result;
      return Enum.TryParse<TEnum>(str, true, out result) && Enum.IsDefined(typeof (TEnum), (object) result) ? new TEnum?(result) : throw new BadRequestException("Could not parse [" + str + "] as a valid enum value for property [" + propertyName + "].");
    }
    ...
    internal Collection<T> GetValueCollection<T>(string propertyName)
    {
      if (this.propertyBag != null)
      {
        JToken jtoken1 = this.propertyBag[propertyName];
        if (jtoken1 != null)
        {
          Collection<JToken> collection = jtoken1.ToObject<Collection<JToken>>();
          Collection<T> valueCollection = new Collection<T>();
          foreach (JToken jtoken2 in collection)
          {
            if (jtoken2 != null)
            {
              if (typeof (T).IsEnum() && jtoken2.Type == JTokenType.String)
                valueCollection.Add(jtoken2.ToObject<T>(JsonSerializer.CreateDefault()));
              else
                valueCollection.Add(this.SerializerSettings == null ? jtoken2.ToObject<T>() : jtoken2.ToObject<T>(JsonSerializer.Create(this.SerializerSettings)));
            }
            else
    ...
    internal void SetValueCollection<T>(string propertyName, Collection<T> value)
    {
      ...
      if (value != null)
      {
        Collection<JToken> o1 = new Collection<JToken>();
        foreach (T o2 in value)
        {
          if ((object) o2 != null)
            o1.Add(JToken.FromObject((object) o2));
          else
            o1.Add((JToken) null);
        }
        this.propertyBag[propertyName] = JToken.FromObject((object) o1);
      }
      else
    ...
    internal TSerializable GetObject<TSerializable>(string propertyName, bool returnEmptyObject = false) where TSerializable : JsonSerializable, new()
    {
      if (this.propertyBag != null)
      {
        ...
        if (o != null && (returnEmptyObject || o.HasValues))
        {
      ...
      where TSerializable : JsonSerializable
    {
      if (this.propertyBag != null)
      {
        ...
        if (propertyBag != null && (returnEmptyObject || propertyBag.HasValues))
    ...
    internal Dictionary<string, TSerializable> GetObjectDictionaryWithNullableValues<TSerializable>(
      string propertyName)
      where TSerializable : JsonSerializable, new()
    {
      if (this.propertyBag != null)
      {
        ...
        {
          Dictionary<string, JObject> dictionary = jtoken.ToObject<Dictionary<string, JObject>>();
          Dictionary<string, TSerializable> withNullableValues = new Dictionary<string, TSerializable>();
          foreach (KeyValuePair<string, JObject> keyValuePair in dictionary)
          {
            TSerializable serializable;
            if (keyValuePair.Value == null)
            {
              serializable = default (TSerializable);
            }
            else
            {
              serializable = new TSerializable();
              serializable.propertyBag = keyValuePair.Value;
            }
            withNullableValues.Add(keyValuePair.Key, serializable);
          }
          return withNullableValues;
        }
      }
      return (Dictionary<string, TSerializable>) null;
    }
    ...
    internal void SetObjectDictionaryWithNullableValues<TSerializable>(
      string propertyName,
      Dictionary<string, TSerializable> value)
      where TSerializable : JsonSerializable, new()
    {
      ...
      foreach (KeyValuePair<string, TSerializable> keyValuePair in value)
      {
        if ((object) keyValuePair.Value != null)
        {
          ...
          o.Add(keyValuePair.Key, (JObject) null);
      }
      this.propertyBag[propertyName] = JToken.FromObject((object) o);
    }
