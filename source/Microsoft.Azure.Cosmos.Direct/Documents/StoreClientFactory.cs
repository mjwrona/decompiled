...
namespace Microsoft.Azure.Documents
{
  internal sealed class StoreClientFactory : IStoreClientFactory, IDisposable
  {
    ...
    public StoreClientFactory(
      ...
      TimeSpan localRegionOpenTimeout = default (TimeSpan),
      bool enableChannelMultiplexing = false,
      int rntbdMaxConcurrentOpeningConnectionCount = 65535,
    ...
    internal StoreClientFactory Clone() => new StoreClientFactory(this.protocol, this.retryWithConfiguration, this.transportClient, this.fallbackClient, this.connectionStateListener);
      ...
      bool detectClientConnectivityIssues = false)
    {
      ...
      return useFallbackClient && this.fallbackClient != null ? new StoreClient(addressResolver, sessionContainer, serviceConfigurationReader, authorizationTokenProvider, this.protocol, this.fallbackClient, enableRequestDiagnostics, enableReadRequestsFallback, useMultipleWriteLocations, detectClientConnectivityIssues, this.disableRetryWithRetryPolicy, this.retryWithConfiguration) : new StoreClient(addressResolver, sessionContainer, serviceConfigurationReader, authorizationTokenProvider, this.protocol, this.transportClient, enableRequestDiagnostics, enableReadRequestsFallback, useMultipleWriteLocations, detectClientConnectivityIssues, this.disableRetryWithRetryPolicy, this.retryWithConfiguration);
    }
    ...
    private static void ValidateRntbdMaxConcurrentOpeningConnectionCount(
      ref int rntbdMaxConcurrentOpeningConnectionCount)
    {
      ...
      if (!string.IsNullOrEmpty(environmentVariable))
      {
        ...
        if (int.TryParse(environmentVariable, out result))
          num = result > 0 ? result : throw new ArgumentException("RntbdMaxConcurrentOpeningConnectionUpperLimitConfig should be larger than 0");
      }
      if (rntbdMaxConcurrentOpeningConnectionCount <= num)
        return;
      DefaultTrace.TraceWarning("The value of {0} is too large. Received {1}. Adjusting to {2}", (object) nameof (rntbdMaxConcurrentOpeningConnectionCount), (object) rntbdMaxConcurrentOpeningConnectionCount, (object) num);
