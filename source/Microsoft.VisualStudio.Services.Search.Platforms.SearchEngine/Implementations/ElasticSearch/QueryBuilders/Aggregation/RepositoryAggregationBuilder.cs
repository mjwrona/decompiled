...
// Type: Microsoft.VisualStudio.Services.Search.Platforms.SearchEngine.Implementations.ElasticSearch.QueryBuilders.Aggregation.RepositoryAggregationBuilder
...
{
  internal class RepositoryAggregationBuilder : IAggregationBuilder
  {
    ...
    public RepositoryAggregationBuilder(
      ...
      where T : class
    {
      if (this.m_enableAggregations)
      {
        IEnumerable<string> strings = (IEnumerable<string>) new string[3]
        {
        ...
        foreach (string str in strings)
        {
          ...
          else if (Microsoft.VisualStudio.Services.Search.WebApi.Legacy.Contracts.WorkItem.Constants.FilterCategories.Repositories.Equals(filterCategory, StringComparison.Ordinal))
          {
            name = "filtered_repository_aggs";
            aggregationIdConstant = "repository_aggs";
          ...
          {
            Dictionary<string, IEnumerable<string>> allButCurrentFilter = this.m_searchFilters.Where<KeyValuePair<string, IEnumerable<string>>>((Func<KeyValuePair<string, IEnumerable<string>>, bool>) (kvp => !kvp.Key.Equals(filterCategory, StringComparison.OrdinalIgnoreCase))).ToDictionary<KeyValuePair<string, IEnumerable<string>>, string, IEnumerable<string>>((Func<KeyValuePair<string, IEnumerable<string>>, string>) (kvp => kvp.Key), (Func<KeyValuePair<string, IEnumerable<string>>, IEnumerable<string>>) (kvp => kvp.Value));
            if (allButCurrentFilter.Count > 0)
              aggDescriptor.Filter(name, (Func<FilterAggregationDescriptor<T>, IFilterAggregation>) (f => (IFilterAggregation) f.Filter((Func<QueryContainerDescriptor<T>, QueryContainer>) (fd => new RepositoryFilterBuilder((IDictionary<string, IEnumerable<string>>) allButCurrentFilter).Filters<T>(fd))).Aggregations(new Func<AggregationContainerDescriptor<T>, IAggregationContainer>(TermsAggContainerDescriptor))));
            else
