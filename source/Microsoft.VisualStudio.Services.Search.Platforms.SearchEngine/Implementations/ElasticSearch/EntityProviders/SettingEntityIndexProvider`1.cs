...
// Type: Microsoft.VisualStudio.Services.Search.Platforms.SearchEngine.Implementations.ElasticSearch.EntityProviders.SettingEntityIndexProvider`1
...
{
  internal class SettingEntityIndexProvider<T> : EntityIndexProvider<T> where T : class
  {
      ...
      out Func<IVssRequestContext, SortDescriptor<T>> sort)
    {
      ...
      filter = (Func<QueryContainerDescriptor<T>, QueryContainer>) null;
    ...
    {
      return !isOperationSuccessful ? (SearchHit) new SettingSearchHit() : (SearchHit) new SettingSearchHit((object) hit.Source as SettingContract);
    }
      ...
      IDictionary<string, IEnumerable<Microsoft.VisualStudio.Services.Search.Shared.WebApi.Legacy.Filter>> searchFacets)
    {
      ...
      return (EntitySearchPlatformResponse) new SettingSearchPlatformResponse(totalMatches, (IList<SearchHit>) results, num != 0, facets);
    }
      ...
      ISearchResponse<T> elasticSearchResponse)
    {
      ...
      List<SearchHit> results = new List<SearchHit>((IEnumerable<SearchHit>) new SettingSearchHit[hits.Count]);
      ...
      ref bool isOperationSuccessful)
    {
      ...
      if (sources is SettingContract settingContract && !string.IsNullOrWhiteSpace(settingContract.CollectionName))
        searchHitSources.Add("collectionName", (IEnumerable<string>) new List<string>()
        {
          settingContract.CollectionName
        });
