...
// Type: Microsoft.VisualStudio.Services.Search.Platforms.SearchEngine.ElasticSearch.QueryBuilders.Expression.RepositoryTermExpressionQueryBuilderBase
...
{
  internal abstract class RepositoryTermExpressionQueryBuilderBase : 
    ProjectRepoExpressionQueryBuilderBase
  {
    public string Build(IVssRequestContext requestcontext, IExpression expression)
    {
      ...
      switch (termExpression.Operator)
      {
        ...
        default:
          throw new NotSupportedException(FormattableString.Invariant(FormattableStringFactory.Create("Operator [{0}] is not supported in ProjectRepo search.", (object) termExpression.Operator)));
      }
    }

    protected override IList<string> GetPlatformFieldNamesToQuery(IVssRequestContext requestContext) => throw new NotImplementedException("Implement in the child class");
