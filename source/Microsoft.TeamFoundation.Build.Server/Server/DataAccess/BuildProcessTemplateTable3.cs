...
// Type: Microsoft.TeamFoundation.Build.Server.DataAccess.BuildProcessTemplateTable3
...
{
  [Obsolete]
  internal sealed class BuildProcessTemplateTable3 : 
    TeamFoundationTableValueParameter<ProcessTemplate>
  {
    private bool m_isAdd;
    private static SqlMetaData[] s_metadata = new SqlMetaData[8]
    {
    ...
    public BuildProcessTemplateTable3(IEnumerable<ProcessTemplate> rows, bool isAdd)
      : base(rows, "typ_BuildProcessTemplateTableV3", BuildProcessTemplateTable3.s_metadata)
    {
      this.m_isAdd = isAdd;
    }
    ...
    public override void SetRecord(ProcessTemplate row, SqlDataRecord record)
    {
      ...
      if (!string.IsNullOrEmpty(row.Version))
        record.SetString(7, row.Version);
      else
        record.SetDBNull(7);
    }
  }
