...
// Type: Microsoft.TeamFoundation.Build.Server.DataAccess.BuildDefinitionWorkspaceTable3
...
{
  [Obsolete]
  internal sealed class BuildDefinitionWorkspaceTable3 : 
    TeamFoundationTableValueParameter<WorkspaceMapping>
  {
    ...
    public BuildDefinitionWorkspaceTable3(
      IEnumerable<WorkspaceMapping> rows,
      BuildSqlResourceComponent component)
      : base(rows, "typ_BuildDefinitionWorkspaceTableV3", BuildDefinitionWorkspaceTable3.s_metadata)
    {
    ...
    public override void SetRecord(WorkspaceMapping row, SqlDataRecord record)
    {
      ...
      record.SetInt32(1, this.m_component.GetDataspaceId(row.ProjectId));
      record.SetString(2, this.m_component.VersionControlPathToDataspaceDBPath(row.ServerItem));
      if (!string.IsNullOrEmpty(row.LocalItem))
        record.SetString(3, DBHelper.LocalPathToDBPath(row.LocalItem));
      else
        record.SetDBNull(3);
      record.SetByte(4, (byte) row.MappingType);
      record.SetInt32(5, row.Depth);
    }
  }
}
