...
// Type: Microsoft.TeamFoundation.Build.Server.DataAccess.BuildDeploymentComponent4
...
{
  internal class BuildDeploymentComponent4 : BuildDeploymentComponent3
  {
    public BuildDeploymentComponent4()
    {
      ...
      string environmentName)
    {
      ...
      resultCollection.AddBinder<BuildDeployment>((ObjectBinder<BuildDeployment>) new BuildDeploymentBinder3((BuildSqlResourceComponent) this));
    ...
    internal override bool IsADeployment(BuildDetail build)
    {
      ...
      this.BindInt("@dataspaceId", this.GetDataspaceId(build.Definition.TeamProject.Id));
      ...
      BuildDeploymentSpec spec)
    {
      ...
      if (!string.IsNullOrWhiteSpace(spec.TeamProject))
        this.BindInt("@dataspaceId", this.GetDataspaceId(this.GetTeamProjectFromGuidOrName(requestContext, spec.TeamProject).Id));
      ...
      IList<string> deploymentUris)
    {
      ...
      this.BindUrisToOrderedStringTable("@deploymentUriTable", (IEnumerable<string>) deploymentUris);
      ResultCollection resultCollection = new ResultCollection((IDataReader) this.ExecuteReader(), this.ProcedureName, this.RequestContext);
      resultCollection.AddBinder<BuildDeployment>((ObjectBinder<BuildDeployment>) new BuildDeploymentBinder3((BuildSqlResourceComponent) this));
      resultCollection.AddBinder<Tuple<string, Guid>>((ObjectBinder<Tuple<string, Guid>>) new BuildDeploymentRequestForBinder2());
      resultCollection.AddBinder<Tuple<string, ChangesetDisplayInformation>>((ObjectBinder<Tuple<string, ChangesetDisplayInformation>>) new BuildDeploymentChangesetBinder2());
