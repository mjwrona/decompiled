...
// Type: HdrHistogram.IntConcurrentHistogram
...
namespace HdrHistogram
{
  internal class IntConcurrentHistogram : HistogramBase
  {
    ...
    {
      IntConcurrentHistogram concurrentHistogram = new IntConcurrentHistogram(this.InstanceId, this.LowestTrackableValue, this.HighestTrackableValue, this.NumberOfSignificantValueDigits);
    ...
    protected override long GetCountAtIndex(int index)
    {
      try
      {
        ...
        return (long) this._counts[index];
      }
      finally
      {
    ...
    protected override void AddToCountAtIndex(int index, long addend)
    {
      ...
      try
      {
        this._counts.AddAndGet(index, (int) addend);
