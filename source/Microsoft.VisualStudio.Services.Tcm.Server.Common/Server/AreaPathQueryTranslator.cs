...
// Type: Microsoft.TeamFoundation.TestManagement.Server.AreaPathQueryTranslator
...
{
  internal class AreaPathQueryTranslator : ProjectQueryTranslator
  {
    private ResultsStoreQuery m_query;

    public AreaPathQueryTranslator(
      TestManagementRequestContext context,
      ResultsStoreQuery query,
      List<Tuple<Type, string, string>> tables = null)
      : base(context, query, (Func<string, string, string>) null, tables)
    {
      this.m_query = query;
    ...
    {
      this.m_fieldMapping.Add("AreaPath", "AreaPath");
      this.m_fieldMapping.Add("TestCaseArea", "TestCaseArea");
    }

    protected override void TranslateValue(NodeCondition cond)
    {
      base.TranslateValue(cond);
      if (!string.Equals(cond.Left.Value, "AreaPath", StringComparison.OrdinalIgnoreCase) && !string.Equals(cond.Left.Value, "TestCaseArea", StringComparison.OrdinalIgnoreCase))
        return;
