...
// Type: Microsoft.TeamFoundation.TestManagement.Server.ResultGroupsByRelease5Controller
...
namespace Microsoft.TeamFoundation.TestManagement.Server
{
  ...
  [VersionedApiControllerCustomName(Area = "Test", ResourceName = "ResultGroupsByRelease", ResourceVersion = 2)]
  public class ResultGroupsByRelease5Controller : TestManagementController
  {
      ...
      string continuationToken = null)
    {
      ...
      {
        pagedList = this.ResultGroupsHelper.GetTestResultsGroupsByRelease(this.ProjectInfo, releaseId, releaseEnvId, publishContext, fields, continuationToken);
      }
      else
      {
        List<string> groupByFields = ParsingHelper.ParseCommaSeparatedString(fields);
        pagedList = TestManagementController.InvokeAction<IPagedList<FieldDetailsForTestResults>>((Func<IPagedList<FieldDetailsForTestResults>>) (() => this.TestResultsHttpClient.GetResultGroupsByReleaseWithContinuationTokenAsync(this.ProjectInfo.Id, releaseId, publishContext, new int?(releaseEnvId), (IEnumerable<string>) groupByFields, continuationToken)?.Result));
