...
// Type: Microsoft.TeamFoundation.TestManagement.Server.Charting.TestRunCapabilityProvider
...
{
  public class TestRunCapabilityProvider : IDataServiceCapabilityProvider, IDataServicesService
  {
    public IVssRequestContext RequestContext { get; set; }

    public string GetScopeName() => FeatureProviderScopes.TestRunSummary;
    ...
    public IEnumerable<FieldInfo> GetFields(IVssRequestContext requestContext)
    {
      ...
      foreach (TestReportData.DimensionNameLabelPair dimension in new TestRunReportdata().Dimensions())
      {
