...
// Type: Microsoft.TeamFoundation.TestManagement.Server.TestResultsReportsForReleaseController
...
namespace Microsoft.TeamFoundation.TestManagement.Server
{
  ...
  [VersionedApiControllerCustomName(Area = "Test", ResourceName = "ResultSummaryByRelease", ResourceVersion = 1)]
  [DemandFeature("D104EA57-16EA-4191-9B60-160D664EE9A8", true)]
  public class TestResultsReportsForReleaseController : TestResultsControllerBase
  {
      ...
      [FromUri(Name = "releaseToCompare")] Microsoft.TeamFoundation.TestManagement.WebApi.ReleaseReference releaseToCompare = null)
    {
      ...
      if (!this.TestManagementRequestContext.RequestContext.IsFeatureEnabled("TestManagement.Server.EnableDirectTCMS2SCallFromTFS"))
      {
        ...
        summary = testReportsHelper.QueryTestReportForRelease(projectId, release, sourceWorkflow, releaseToCompare1, true, num != 0);
      }
      else
      {
        summary = TestManagementController.InvokeAction<TestResultSummary>((Func<TestResultSummary>) (() => this.TestResultsHttpClient.QueryTestResultsReportForReleaseAsync(this.ProjectId.ToString(), releaseId, releaseEnvId, publishContext, new bool?(includeFailureDetails), releaseToCompare)?.Result));
        this.TestReportsHelper.SecureTestResultSummary(summary);
      }
      summary.TotalRunsCount = 0;
      summary.NoConfigRunsCount = 0;
      return summary;
    }
      ...
      List<Microsoft.TeamFoundation.TestManagement.WebApi.ReleaseReference> releases)
    {
      ...
      if (!this.TestManagementRequestContext.RequestContext.IsFeatureEnabled("TestManagement.Server.EnableDirectTCMS2SCallFromTFS"))
      {
        testResultSummaryList = this.TestReportsHelper.QueryTestSummaryForReleases(new GuidAndString(this.ProjectInfo.Uri, this.ProjectInfo.Id), releases);
      }
      else
      {
        testResultSummaryList = TestManagementController.InvokeAction<List<TestResultSummary>>((Func<List<TestResultSummary>>) (() => this.TestResultsHttpClient.QueryTestResultsSummaryForReleasesAsync((IEnumerable<Microsoft.TeamFoundation.TestManagement.WebApi.ReleaseReference>) releases, this.ProjectId)?.Result));
        testResultSummaryList?.ForEach((Action<TestResultSummary>) (summary => this.TestReportsHelper.SecureTestResultSummary(summary)));
      }
      foreach (TestResultSummary testResultSummary in testResultSummaryList)
      {
        testResultSummary.TotalRunsCount = 0;
        testResultSummary.NoConfigRunsCount = 0;
      }
