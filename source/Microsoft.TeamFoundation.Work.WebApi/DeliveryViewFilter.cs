// Decompiled with JetBrains decompiler
// Type: Microsoft.TeamFoundation.Work.WebApi.DeliveryViewFilter
// Assembly: Microsoft.TeamFoundation.Work.WebApi, Version=19.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a
// MVID: 0C4CCFA0-0616-4E48-A4F0-952E1CB10B12
// Assembly location: C:\Program Files\Azure DevOps Server 2022\Application Tier\Web Services\bin\Plugins\Microsoft.TeamFoundation.Work.WebApi.dll

using System;
using System.Collections.Generic;

namespace Microsoft.TeamFoundation.Work.WebApi
{
  public class DeliveryViewFilter : PlanViewFilter
  {
    public IReadOnlyList<TeamBacklogMapping> ExpandedTeamBacklogMappings;
    public IReadOnlyList<TeamBacklogMapping> UnExpandedTeamBacklogMappings;
    public DateTime StartDate;
    public DateTime EndDate;
    public IEnumerable<string> AdditionalFields;
    public IReadOnlyList<FilterClause> Criteria;
    public IReadOnlyList<List<FilterClause>> FilterClauses;
  }
}
