...
// Type: Microsoft.VisualStudio.Services.ServiceHooks.Server.NotificationResultSummaryBinder
...
{
  internal class NotificationResultSummaryBinder : ObjectBinder<NotificationResultsSummaryDetail>
  {
    private SqlColumnBinder Result = new SqlColumnBinder(nameof (Result));
    private SqlColumnBinder NotificationCount = new SqlColumnBinder(nameof (NotificationCount));

    protected override NotificationResultsSummaryDetail Bind() => new NotificationResultsSummaryDetail()
    {
      Result = (NotificationResult) this.Result.GetInt16((IDataReader) this.Reader),
      NotificationCount = this.NotificationCount.GetInt32((IDataReader) this.Reader)
    };
  }
}
