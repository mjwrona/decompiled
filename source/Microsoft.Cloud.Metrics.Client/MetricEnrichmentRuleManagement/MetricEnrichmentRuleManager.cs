...
// Type: Microsoft.Cloud.Metrics.Client.MetricEnrichmentRuleManagement.MetricEnrichmentRuleManager
...
namespace Microsoft.Cloud.Metrics.Client.MetricEnrichmentRuleManagement
{
  public sealed class MetricEnrichmentRuleManager
  {
    ...
    public async Task<IReadOnlyList<MetricEnrichmentRule>> GetAllAsync(string monitoringAccount)
    {
      ...
      string path = this.configurationUrlPrefix + "getAll/monitoringAccount/" + monitoringAccount;
      IReadOnlyList<MetricEnrichmentRule> allAsync = (IReadOnlyList<MetricEnrichmentRule>) JsonConvert.DeserializeObject<List<MetricEnrichmentRule>>(await HttpClientHelper.GetResponseAsStringAsync(new UriBuilder(await this.connectionInfo.GetEndpointAsync(monitoringAccount).ConfigureAwait(false))
      {
        Path = path,
        Query = "clientVersion=1"
      }.Uri, HttpMethod.Get, this.httpClient, monitoringAccount, this.configurationUrlPrefix).ConfigureAwait(false), this.serializerSettings);
      path = (string) null;
      return allAsync;
    }
    ...
    public async Task SaveAsync(string monitoringAccount, MetricEnrichmentRule rule)
    {
      ...
      string path = this.configurationUrlPrefix + "monitoringAccount/" + monitoringAccount;
      ...
      string serializedContent = JsonConvert.SerializeObject((object) rule);
