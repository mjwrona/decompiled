...
// Type: Microsoft.TeamFoundation.DistributedTask.Orchestration.Tasks.RunServerJob6
...
{
  internal sealed class RunServerJob6 : 
    ...
    public override void OnEvent(OrchestrationContext context, string name, object input)
    {
      switch (name)
      {
        ...
        case "JobCompleted":
          ...
          context.TraceJobError(10015551, this.m_input.ScopeId, this.m_input.PlanId, this.m_input.Job.InstanceId, "RunServerJob6::OnEvent -- Unexpected JobCompleted event when more than one task exist.");
          break;
        case "JobStarted":
        ...
        case "TaskStarted":
          ...
          break;
        default:
          context.TraceJobError(10015551, this.m_input.ScopeId, this.m_input.PlanId, this.m_input.Job.InstanceId, "RunServerJob6::OnEvent -- Unexpected eventName: " + name);
          break;
      }
    }
      ...
      RunServerJobInput input)
    {
      ...
      try
      {
        ...
        foreach (TaskInstance task1 in job.Tasks)
        {
          ...
          ++taskPosition;
          if (task == null)
            context.TraceJobError(10015551, this.m_input.ScopeId, this.m_input.PlanId, this.m_input.Job.InstanceId, string.Format("RunServerJob6::RunTask -- Task is null at position: {0}", (object) taskPosition));
          ...
          else
          {
            ...
            Task<JobResult> subOrchestrationTask = RunServerJob6.CreateSubOrchestrationInstance(context, serverTaskInput, orchestrationId);
            while (true)
            {
              Task task2;
              do
              {
                ...
                else if (task2 == this.m_jobAssigned.Task)
                {
                  ...
                  await this.ExecuteAsync(context, this.m_input, closure_12 ?? (closure_12 = (Func<Task>) (() => this.ExecutionControl.AssignTask(this.m_input.ScopeId, this.m_input.PlanId, this.m_input.Job.InstanceId, task.InstanceId))));
                }
                else if (task2 == this.m_jobStarted.Task)
                {
                  ...
                  if (this.m_hasOnlyOneTask && !this.m_hasReceivedTaskStartedNotification)
                  {
                    ...
                    await this.ExecuteAsync(context, this.m_input, closure_13 ?? (closure_13 = (Func<Task>) (() => this.ExecutionControl.StartTask(this.m_input.ScopeId, this.m_input.PlanId, this.m_input.Job.InstanceId, task.InstanceId))));
                  }
                }
              }
...
label_45:
            jobResult = RunServerJob6.MergeResult(jobResult, taskResult);
    ...
    {
      return context.CreateSubOrchestrationInstance<JobResult>("RunServerTask", "3.0", orchestrationId, (object) input);
    }
      ...
      TaskResult jobResult)
    {
      ...
      switch (jobResult)
      {
        ...
        default:
          flag = false;
          context.TraceJobError(10015551, this.m_input.ScopeId, this.m_input.PlanId, this.m_input.Job.InstanceId, string.Format("RunServerJob6::CanSkipExecution -- Unexpected jobResult: {0}, so skipping task: {1}", (object) jobResult, (object) taskInstance.InstanceId));
          goto case TaskResult.Succeeded;
      }
    }
