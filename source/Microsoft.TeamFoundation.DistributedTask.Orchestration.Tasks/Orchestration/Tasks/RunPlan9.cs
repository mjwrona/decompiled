...
// Type: Microsoft.TeamFoundation.DistributedTask.Orchestration.Tasks.RunPlan9
...
{
  internal sealed class RunPlan9 : RunPlanBase<RunPlanInput3>
  {
    ...
    protected override string RunServerJobVersion => "2.0";
    ...
    public override async Task<TaskResult> RunTask(
      ...
      RunPlanInput3 input)
    {
      RunPlan9 runPlan9 = this;
      runPlan9.EnsureExtensions(context);
      context.TracePlanStarted(input.ScopeId, input.PlanId);
      string resultCode = "";
      TaskResult result = TaskResult.Failed;
      try
      {
        await runPlan9.ExecuteAsync(context, input, (Func<Task>) (() => this.PlanTracker.PlanStarted(input.ScopeId, input.PlanId, context.CurrentUtcDateTime)));
        try
        {
          Tuple<TaskResult, string> tuple = await runPlan9.ExecuteAsync(context, input, input.Implementation);
          result = tuple.Item1;
      ...
      {
        await runPlan9.ExecuteAsync(context, input, (Func<Task>) (() => this.PlanTracker.PlanCompleted(input.ScopeId, input.PlanId, context.CurrentUtcDateTime, result, resultCode)));
    ...
    {
      RunPlan9 runPlan9 = this;
      TaskResult taskResult = await runPlan9.ExecuteContainerAsync(context, input, item);
      if (taskResult != TaskResult.Failed || item.Rollback == null)
        return new Tuple<TaskResult, string>(taskResult, (string) null);
      if (runPlan9.RunState == RunState.Failed)
        runPlan9.SetRunInProgressForRollback();
      runPlan9.m_hasFailedWhilePaused = false;
      return new Tuple<TaskResult, string>(await runPlan9.ExecuteItemAsync(context, input, (TaskOrchestrationItem) item.Rollback, item.ContinueOnError), "Rollback");
    }
      ...
      TaskOrchestrationContainer item)
    {
      ...
      foreach (TaskOrchestrationItem orchestrationItem in item.Children.Take<TaskOrchestrationItem>(count))
        currentlyRunningTasks.Add(runPlan9.ExecuteItemAsync(context, input, orchestrationItem, item.ContinueOnError));
      while (currentlyRunningTasks.Count > 0)
      {
        ...
        containerResult = RunPlan9.Merge(containerResult, jobResult);
        if (containerResult == TaskResult.Failed && !item.ContinueOnError)
        {
          runPlan9.TryCancelSource(item.Children[completedIndex]);
          pendingExecution.Clear();
        }
        if (runPlan9.RunState != RunState.Canceled && pendingExecution.Count > 0)
          currentlyRunningTasks.Add(runPlan9.ExecuteItemAsync(context, input, pendingExecution.Dequeue(), item.ContinueOnError));
      }
      TaskResult taskResult = runPlan9.RunState != RunState.Canceled ? containerResult : TaskResult.Canceled;
    ...
    {
      RunPlan9 runPlan9 = this;
      RunState runState = runPlan9.RunState;
      if (runState == RunState.Paused)
        runState = await runPlan9.WaitForUnpause();
      return runState;
    }
    ...
    {
      RunPlan9 runPlan9 = this;
      if (runPlan9.RunState != RunState.Paused)
        throw new InvalidOperationException("RunPlan is not paused");
      int task = (int) await runPlan9.m_resumeHandle.Task;
      runPlan9.m_resumeHandle = new TaskCompletionSource<RunState>();
