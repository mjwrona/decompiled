...
// Type: Microsoft.TeamFoundation.DistributedTask.Orchestration.Tasks.RunServerJob3
...
{
  internal sealed class RunServerJob3 : 
    TaskOrchestration<JobResult, RunServerJobInput2, object, string>
  {
    private RunServerJobInput2 m_input;
...
label_27:
    ...
    public override async Task<JobResult> RunTask(
      ...
      RunServerJobInput2 input)
    {
      this.EnsureExtensions(context);
      this.m_input = input;
      TaskOrchestrationJob job = await this.ExecuteAsync<TaskOrchestrationJob>(context, this.m_input, (Func<Task<TaskOrchestrationJob>>) (() => this.JobExecutionHandler.GetJob(this.m_input.ScopeId, this.m_input.PlanId, this.m_input.Job.InstanceId)));
      TaskInstance taskInstance1;
      if (input.TaskToExecuteInstanceId.HasValue)
        taskInstance1 = job.Tasks.SingleOrDefault<TaskInstance>((Func<TaskInstance, bool>) (t => t.InstanceId == input.TaskToExecuteInstanceId.Value));
      if (taskInstance1 == null && job.Tasks.Count != 1)
      {
      ...
      TaskInstance taskInstance2 = taskInstance1 ?? job.Tasks.Single<TaskInstance>();
      JobResult jobResult = new JobResult()
      {
        Result = TaskResult.Succeeded
      };
      if (taskInstance2.Enabled)
        jobResult = await this.RunServerTask(context, taskInstance2, input);
      context.TraceServerJobCompleted(this.m_input.ScopeId, this.m_input.PlanId, this.m_input.Job.InstanceId, jobResult.Result);
      if (!input.TaskToExecuteInstanceId.HasValue || job.Tasks.Count == 1)
      ...
      RunServerJobInput2 input)
    {
      ...
      try
      {
        ...
        while (waitQueue.Count >= 1)
        {
          ...
          RunServerJob3.WaitEventData waitEventDetails = this.GetWaitEventDetails(context, eventName);
...
label_40:
    ...
    private RunServerJob3.WaitEventData GetWaitEventDetails(
      OrchestrationContext context,
      string eventName)
    {
      ...
      return new RunServerJob3.WaitEventData()
      {
        WaitTask = task,
        Timeout = nullable
      };
