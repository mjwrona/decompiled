...
// Type: Microsoft.TeamFoundation.DistributedTask.Orchestration.Tasks.RunPlan12
...
{
  internal sealed class RunPlan12 : RunPlanBase<RunPlanInput3>
  {
    ...
    protected override string RunServerJobVersion => "6.0";
    ...
    public override async Task<TaskResult> RunTask(
      ...
      RunPlanInput3 input)
    {
      RunPlan12 runPlan12 = this;
      runPlan12.EnsureExtensions(context);
      context.TracePlanStarted(input.ScopeId, input.PlanId);
      string resultCode = string.Empty;
      TaskResult result = TaskResult.Failed;
      try
      {
        await runPlan12.ExecuteAsync(context, input, (Func<Task>) (() => this.PlanTracker.PlanStarted(input.ScopeId, input.PlanId, context.CurrentUtcDateTime)));
        try
        {
          Tuple<TaskResult, string> tuple = await runPlan12.ExecuteAsync(context, input, input.Implementation);
          result = tuple.Item1;
      ...
      {
        await runPlan12.ExecuteAsync(context, input, (Func<Task>) (() => this.PlanTracker.PlanCompleted(input.ScopeId, input.PlanId, context.CurrentUtcDateTime, result, resultCode)));
    ...
    {
      RunPlan12 runPlan12 = this;
      TaskResult taskResult = await runPlan12.ExecuteContainerAsync(context, input, item);
      if (taskResult != TaskResult.Failed || item.Rollback == null)
        return new Tuple<TaskResult, string>(taskResult, (string) null);
      if (runPlan12.RunState == RunState.Failed)
        runPlan12.SetRunInProgressForRollback();
      runPlan12.m_hasFailedWhilePaused = false;
      return new Tuple<TaskResult, string>(await runPlan12.ExecuteItemAsync(context, input, (TaskOrchestrationItem) item.Rollback, item.ContinueOnError), "Rollback");
    }
      ...
      TaskOrchestrationContainer item)
    {
      ...
      foreach (TaskOrchestrationItem orchestrationItem in item.Children.Take<TaskOrchestrationItem>(count))
        currentlyRunningTasks.Add(runPlan12.ExecuteItemAsync(context, input, orchestrationItem, item.ContinueOnError));
      while (currentlyRunningTasks.Count > 0)
      {
        ...
        containerResult = RunPlan12.Merge(containerResult, jobResult);
        if (containerResult == TaskResult.Failed && !item.ContinueOnError)
        {
          runPlan12.TryCancelSource(item.Children[completedIndex]);
          pendingExecution.Clear();
        }
        if (runPlan12.RunState != RunState.Canceled && pendingExecution.Count > 0)
          currentlyRunningTasks.Add(runPlan12.ExecuteItemAsync(context, input, pendingExecution.Dequeue(), item.ContinueOnError));
      }
      TaskResult taskResult = runPlan12.RunState != RunState.Canceled ? containerResult : TaskResult.Canceled;
    ...
    {
      RunPlan12 runPlan12 = this;
      RunState runState = runPlan12.RunState;
      if (runState == RunState.Paused)
        runState = await runPlan12.WaitForUnpause();
      return runState;
    }
    ...
    {
      RunPlan12 runPlan12 = this;
      if (runPlan12.RunState != RunState.Paused)
        throw new InvalidOperationException("RunPlan is not paused");
      int task = (int) await runPlan12.m_resumeHandle.Task;
      runPlan12.m_resumeHandle = new TaskCompletionSource<RunState>();
