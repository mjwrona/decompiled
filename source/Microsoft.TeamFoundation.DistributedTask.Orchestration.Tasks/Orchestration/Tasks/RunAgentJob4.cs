...
// Type: Microsoft.TeamFoundation.DistributedTask.Orchestration.Tasks.RunAgentJob4
...
{
  internal sealed class RunAgentJob4 : TaskOrchestration<TaskResult, RunJobInput3, object, string>
  {
    ...
    public IAgentPoolExtension4 Pool { get; private set; }
    ...
    public override async Task<TaskResult> RunTask(OrchestrationContext context, RunJobInput3 input)
    {
      ...
      try
      {
        ...
        if (reservation.ReservedAgent != null)
        {
          ...
          await this.ExecuteAsync(context, input, (Func<Task>) (() => this.Pool.StartJob(input.PoolId, reservation.RequestId, reservation.ReservedAgent, input.ScopeId, input.PlanId, input.Job.InstanceId)));
          ...
          while (true)
          {
            ...
            if (task == this.m_jobCanceled.Task)
            {
              ...
              await this.ExecuteAsync(context, input, (Func<Task>) (() => this.Pool.CancelJob(input.PoolId, reservation.RequestId, reservation.ReservedAgent, input.ScopeId, input.PlanId, input.Job.InstanceId, canceledEvent.Timeout)));
              if (!string.IsNullOrEmpty(canceledEvent.Reason))
                await this.ExecuteAsync(context, input, (Func<Task>) (() => this.Tracker.LogIssue(input.ScopeId, input.PlanId, input.Job.InstanceId, context.CurrentUtcDateTime, IssueType.Error, canceledEvent.Reason)));
            }
            else if (task == jobExecutionTimerTask)
            {
              ...
              this.m_canceled = true;
              timerCancel.Cancel();
              await this.ExecuteAsync(context, input, (Func<Task>) (() => this.Pool.CancelJob(input.PoolId, reservation.RequestId, reservation.ReservedAgent, input.ScopeId, input.PlanId, input.Job.InstanceId, this.m_cancelTimeout)));
...
label_39:
    ...
    {
      this.Pool = context.CreateClient<IAgentPoolExtension4>(true);
