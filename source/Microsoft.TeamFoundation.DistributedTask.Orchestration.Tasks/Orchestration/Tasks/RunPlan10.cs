...
// Type: Microsoft.TeamFoundation.DistributedTask.Orchestration.Tasks.RunPlan10
...
{
  internal sealed class RunPlan10 : RunPlanBase<RunPlanInput3>
  {
    ...
    protected override string RunServerJobVersion => "4.0";
    ...
    public override async Task<TaskResult> RunTask(
      ...
      RunPlanInput3 input)
    {
      RunPlan10 runPlan10 = this;
      runPlan10.EnsureExtensions(context);
      context.TracePlanStarted(input.ScopeId, input.PlanId);
      string resultCode = string.Empty;
      TaskResult result = TaskResult.Failed;
      try
      {
        await runPlan10.ExecuteAsync(context, input, (Func<Task>) (() => this.PlanTracker.PlanStarted(input.ScopeId, input.PlanId, context.CurrentUtcDateTime)));
        try
        {
          Tuple<TaskResult, string> tuple = await runPlan10.ExecuteAsync(context, input, input.Implementation);
          result = tuple.Item1;
      ...
      {
        await runPlan10.ExecuteAsync(context, input, (Func<Task>) (() => this.PlanTracker.PlanCompleted(input.ScopeId, input.PlanId, context.CurrentUtcDateTime, result, resultCode)));
    ...
    {
      RunPlan10 runPlan10 = this;
      TaskResult taskResult = await runPlan10.ExecuteContainerAsync(context, input, item);
      if (taskResult != TaskResult.Failed || item.Rollback == null)
        return new Tuple<TaskResult, string>(taskResult, (string) null);
      if (runPlan10.RunState == RunState.Failed)
        runPlan10.SetRunInProgressForRollback();
      runPlan10.m_hasFailedWhilePaused = false;
      return new Tuple<TaskResult, string>(await runPlan10.ExecuteItemAsync(context, input, (TaskOrchestrationItem) item.Rollback, item.ContinueOnError), "Rollback");
    }
      ...
      TaskOrchestrationContainer item)
    {
      ...
      foreach (TaskOrchestrationItem orchestrationItem in item.Children.Take<TaskOrchestrationItem>(count))
        currentlyRunningTasks.Add(runPlan10.ExecuteItemAsync(context, input, orchestrationItem, item.ContinueOnError));
      while (currentlyRunningTasks.Count > 0)
      {
        ...
        containerResult = RunPlan10.Merge(containerResult, jobResult);
        if (containerResult == TaskResult.Failed && !item.ContinueOnError)
        {
          runPlan10.TryCancelSource(item.Children[completedIndex]);
          pendingExecution.Clear();
        }
        if (runPlan10.RunState != RunState.Canceled && pendingExecution.Count > 0)
          currentlyRunningTasks.Add(runPlan10.ExecuteItemAsync(context, input, pendingExecution.Dequeue(), item.ContinueOnError));
      }
      TaskResult taskResult = runPlan10.RunState != RunState.Canceled ? containerResult : TaskResult.Canceled;
    ...
    {
      RunPlan10 runPlan10 = this;
      RunState runState = runPlan10.RunState;
      if (runState == RunState.Paused)
        runState = await runPlan10.WaitForUnpause();
      return runState;
    }
    ...
    {
      RunPlan10 runPlan10 = this;
      if (runPlan10.RunState != RunState.Paused)
        throw new InvalidOperationException("RunPlan is not paused");
      int task = (int) await runPlan10.m_resumeHandle.Task;
      runPlan10.m_resumeHandle = new TaskCompletionSource<RunState>();
