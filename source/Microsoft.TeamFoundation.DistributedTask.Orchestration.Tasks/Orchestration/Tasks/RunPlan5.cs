...
// Type: Microsoft.TeamFoundation.DistributedTask.Orchestration.Tasks.RunPlan5
...
{
  internal sealed class RunPlan5 : TaskOrchestration<TaskResult, RunPlanInput3, object, string>
  {
      ...
      TaskOrchestrationContainer item)
    {
      ...
      if (item.Parallel)
      {
        ...
        while (allTasks.Count > 0)
        {
          ...
          containerResult = RunPlan5.Merge(containerResult, jobResult);
      ...
      bool continueOnError)
    {
      ...
      try
      {
        ...
        jobExecuted = true;
        RunJobInput3 jobInput = new RunJobInput3()
        {
          PoolId = input.PoolId,
        ...
        for (int attempt = 0; attempt < 2; ++attempt)
        {
          ...
          Task<JobResult> jobTask = context.CreateSubOrchestrationInstance<JobResult>("RunAgentJob", "5.0", jobInstanceId, (object) jobInput);
          if (await Task.WhenAny((Task) this.m_cancellationSource.Task, (Task) jobTask) == this.m_cancellationSource.Task)
          {
            ...
            await context.ExecuteAsync((Func<Task>) (() => this.JobController.CancelJob(jobInstanceId, canceledEvent.Timeout, canceledEvent.Reason)), canRetry: this.RetryException, traceException: closure_5 ?? (closure_5 = (Action<Exception>) (ex => context.TracePlanException(input.ScopeId, input.PlanId, ex))));
          }
          JobResult jobResult = await jobTask;
          result = jobResult.Result;
          errorMessage = jobResult.Message;
          if (!jobResult.IsRetryable || this.RunState == RunState.Canceled || jobResult.Result != TaskResult.Failed && jobResult.Result != TaskResult.Abandoned)
            break;
        }
        jobInput = (RunJobInput3) null;
      }
...
label_17:
      ...
      if (!string.IsNullOrEmpty(errorMessage))
        await this.LogIssue(context, input, job, IssueType.Error, errorMessage);
      ...
      string message)
    {
      try
      {
        await this.ExecuteAsync(context, input, (Func<Task>) (() => this.PlanTracker.LogIssue(input.ScopeId, input.PlanId, job.InstanceId, context.CurrentUtcDateTime, issueType, message)));
      }
      catch (TaskFailedException ex)
      {
      }
