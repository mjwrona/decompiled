...
// Type: Microsoft.TeamFoundation.DistributedTask.Orchestration.Tasks.RunServerJob4
...
{
  internal sealed class RunServerJob4 : 
    ...
    public override void OnEvent(OrchestrationContext context, string name, object input)
    {
      switch (name)
      {
        ...
        case "JobCompleted":
          ...
          context.TraceJobError(10015551, this.m_input.ScopeId, this.m_input.PlanId, this.m_input.Job.InstanceId, "RunServerJob4::OnEvent -- Unexpected JobCompleted event when more than one task exist.");
          break;
        case "JobStarted":
        ...
        case "TaskStarted":
          ...
          break;
        default:
          context.TraceJobError(10015551, this.m_input.ScopeId, this.m_input.PlanId, this.m_input.Job.InstanceId, "RunServerJob4::OnEvent -- Unexpected eventName: " + name);
          break;
      }
    }
      ...
      RunServerJobInput input)
    {
      ...
      try
      {
        ...
        foreach (TaskInstance task1 in job.Tasks)
        {
          ...
          ++taskPosition;
          if (task == null)
            context.TraceJobError(10015551, this.m_input.ScopeId, this.m_input.PlanId, this.m_input.Job.InstanceId, string.Format("RunServerJob4::RunTask -- Task is null at position: {0}", (object) taskPosition));
          ...
          else
          {
            ...
            Task<JobResult> subOrchestrationTask = RunServerJob4.CreateSubOrchestrationInstance(context, serverTaskInput, orchestrationId);
            while (true)
            {
              Task task2;
              do
...
label_46:
            jobResult = RunServerJob4.MergeResult(jobResult, taskResult);
    ...
    {
      return context.CreateSubOrchestrationInstance<JobResult>("RunServerTask", "1.0", orchestrationId, (object) input);
    }
      ...
      TaskResult jobResult)
    {
      ...
      switch (jobResult)
      {
        ...
        default:
          flag = false;
          context.TraceJobError(10015551, this.m_input.ScopeId, this.m_input.PlanId, this.m_input.Job.InstanceId, string.Format("RunServerJob4::CanSkipExecution -- Unexpected jobResult: {0}, so skipping task: {1}", (object) jobResult, (object) taskInstance.InstanceId));
          goto case TaskResult.Succeeded;
      }
    }
