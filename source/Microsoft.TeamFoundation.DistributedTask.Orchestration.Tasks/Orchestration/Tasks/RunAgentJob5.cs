...
// Type: Microsoft.TeamFoundation.DistributedTask.Orchestration.Tasks.RunAgentJob5
...
{
  internal sealed class RunAgentJob5 : TaskOrchestration<JobResult, RunJobInput3, object, string>
  {
    private bool m_timedOut;
    private bool m_canceled;
    private RunJobInput3 m_input;
    ...
    public override async Task<JobResult> RunTask(OrchestrationContext context, RunJobInput3 input)
    {
      ...
      try
      {
        ...
        this.m_agentRequest = await this.ExecuteAsync<TaskAgentJobRequest>(context, input, (Func<Task<TaskAgentJobRequest>>) (() => this.Pool.QueueAgentRequest(input.PoolId, (IList<Demand>) input.Job.Demands, input.ScopeId, input.PlanId, input.Job.InstanceId)));
        ...
        if (this.m_agentRequest.ReservedAgent != null)
        {
          ...
          await this.ExecuteAsync(context, input, (Func<Task>) (() => this.Pool.StartJob(input.PoolId, this.m_agentRequest.RequestId, this.m_agentRequest.ReservedAgent, input.ScopeId, input.PlanId, input.Job.InstanceId)));
          jobStarted = true;
