...
// Type: Microsoft.TeamFoundation.DistributedTask.Orchestration.Tasks.RunPlan8
...
{
  internal sealed class RunPlan8 : RunPlanBase<RunPlanInput3>
  {
    ...
    public override async Task<TaskResult> RunTask(
      ...
      RunPlanInput3 input)
    {
      RunPlan8 runPlan8 = this;
      runPlan8.EnsureExtensions(context);
      context.TracePlanStarted(input.ScopeId, input.PlanId);
      string resultCode = "";
      TaskResult result = TaskResult.Failed;
      try
      {
        await runPlan8.ExecuteAsync(context, input, (Func<Task>) (() => this.PlanTracker.PlanStarted(input.ScopeId, input.PlanId, context.CurrentUtcDateTime)));
        try
        {
          Tuple<TaskResult, string> tuple = await runPlan8.ExecuteAsync(context, input, input.Implementation);
          result = tuple.Item1;
      ...
      {
        await runPlan8.ExecuteAsync(context, input, (Func<Task>) (() => this.PlanTracker.PlanCompleted(input.ScopeId, input.PlanId, context.CurrentUtcDateTime, result, resultCode)));
    ...
    {
      RunPlan8 runPlan8 = this;
      TaskResult taskResult = await runPlan8.ExecuteContainerAsync(context, input, item);
      if (taskResult != TaskResult.Failed || item.Rollback == null)
        return new Tuple<TaskResult, string>(taskResult, (string) null);
      if (runPlan8.RunState == RunState.Failed)
        runPlan8.SetRunInProgressForRollback();
      runPlan8.m_hasFailedWhilePaused = false;
      return new Tuple<TaskResult, string>(await runPlan8.ExecuteItemAsync(context, input, (TaskOrchestrationItem) item.Rollback, item.ContinueOnError), "Rollback");
    }
      ...
      TaskOrchestrationContainer item)
    {
      ...
      foreach (TaskOrchestrationItem orchestrationItem in item.Children.Take<TaskOrchestrationItem>(count))
        currentlyRunningTasks.Add(runPlan8.ExecuteItemAsync(context, input, orchestrationItem, item.ContinueOnError));
      while (currentlyRunningTasks.Count > 0)
      {
        ...
        containerResult = RunPlan8.Merge(containerResult, jobResult);
        if (containerResult == TaskResult.Failed && !item.ContinueOnError)
        {
          runPlan8.TryCancelSource(item.Children[completedIndex]);
          pendingExecution.Clear();
        }
        if (runPlan8.RunState != RunState.Canceled && pendingExecution.Count > 0)
          currentlyRunningTasks.Add(runPlan8.ExecuteItemAsync(context, input, pendingExecution.Dequeue(), item.ContinueOnError));
      }
      TaskResult taskResult = runPlan8.RunState != RunState.Canceled ? containerResult : TaskResult.Canceled;
    ...
    {
      RunPlan8 runPlan8 = this;
      RunState runState = runPlan8.RunState;
      if (runState == RunState.Paused)
        runState = await runPlan8.WaitForUnpause();
      return runState;
    }
    ...
    {
      RunPlan8 runPlan8 = this;
      if (runPlan8.RunState != RunState.Paused)
        throw new InvalidOperationException("RunPlan is not paused");
      int task = (int) await runPlan8.m_resumeHandle.Task;
      runPlan8.m_resumeHandle = new TaskCompletionSource<RunState>();
