...
// Type: Microsoft.TeamFoundation.DistributedTask.Pipelines.Tasks.RunAgentPhase
...
{
  public sealed class RunAgentPhase : RunPhase
  {
    protected override Task<JobExecutionState> RunJob(
      ...
      RunPhaseInput input)
    {
      AgentQueueTarget agentQueueTarget = (job.Target ?? phase.Target) as AgentQueueTarget;
      RunAgentJobInput input1 = new RunAgentJobInput()
      {
        ...
        QueueId = agentQueueTarget.Queue.Id,
        Job = job,
        AgentSpecification = agentQueueTarget.AgentSpecification
      };
