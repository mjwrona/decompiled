...
<script id="buildvnext_repository_editor_github" type="text/html">
    <!-- ko with: baseViewModel -->
    <table class="buildvnext-definition-repo buildvnext-variables-grid-table">
        <tbody>
            <tr>
                <td class="option-label connection-id-label"><label><%:BuildServerResources.GitHubConnectionLabelText %></label></td>
                <td class="option-value connection-id-value">
                    <select data-bind="visible: hasConnections(), options: computedConnections, optionsText: 'name', value: selectedConnection"></select>
                    <input type="text" class="invalid" data-bind="enable: false, visible: !hasConnections()"/>
                </td>
                <td class="helpMarkDown" data-bind="visible: invalidAccessToken && accessTokenErrorMessage, showTooltip: { text: accessTokenErrorMessage }" /> 
                ...
                <td class="connection-details"><a href="#" class="github-manage-link" target="_blank" title="<%:BuildServerResources.ConnectionManageLinkText %>"><%:BuildServerResources.ConnectionManageLinkText %></a></td>
            </tr>
            <tr>
                <td class="option-label"><label><%:BuildServerResources.BuildUserRepositoriesLabel %></label></td>
                <td class="option-value">
                    <select data-bind="visible: !showRepositorySpinner() && hasConnections(), options: computedRepositoryUrls, optionsText: 'displayValue', optionsValue: 'value', value: selectedRepositoryUrl"></select>
                    <input disabled type="text" data-bind="visible: showRepositorySpinner() || !hasConnections()"/>
                </td>
                <td>
                    <img data-bind="visible: showRepositorySpinner()" style="vertical-align: top; margin-right: 2px;" src="<%= StaticResources.Versioned.Content.GetLocation("spinner.gif") %>"/>
                </td>
            </tr>
              <tr>
                <td class="option-label"><label><%:BuildServerResources.BuildRepositoryGitDefaultBranchLabel %></label></td>
                <td class="option-value">
                    <select data-bind="visible: !showRepositorySpinner() && !showBranchSpinner() && hasConnections(), options: computedBranchNames, value: selectedBranchName"></select>
                    <input disabled type="text" data-bind="visible: showRepositorySpinner() ||showBranchSpinner() || !hasConnections()" />
                </td>
                <td>
                    <img data-bind="visible: showBranchSpinner()" style="vertical-align: top; margin-right: 2px;" src="<%= StaticResources.Versioned.Content.GetLocation("spinner.gif") %>"/>
                </td>
            </tr>
            <tr>
                <td><label><%:BuildServerResources.BuildRepositoryCheckoutSubmodulesLabel %></label></td>
                <td><input type="checkbox" data-bind="checked: checkoutSubmodules" /></td>
            </tr>
            <tr>
