    ...
    <xsl:for-each select="planAndSuites">
      <div id="exported-data">
        ...
        <xsl:for-each select="testPlan">
          <div class="heading">
              Testplan <xsl:call-template name="url-generator">
                  <xsl:with-param name="label">
                      Testplan:<xsl:value-of select="@title"/> 
                  </xsl:with-param>
          ...
          <xsl:for-each select="configurations">
            ...
            <div class="sub-tab">
              KONFIGURATIONEN IM TESTPLAN
            </div>
            <table class="table-style">
              <tr>
                <th width="5%">ID</th>
                <th width="45%">Name</th>
                <th width="50%">Konfigurationsvariablen</th>
              </tr>
            ...
            <xsl:for-each select="additionalConfiguration">
              <div class="sub-tab">
                ZUSÄTZLICHE KONFIGURATIONEN, AUF DIE VON UNTERGEORDNETEN SAMMLUNGEN VERWIESEN WIRD
              </div>
              <table class="table-style">
                <tr>
                  <th width="5%">ID</th>
        ...
        <xsl:for-each select="testSuites">
          <xsl:for-each select="testSuite">
            ...
            <div class="heading">
              Testsammlung <xsl:call-template name="url-generator">
                <xsl:with-param name="label">
                    Testsammlung: <xsl:value-of select="@title"/>
                </xsl:with-param>
              </xsl:call-template>: <xsl:value-of select="@title"/>
            </div>
                    ...
                    <tr>
                      ...
                      <td>
                          <xsl:call-template name="url-generator">
                              <xsl:with-param name="label">
                                  Anforderung:<xsl:value-of select="@title"/>
                              </xsl:with-param>
                          </xsl:call-template>: <xsl:value-of select="@title"/>
                      </td>
                    </tr>
            ...
            <xsl:for-each select="testCases">
              ...
              <div class="tab">
                Testfälle (<xsl:value-of select="@count"/>)
                <hr/>
              </div>

              <xsl:for-each select="testCase">
                <div class="test-case-heading heading">
                  Testfall <xsl:call-template name="url-generator">
                    <xsl:with-param name="label">
                        Testfall:<xsl:value-of select="@title"/>
                       </xsl:with-param>
                    </xsl:call-template>: <xsl:value-of select="@title"/>
                </div>
                ...
                <xsl:comment>Zusammenfassung</xsl:comment>
                ...
                <xsl:for-each select="parameters">
                  ...
                  <xsl:for-each select="sharedParameterDataSet">
                    <div class="sub-tab">
                      Freigegebener Parameter <xsl:call-template name="url-generator">
                          <xsl:with-param name="label">
                            Freigegebener Parameter:<xsl:value-of select="@title"/>
                          </xsl:with-param>
                ...
                <xsl:comment>Automatisierung</xsl:comment>

                <xsl:for-each select="automation">
                  <xsl:if test="*">
                    <div class="sub-tab">
                      ZUGEORDNETE AUTOMATISIERUNG
                    </div>
                    <xsl:apply-templates select="properties"/>
                  </xsl:if>
                </xsl:for-each>
                <xsl:comment>Aktuelles Testergebnis</xsl:comment>
                <xsl:for-each select="latestTestOutcomes">
                  <div class="avoid-page-break">
                    <div class="sub-tab">
                      AKTUELLSTES TESTERGEBNIS<br/>
                    </div>
                  </div>
