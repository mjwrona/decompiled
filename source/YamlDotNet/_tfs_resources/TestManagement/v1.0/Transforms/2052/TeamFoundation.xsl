...
<!-- Common strings -->
<xsl:variable name="morePrompt">更多 ...</xsl:variable>
<xsl:variable name="FileType">文件</xsl:variable>
<xsl:variable name="FolderType">文件夹</xsl:variable>
<xsl:variable name="datetime">- 所有日期和时间都按 UTC 格式显示</xsl:variable>
...
<xsl:variable name="subscriberText"> 创建者 </xsl:variable>
<xsl:variable name="subscriptionIdText"> 具有 ID </xsl:variable>
<xsl:variable name="Error">处理请求时发生错误。这可能是一个瞬态错误，重试请求或许能够解决问题。如果错误仍然出现，并且您认为此错误需要特别关注，请将本消息和出现的错误消息提供给您的管理员</xsl:variable>
<xsl:variable name="DetailedErrorHeader">详细错误消息(适用于管理人员): </xsl:variable>

  <!-- Item information webview title -->
<xsl:variable name="ChangesetViewTitle">变更集信息</xsl:variable>
<xsl:variable name="ShelvesetViewTitle">搁置集信息</xsl:variable>
<xsl:variable name="CheckinActionResolve">解决</xsl:variable>
<xsl:variable name="CheckinActionAssociate">关联</xsl:variable>
  
<xsl:variable name="alerts-unsubscribe">取消订阅</xsl:variable>
<xsl:variable name="alerts-view">查看</xsl:variable>

<!-- Matcher types -->
<xsl:variable name="matcher-follows">FollowsMatcher</xsl:variable>

<xsl:variable name="follows-reason">- 我们向你发送了此通知，因为你关注了此</xsl:variable>
...
<xsl:text>
</xsl:text><xsl:text>服务器路径: </xsl:text>       <xsl:value-of select="Item/@item"/>
<xsl:text>
</xsl:text><xsl:text>变更集: </xsl:text>         <xsl:if test="string-length(Item/@csurl) &gt; 0">
<xsl:call-template name="link">
...
<xsl:value-of select="Item/@cs"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>上次修改时间: </xsl:text>  <xsl:value-of select="Item/@date"/>
<xsl:if test="Item/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>类型: </xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
<xsl:if test="Item/@type = 'Folder'">
  <xsl:text>
</xsl:text><xsl:text>类型: </xsl:text>              <xsl:value-of select="$FolderType"/>
</xsl:if>
<xsl:if test="@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>文件大小(字节): </xsl:text> <xsl:value-of select="Item/@len"/>
</xsl:if>
</xsl:when>
</xsl:choose>
...
<xsl:text>
</xsl:text><xsl:text>服务器路径: </xsl:text>       <xsl:value-of select="PendingChange/@item"/>
<xsl:if test="string-length(PendingChange/@srcitem) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>源服务器路径: </xsl:text>       <xsl:value-of select="PendingChange/@srcitem"/>
</xsl:if>
<xsl:if test="string-length(PendingChange/@ssurl) &gt;0">
<xsl:text>
</xsl:text><xsl:text>搁置集: </xsl:text>
<xsl:call-template name="link">
<xsl:with-param name="format" select="'text'"/>
<xsl:with-param name="link" select="PendingChange/@ssurl"/>
<xsl:with-param name="addTracking" select="'true'"/>
<xsl:with-param name="displayText" select="PendingChange/@ss"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>创建时间: </xsl:text>  <xsl:value-of select="PendingChange/@date"/>
<xsl:if test="PendingChange/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>类型: </xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
...
<xsl:if test="string-length(PendingChange/@ssurl) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>更改: </xsl:text>              <xsl:value-of select="PendingChange/@chg"/>
</xsl:if>
</xsl:when>
</xsl:choose>
<xsl:call-template name="footer">
...
<xsl:choose>
    <xsl:when test="Owner != Committer">
        <td class="PropName">代表以下用户签入: </td>
    </xsl:when>
    <xsl:when test="Owner = Committer">
        <td class="PropName">签入者: </td>
    </xsl:when>
</xsl:choose>
<td class="PropValue">
...
<tr>
  ...
  <td class="PropValue">
    ...
    <xsl:if test="$valueLength = 0"><pre>无</pre></xsl:if>
  </td>
</tr>
</xsl:for-each>
<tr>
<td class="PropName">注释: </td>
<td class="PropValue">
...
<tr>
  <td class="ColHeadingSmall">类型</td>
  <td class="ColHeadingXSmall">ID</td>
  <td class="ColHeading">标题</td>
  <td class="ColHeadingSmall">状态</td>
  <td class="ColHeadingSmall">指派给</td>
</tr>
...
<table class="WithBorder">
<tr>
<td class="ColHeading">名称</td>
<td class="ColHeadingSmall">更改</td>
<td class="ColHeading">文件夹</td>
</tr>
...
<tr>
  ...
  <td class="ColHeadingXSmall">ID</td>
  <td class="ColHeading">标题</td>
  <td class="ColHeadingSmall">操作</td>
  <td class="ColHeadingSmall">指派给</td>
</tr>
...
<xsl:template name="wiItem">
    ...
    <td class="ColDataSmall">
      ...
      <xsl:if test="$assignedToLength = 0">不适用</xsl:if>
    </td>
</xsl:template> <!-- wiItem -->
