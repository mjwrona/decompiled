...
<!-- Common strings -->
<xsl:variable name="morePrompt">Altro...</xsl:variable>
<xsl:variable name="FileType">File</xsl:variable>
<xsl:variable name="FolderType">Cartella</xsl:variable>
<xsl:variable name="datetime">- Tutti i valori di data e ora sono espressi nel formato UTC</xsl:variable>
...
<xsl:variable name="by">Fornito da </xsl:variable>
<xsl:variable name="subscriberText"> creata da </xsl:variable>
<xsl:variable name="subscriptionIdText"> con ID </xsl:variable>
<xsl:variable name="Error">Si è verificato un errore durante l'elaborazione della richiesta. Poiché potrebbe trattarsi di un errore temporaneo, si consiglia di riprovare la richiesta. Se l’errore persiste o se lo si ritiene opportuno, fornire questo messaggio e il messaggio o i messaggi di errore visualizzati al personale amministrativo</xsl:variable>
<xsl:variable name="DetailedErrorHeader">Messaggi di errore dettagliati (per il personale amministrativo):</xsl:variable>

  <!-- Item information webview title -->
<xsl:variable name="ChangesetViewTitle">Informazioni insieme di modifiche</xsl:variable>
<xsl:variable name="ShelvesetViewTitle">Informazioni shelveset</xsl:variable>
<xsl:variable name="CheckinActionResolve">Risolvi</xsl:variable>
<xsl:variable name="CheckinActionAssociate">Associa</xsl:variable>
  
<xsl:variable name="alerts-unsubscribe">Annulla sottoscrizione</xsl:variable>
<xsl:variable name="alerts-view">Visualizza</xsl:variable>

<!-- Matcher types -->
<xsl:variable name="matcher-follows">FollowsMatcher</xsl:variable>

<xsl:variable name="follows-reason">- Si è ricevuta questa notifica perché si segue il tipo di artefatto </xsl:variable>
...
<xsl:choose>
    ...
    <table>
        <tr>
            <td class="PropName">Percorso server:</td>
            <td class="PropValue"><xsl:value-of select="Item/@item"/></td>
        </tr>
        <tr>
            <td class="PropName">Insieme di modifiche:</td>
            <td class="PropValue">
        ...
        <tr>
            <td class="PropName">Ultima modifica:</td>
        ...
        <xsl:if test="Item/@type = 'File'">
        <tr>
            <td class="PropName">Dimensione file (byte):</td>
...
<xsl:text>
</xsl:text><xsl:text>Riepilogo</xsl:text>
<xsl:text>
</xsl:text><xsl:text>Percorso server:</xsl:text>       <xsl:value-of select="Item/@item"/>
<xsl:text>
</xsl:text><xsl:text>Insieme di modifiche:</xsl:text>         <xsl:if test="string-length(Item/@csurl) &gt; 0">
<xsl:call-template name="link">
...
<xsl:value-of select="Item/@cs"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>Ultima modifica:</xsl:text>  <xsl:value-of select="Item/@date"/>
<xsl:if test="Item/@type = 'File'">
  <xsl:text>
...
<xsl:if test="@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>Dimensione file (byte):</xsl:text> <xsl:value-of select="Item/@len"/>
</xsl:if>
</xsl:when>
</xsl:choose>
...
<xsl:choose>
    ...
    <table>
        ...
        <tr>
            <td class="PropName">Data creazione:</td>
...
<xsl:text>
</xsl:text><xsl:text>Percorso server:</xsl:text>       <xsl:value-of select="PendingChange/@item"/>
<xsl:if test="string-length(PendingChange/@srcitem) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>Percorso server origine:</xsl:text>       <xsl:value-of select="PendingChange/@srcitem"/>
</xsl:if>
<xsl:if test="string-length(PendingChange/@ssurl) &gt;0">
<xsl:text>
</xsl:text><xsl:text>Shelveset:</xsl:text>
<xsl:call-template name="link">
<xsl:with-param name="format" select="'text'"/>
<xsl:with-param name="link" select="PendingChange/@ssurl"/>
<xsl:with-param name="addTracking" select="'true'"/>
<xsl:with-param name="displayText" select="PendingChange/@ss"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>Data creazione:</xsl:text>  <xsl:value-of select="PendingChange/@date"/>
<xsl:if test="PendingChange/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>Tipo:</xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
...
<xsl:if test="string-length(PendingChange/@ssurl) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>Modifica:</xsl:text>              <xsl:value-of select="PendingChange/@chg"/>
</xsl:if>
</xsl:when>
</xsl:choose>
<xsl:call-template name="footer">
...
<tr>
<td class="PropName">Progetti team:</td>
<td class="PropValue">
...
<xsl:choose>
    <xsl:when test="Owner != Committer">
        <td class="PropName">Archiviato per conto di:</td>
    </xsl:when>
    <xsl:when test="Owner = Committer">
        <td class="PropName">Archiviato da:</td>
    </xsl:when>
</xsl:choose>
<td class="PropValue">
...
<tr>
<td class="PropName">Archiviato il:</td>
<td class="PropValue">
...
<tr>
  ...
  <td class="PropValue">
    ...
    <xsl:if test="$valueLength = 0"><pre>Nessuno</pre></xsl:if>
  </td>
</tr>
</xsl:for-each>
<tr>
<td class="PropName">Commento:</td>
<td class="PropValue">
...
<xsl:if test="string-length(PolicyOverrideComment) &gt; 0">
 <tr>
 <td class="PropName">Motivo dell'override dei criteri:</td>
<td class="PropValue">
...
<br/>
<b>Elementi di lavoro risolti</b>
<table class="WithBorder">
<tr>
  <td class="ColHeadingSmall">Tipo</td>
  <td class="ColHeadingXSmall">ID</td>
  <td class="ColHeading">Titolo</td>
  <td class="ColHeadingSmall">Stato</td>
  <td class="ColHeadingSmall">Assegnato a</td>
</tr>
...
<br/>
<b>Elementi di lavoro associati</b>
<table class="WithBorder">
<tr>
  <td class="ColHeadingSmall">Tipo</td>
  <td class="ColHeadingXSmall">ID</td>
...
<br/>
<b>Errori dei criteri</b>
<table class="WithBorder">
<tr>
  <td class="ColHeading">Tipo</td>
  <td class="ColHeading">Descrizione</td>
</tr>
...
<table class="WithBorder">
<tr>
<td class="ColHeading">Nome</td>
<td class="ColHeadingSmall">Modifica</td>
<td class="ColHeading">Cartella</td>
</tr>
...
<tr>
<td class="PropName">Proprietario:</td>
<td class="PropValue">
    <xsl:variable name="owner" select="OwnerDisplay"/>
    <xsl:if test="$owner=''">
        <xsl:value-of select="OwnerDisplay"/>
...
<tr>
  ...
  <td class="PropValue">
    <xsl:variable name="valueLength" select="string-length(@val)"/>
    <xsl:if test="$valueLength &gt; 0"><pre><xsl:value-of select="@val"/></pre></xsl:if>
    <xsl:if test="$valueLength = 0"><pre>Nessuna</pre></xsl:if>
  </td>
</tr>
</xsl:for-each>
<tr>
...
<br/>
<b>Elementi di lavoro</b>
<table class="WithBorder">
<tr>
  <td class="ColHeadingSmall">Tipo</td>
  <td class="ColHeadingXSmall">ID</td>
  <td class="ColHeading">Titolo</td>
  <td class="ColHeadingSmall">Azione</td>
  <td class="ColHeadingSmall">Assegnato a</td>
</tr>
...
<br/>
<b>Modifiche sottoposte a shelving</b>
<table class="WithBorder">
<tr>
<td class="ColHeading">Nome</td>
<td class="ColHeadingSmall">Modifica</td>
