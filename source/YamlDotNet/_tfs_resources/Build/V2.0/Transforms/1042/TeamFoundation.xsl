...
<!-- Common strings -->
<xsl:variable name="morePrompt">자세히...</xsl:variable>
<xsl:variable name="FileType">파일</xsl:variable>
<xsl:variable name="FolderType">폴더</xsl:variable>
<xsl:variable name="datetime">- 모든 날짜 및 시간이 UTC로 표시됩니다.</xsl:variable>
...
<xsl:variable name="by">제공자 </xsl:variable>
<xsl:variable name="subscriberText"> 만든 사람 </xsl:variable>
<xsl:variable name="subscriptionIdText"> Id 포함</xsl:variable>
<xsl:variable name="Error">요청을 처리하는 동안 오류가 발생했습니다. 일시적인 오류일 수 있으므로 다시 시도해 보십시오. 이 오류가 계속되거나 주의를 기울여야 할 오류로 보이면 이 메시지와 오류 메시지를 관리자에게 전달해 주십시오.</xsl:variable>
<xsl:variable name="DetailedErrorHeader">자세한 오류 메시지(관리자용):</xsl:variable>

  <!-- Item information webview title -->
<xsl:variable name="ChangesetViewTitle">변경 집합 정보</xsl:variable>
<xsl:variable name="ShelvesetViewTitle">보류 집합 정보</xsl:variable>
<xsl:variable name="CheckinActionResolve">해결</xsl:variable>
<xsl:variable name="CheckinActionAssociate">연결</xsl:variable>
  
<xsl:variable name="alerts-unsubscribe">구독 취소</xsl:variable>
<xsl:variable name="alerts-view">보기</xsl:variable>

<!-- Matcher types -->
<xsl:variable name="matcher-follows">FollowsMatcher</xsl:variable>

<xsl:variable name="follows-reason">- 팔로우하고 있으므로 이 알림을 보냈습니다. </xsl:variable>
...
<xsl:choose>
    ...
    <table>
        <tr>
            <td class="PropName">서버 경로:</td>
            <td class="PropValue"><xsl:value-of select="Item/@item"/></td>
        </tr>
        <tr>
            <td class="PropName">변경 집합:</td>
            <td class="PropValue">
        ...
        <tr>
            <td class="PropName">마지막으로 수정한 날짜:</td>
            <td class="PropValue"><xsl:value-of select="Item/@date"/></td>
        </tr>
        <tr>
            <td class="PropName">형식:</td>
            <td class="PropValue">
        ...
        <xsl:if test="Item/@type = 'File'">
        <tr>
            <td class="PropName">파일 크기(바이트):</td>
...
<xsl:text>
</xsl:text><xsl:text>요약</xsl:text>
<xsl:text>
</xsl:text><xsl:text>서버 경로:</xsl:text>       <xsl:value-of select="Item/@item"/>
<xsl:text>
</xsl:text><xsl:text>변경 집합:</xsl:text>         <xsl:if test="string-length(Item/@csurl) &gt; 0">
<xsl:call-template name="link">
...
<xsl:value-of select="Item/@cs"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>마지막으로 수정한 날짜:</xsl:text>  <xsl:value-of select="Item/@date"/>
<xsl:if test="Item/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>형식:</xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
<xsl:if test="Item/@type = 'Folder'">
  <xsl:text>
</xsl:text><xsl:text>형식:</xsl:text>              <xsl:value-of select="$FolderType"/>
</xsl:if>
<xsl:if test="@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>파일 크기(바이트):</xsl:text> <xsl:value-of select="Item/@len"/>
</xsl:if>
</xsl:when>
</xsl:choose>
...
<xsl:text>
</xsl:text><xsl:text>서버 경로:</xsl:text>       <xsl:value-of select="PendingChange/@item"/>
<xsl:if test="string-length(PendingChange/@srcitem) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>소스 서버 경로:</xsl:text>       <xsl:value-of select="PendingChange/@srcitem"/>
</xsl:if>
<xsl:if test="string-length(PendingChange/@ssurl) &gt;0">
<xsl:text>
</xsl:text><xsl:text>보류 집합:</xsl:text>
<xsl:call-template name="link">
<xsl:with-param name="format" select="'text'"/>
<xsl:with-param name="link" select="PendingChange/@ssurl"/>
<xsl:with-param name="addTracking" select="'true'"/>
<xsl:with-param name="displayText" select="PendingChange/@ss"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>만든 날짜:</xsl:text>  <xsl:value-of select="PendingChange/@date"/>
<xsl:if test="PendingChange/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>형식:</xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
...
<xsl:if test="string-length(PendingChange/@ssurl) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>변경:</xsl:text>              <xsl:value-of select="PendingChange/@chg"/>
</xsl:if>
</xsl:when>
</xsl:choose>
<xsl:call-template name="footer">
...
<xsl:choose>
    <xsl:when test="Owner != Committer">
        <td class="PropName">다음 사용자 대신 체크 인:</td>
    </xsl:when>
    <xsl:when test="Owner = Committer">
        <td class="PropName">체크 인한 사용자:</td>
    </xsl:when>
</xsl:choose>
<td class="PropValue">
...
<tr>
  ...
  <td class="PropValue">
    ...
    <xsl:if test="$valueLength = 0"><pre>없음</pre></xsl:if>
  </td>
</tr>
</xsl:for-each>
<tr>
<td class="PropName">설명:</td>
<td class="PropValue">
...
<tr>
  <td class="ColHeadingSmall">형식</td>
  <td class="ColHeadingXSmall">ID</td>
  <td class="ColHeading">제목</td>
  <td class="ColHeadingSmall">상태</td>
  <td class="ColHeadingSmall">담당자</td>
</tr>
...
<table class="WithBorder">
<tr>
  <td class="ColHeading">형식</td>
  <td class="ColHeading">설명</td>
</tr>
...
<table class="WithBorder">
<tr>
<td class="ColHeading">이름</td>
<td class="ColHeadingSmall">변경</td>
<td class="ColHeading">폴더</td>
</tr>
...
<tr>
  ...
  <td class="ColHeadingXSmall">ID</td>
  <td class="ColHeading">제목</td>
  <td class="ColHeadingSmall">작업</td>
  <td class="ColHeadingSmall">담당자</td>
</tr>
...
<xsl:template name="wiItem">
    ...
    <td class="ColDataSmall">
      ...
      <xsl:if test="$assignedToLength = 0">해당 없음</xsl:if>
    </td>
</xsl:template> <!-- wiItem -->
