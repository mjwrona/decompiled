...
<!-- Common strings -->
<xsl:variable name="morePrompt">其他 ...</xsl:variable>
<xsl:variable name="FileType">檔案</xsl:variable>
<xsl:variable name="FolderType">資料夾</xsl:variable>
<xsl:variable name="datetime">- 所有日期與時間皆會以 UTC 顯示</xsl:variable>
...
<xsl:variable name="by">提供者 </xsl:variable>
<xsl:variable name="subscriberText"> 建立者 </xsl:variable>
<xsl:variable name="subscriptionIdText"> ID 為 </xsl:variable>
<xsl:variable name="Error">處理您的要求時發生錯誤。這可能是暫時性的錯誤，重試要求或許有助於解決這個問題。如果這項錯誤持續發生，或是您認為應該多加注意這項錯誤，請將這個訊息和出現的錯誤訊息提供給管理人員。</xsl:variable>
<xsl:variable name="DetailedErrorHeader">詳細錯誤訊息 (管理人員適用): </xsl:variable>

  <!-- Item information webview title -->
<xsl:variable name="ChangesetViewTitle">變更集資訊</xsl:variable>
<xsl:variable name="ShelvesetViewTitle">擱置集資訊</xsl:variable>
<xsl:variable name="CheckinActionResolve">解析</xsl:variable>
<xsl:variable name="CheckinActionAssociate">關聯</xsl:variable>
  
<xsl:variable name="alerts-unsubscribe">取消訂閱</xsl:variable>
<xsl:variable name="alerts-view">檢視</xsl:variable>

<!-- Matcher types -->
<xsl:variable name="matcher-follows">FollowsMatcher</xsl:variable>

<xsl:variable name="follows-reason">- 我們傳送此通知給您的原因是您正在追蹤此 </xsl:variable>
...
<xsl:text>
</xsl:text><xsl:text>摘要</xsl:text>
<xsl:text>
</xsl:text><xsl:text>伺服器路徑: </xsl:text>       <xsl:value-of select="Item/@item"/>
<xsl:text>
</xsl:text><xsl:text>變更集: </xsl:text>         <xsl:if test="string-length(Item/@csurl) &gt; 0">
<xsl:call-template name="link">
...
<xsl:value-of select="Item/@cs"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>最後修改時間: </xsl:text>  <xsl:value-of select="Item/@date"/>
<xsl:if test="Item/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>類型: </xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
<xsl:if test="Item/@type = 'Folder'">
  <xsl:text>
</xsl:text><xsl:text>類型: </xsl:text>              <xsl:value-of select="$FolderType"/>
</xsl:if>
<xsl:if test="@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>檔案大小 (位元組): </xsl:text> <xsl:value-of select="Item/@len"/>
</xsl:if>
</xsl:when>
</xsl:choose>
...
<xsl:text>
</xsl:text><xsl:text>伺服器路徑: </xsl:text>       <xsl:value-of select="PendingChange/@item"/>
<xsl:if test="string-length(PendingChange/@srcitem) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>來源伺服器路徑: </xsl:text>       <xsl:value-of select="PendingChange/@srcitem"/>
</xsl:if>
<xsl:if test="string-length(PendingChange/@ssurl) &gt;0">
<xsl:text>
</xsl:text><xsl:text>擱置集: </xsl:text>
<xsl:call-template name="link">
<xsl:with-param name="format" select="'text'"/>
<xsl:with-param name="link" select="PendingChange/@ssurl"/>
<xsl:with-param name="addTracking" select="'true'"/>
<xsl:with-param name="displayText" select="PendingChange/@ss"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>建立時間: </xsl:text>  <xsl:value-of select="PendingChange/@date"/>
<xsl:if test="PendingChange/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>類型: </xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
...
<xsl:if test="string-length(PendingChange/@ssurl) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>變更: </xsl:text>              <xsl:value-of select="PendingChange/@chg"/>
</xsl:if>
</xsl:when>
</xsl:choose>
<xsl:call-template name="footer">
...
<xsl:choose>
    <xsl:when test="Owner != Committer">
        <td class="PropName">代表以下簽入: </td>
    </xsl:when>
    <xsl:when test="Owner = Committer">
        <td class="PropName">簽入者: </td>
    </xsl:when>
</xsl:choose>
<td class="PropValue">
...
<tr>
  ...
  <td class="PropValue">
    ...
    <xsl:if test="$valueLength = 0"><pre>無</pre></xsl:if>
  </td>
</tr>
</xsl:for-each>
<tr>
<td class="PropName">註解: </td>
<td class="PropValue">
...
<tr>
  <td class="ColHeadingSmall">類型</td>
  <td class="ColHeadingXSmall">ID</td>
  <td class="ColHeading">標題</td>
  <td class="ColHeadingSmall">狀態</td>
  <td class="ColHeadingSmall">指派給</td>
</tr>
...
<table class="WithBorder">
<tr>
  <td class="ColHeading">類型</td>
  <td class="ColHeading">描述</td>
</tr>
...
<table class="WithBorder">
<tr>
<td class="ColHeading">名稱</td>
<td class="ColHeadingSmall">變更</td>
<td class="ColHeading">資料夾</td>
</tr>
...
<tr>
  ...
  <td class="ColHeadingXSmall">ID</td>
  <td class="ColHeading">標題</td>
  <td class="ColHeadingSmall">動作</td>
  <td class="ColHeadingSmall">指派給</td>
</tr>
...
<xsl:template name="wiItem">
    ...
    <td class="ColDataSmall">
      ...
      <xsl:if test="$assignedToLength = 0">無</xsl:if>
    </td>
</xsl:template> <!-- wiItem -->
