...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="/Report">
    <html>
      <head>
        ...
        <title>Détails de la build</title>
        <style type="text/css">
          .title {
          }
          .title td {
        ...
        <script type="text/javascript" language="javascript">
            ...
            location.href=elem.innerHTML;
              ...
              alert("Impossible d'ouvrir l'URI de fichier à partir de la zone actuelle.");
            }
            return false;
          }
        </script>
  ...
  <xsl:template match="BuildDetail">
    ...
    <table class="heading" cellspacing="0" cellpadding="0" width="100%">
      ...
      <tr>
        <td width="20%">Nom de la build :</td>
        <td>
          <xsl:choose>
      ...
      <tr>
        <td>Éléments de travail ouverts :</td>
        <xsl:choose>
      ...
      <tr>
        <td>Version du contrôle de code source :</td>
        <td>
          <xsl:value-of select="@SourceGetVersion"/>
        </td>
      </tr>
      <tr>
        <td>Nom du jeu de réservations :</td>
        <td>
  ...
  <xsl:template match="BuildInformationNode[@Type='AgentScopeActivityTracking']" name="vstfbuild_agentScopeActivityTrackingNode">
    <div class="information">
      ...
      <div class="property">
        <span>
            État de réservation : <xsl:value-of select="InformationField[Name='ReservationStatus']/Value"/>
        </span>
      </div>
      <div class="property">
        <span>
            Agent réservé : <xsl:value-of select="InformationField[Name='ReservedAgentName']/Value"/> (<xsl:value-of select="InformationField[Name='ReservedAgentUri']/Value"/>)
        </span>
      </div>
  ...
  <xsl:template match="BuildInformationNode[@Type='BuildError']" name="vstfbuild_errorNode">
    <div class="information">
      ...
      <b>Erreur</b>: <xsl:value-of select="InformationField[Name='File']/Value"/>(<xsl:value-of select="InformationField[Name='LineNumber']/Value"/>): <xsl:value-of select="InformationField[Name='Code']/Value"/>: <xsl:value-of select="InformationField[Name='Message']/Value"/>
      <xsl:apply-templates select="BuildInformationNode[@Type!='ActivityProperties']"/>
    </div>
  </xsl:template>
  ...
  <xsl:template match="BuildInformationNode[@Type='BuildWarning']" name="vstfbuild_warningNode">
    <div class="information">
      ...
      <b>Avertissement</b>: <xsl:value-of select="InformationField[Name='File']/Value"/>(<xsl:value-of select="InformationField[Name='LineNumber']/Value"/>): <xsl:value-of select="InformationField[Name='Code']/Value"/>: <xsl:value-of select="InformationField[Name='Message']/Value"/>
      <xsl:apply-templates select="BuildInformationNode[@Type!='ActivityProperties']"/>
    </div>
  </xsl:template>
      ...
      <span>
        <xsl:choose>
          <xsl:when test="string-length(InformationField[Name='ServerPath']/Value) &gt; 0">
            Généré <xsl:value-of select="InformationField[Name='ServerPath']/Value"/>
          </xsl:when>
          <xsl:otherwise>
            Généré <xsl:value-of select="InformationField[Name='LocalPath']/Value"/>
          </xsl:otherwise>
        </xsl:choose>
      </span>
      <xsl:apply-templates select="BuildInformationNode[@Type!='ActivityProperties']"/>
