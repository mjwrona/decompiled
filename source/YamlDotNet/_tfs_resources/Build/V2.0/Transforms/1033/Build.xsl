...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="/Report">
    <html>
      <head>
        ...
        <title _locID="Title">Build Details</title>
        <style type="text/css">
          .title {
          }
          .title td {
        ...
        <script type="text/javascript" language="javascript">
            ...
            location.href=elem.innerHTML;
              ...
              alert("Cannot open file URI from current zone.");
            }
            return false;
          }
        </script>
  ...
  <xsl:template match="BuildDetail">
    ...
    <table class="heading" cellspacing="0" cellpadding="0" width="100%">
      ...
      <tr>
        <td width="20%" _locID="BuildName">Build name:</td>
        <td>
          <xsl:choose>
      ...
      <tr>
        <td _locID="RequestedBy">Requested by:</td>
        <td>
          <xsl:value-of select="@RequestedFor" />
        </td>
      </tr>
      <tr>
        <td _locID="TeamProject">Team project:</td>
        <td>
          <xsl:value-of select="../BuildDefinition[@Uri=$definitionUri]/@TeamProject" />
        </td>
      </tr>
      <tr>
        <td _locID="DefinitionName">Definition name:</td>
        <td>
          <xsl:value-of select="../BuildDefinition[@Uri=$definitionUri]/@Name" />
        </td>
      </tr>
      <tr>
        <td _locID="ControllerName">Controller name:</td>
        <td>
          <xsl:value-of select="../BuildController[@Uri=$controllerUri]/@Name" />
        </td>
      </tr>
      <tr>
        <td _locID="ProcessParameters">Process parameters:</td>
        <td>
          <xsl:apply-templates select="ProcessParameters" />
        </td>
      </tr>
      <tr>
        <td _locID="StartedOn">Started on:</td>
        <td>
          <xsl:value-of select="@StartTime" />
        </td>
      </tr>
      <tr>
        <td _locID="CompletedOn">Completed on:</td>
        <td>
          <xsl:value-of select="@FinishTime" />
        </td>
      </tr>
      <tr>
        <td _locID="LastChangedBy">Last changed by:</td>
        <td>
          <xsl:value-of select="@LastChangedBy" />
        </td>
      </tr>
      <tr>
        <td _locID="LastChangedOn">Last changed on:</td>
        <td>
          <xsl:value-of select="@LastChangedOn" />
        </td>
      </tr>
      <tr>
        <td _locID="Quality">Quality:</td>
        <td>
          <xsl:value-of select="@Quality"/>
        </td>
      </tr>
      <tr>
        <td _locID="WorkItemsOpened">Work items opened:</td>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <td _locID="NotAvailable">Not available</td>
          </xsl:otherwise>
        </xsl:choose>
      </tr>
      <tr>
        <td _locID="SourceControlVersion">Source control version:</td>
        <td>
          <xsl:value-of select="@SourceGetVersion" />
        </td>
      </tr>
      <tr>
        <td _locID="ShelvesetName">Shelveset name:</td>
        <td>
          <xsl:value-of select="@ShelvesetName" />
        </td>
      </tr>
      <tr>
        <td _locID="Log">Log:</td>
        <td>
  ...
  <xsl:template match="BuildInformationNode[@Type='AgentScopeActivityTracking']" name="vstfbuild_agentScopeActivityTrackingNode">
    <div class="information">
      ...
      <span _locID="AgentScopeActivity">
          <xsl:value-of select="InformationField[Name='DisplayText']/Value"/>
      </span>

      <div class="property">
        <span _locID="ReservationStatus">
            Reservation Status: <xsl:value-of select="InformationField[Name='ReservationStatus']/Value"/>
        </span>
      </div>
      <div class="property">
        <span _locID="ReservedAgent">
            Reserved Agent: <xsl:value-of select="InformationField[Name='ReservedAgentName']/Value"/> (<xsl:value-of select="InformationField[Name='ReservedAgentUri']/Value"/>)
        </span>
      </div>
  ...
  <xsl:template match="BuildInformationNode[@Type='BuildMessage']" name="vstfbuild_messageNode">
    <div class="information" _locID="BuildMessage">
  ...
  <xsl:template match="BuildInformationNode[@Type='BuildError']" name="vstfbuild_errorNode">
    <div class="information" _locID="BuildError">
      <xsl:call-template name="vstfbuild_statusImage">
        <xsl:with-param name="status" select="'Failed'" />
      </xsl:call-template>
      <b>Error</b>: <xsl:value-of select="InformationField[Name='File']/Value"/>(<xsl:value-of select="InformationField[Name='LineNumber']/Value"/>): <xsl:value-of select="InformationField[Name='Code']/Value"/>: <xsl:value-of select="InformationField[Name='Message']/Value"/>
      <xsl:apply-templates select="BuildInformationNode[@Type!='ActivityProperties']"/>
    </div>
  </xsl:template>

  <xsl:template match="BuildInformationNode[@Type='BuildWarning']" name="vstfbuild_warningNode">
    <div class="information" _locID="BuildWarning">
      <xsl:call-template name="vstfbuild_statusImage">
        <xsl:with-param name="status" select="'Warning'" />
      </xsl:call-template>
      <b>Warning</b>: <xsl:value-of select="InformationField[Name='File']/Value"/>(<xsl:value-of select="InformationField[Name='LineNumber']/Value"/>): <xsl:value-of select="InformationField[Name='Code']/Value"/>: <xsl:value-of select="InformationField[Name='Message']/Value"/>
      <xsl:apply-templates select="BuildInformationNode[@Type!='ActivityProperties']"/>
    </div>
  </xsl:template>
  ...
  <xsl:template match="BuildInformationNode[@Type='BuildProject']" name="vstfbuild_buildProjectNode">
    <div class="information">
      <span _locID="BuildingProject">
        <xsl:choose>
          <xsl:when test="string-length(InformationField[Name='ServerPath']/Value) > 0">
            Built <xsl:value-of select="InformationField[Name='ServerPath']/Value"/>
          </xsl:when>
          <xsl:otherwise>
            Built <xsl:value-of select="InformationField[Name='LocalPath']/Value"/>
          </xsl:otherwise>
        </xsl:choose>
      </span>
      <xsl:apply-templates select="BuildInformationNode[@Type!='ActivityProperties']"/>
