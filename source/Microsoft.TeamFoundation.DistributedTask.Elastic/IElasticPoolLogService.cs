...
// Type: Microsoft.TeamFoundation.DistributedTask.Elastic.IElasticPoolLogService
...
{
  [DefaultServiceImplementation(typeof (ElasticPoolLogService))]
  public interface IElasticPoolLogService : IVssFrameworkService
  {
    Task AddElasticPoolLogAsync(
    ...
    Task<IReadOnlyList<ElasticPoolLog>> GetElasticPoolLogsAsync(
      IVssRequestContext requestContext,
      int poolId,
      int maxLogCount = -1);

    Task DeleteElasticPoolLogsByTimeAsync(
      IVssRequestContext requestContext,
      int poolId,
