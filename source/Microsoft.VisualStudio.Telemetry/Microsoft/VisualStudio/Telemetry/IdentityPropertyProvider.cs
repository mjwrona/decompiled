// Decompiled with JetBrains decompiler
// Type: Microsoft.VisualStudio.Telemetry.IdentityPropertyProvider
// Assembly: Microsoft.VisualStudio.Telemetry, Version=16.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a
// MVID: 0E58FD5B-7E43-40D6-A963-E92D0F67BACC
// Assembly location: C:\Program Files\Azure DevOps Server 2022\Application Tier\Web Services\bin\Microsoft.VisualStudio.Telemetry.dll

using System.Collections.Generic;
using System.Threading;

namespace Microsoft.VisualStudio.Telemetry
{
  internal class IdentityPropertyProvider : IPropertyProvider
  {
    internal static readonly string HardwareIdPropertyName = "VS.Core.HardwareId";

    public void AddSharedProperties(
      List<KeyValuePair<string, object>> sharedProperties,
      TelemetryContext telemetryContext)
    {
      sharedProperties.Add(new KeyValuePair<string, object>(IdentityPropertyProvider.HardwareIdPropertyName, (object) IdentityInformationProvider.HardwareIdNotObtained));
    }

    public void PostProperties(TelemetryContext telemetryContext, CancellationToken token)
    {
    }
  }
}
