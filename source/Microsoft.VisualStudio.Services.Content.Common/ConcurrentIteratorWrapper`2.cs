...
// Type: Microsoft.VisualStudio.Services.Content.Common.ConcurrentIteratorWrapper`2
...
{
  public abstract class ConcurrentIteratorWrapper<T1, T2> : IConcurrentIterator<T2>, IDisposable
  {
    private readonly IConcurrentIterator<T1> baseEnumerator;

    protected ConcurrentIteratorWrapper(IConcurrentIterator<T1> baseEnumerator)
    {
    ...
    public async Task<bool> MoveNextAsync(CancellationToken cancellationToken)
    {
      ...
      while (!this.OnBaseValueEnumerated(this.baseEnumerator.Current));
      return true;
    }
