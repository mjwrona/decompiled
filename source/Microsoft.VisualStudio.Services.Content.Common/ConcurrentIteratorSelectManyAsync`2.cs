...
// Type: Microsoft.VisualStudio.Services.Content.Common.ConcurrentIteratorSelectManyAsync`2
...
{
  public class ConcurrentIteratorSelectManyAsync<T1, T2> : IConcurrentIterator<T2>, IDisposable
  {
    private readonly IConcurrentIterator<T1> baseEnumerator;
    private IConcurrentIterator<T2> currentEnumerator;
    private readonly Func<T1, CancellationToken, Task<IConcurrentIterator<T2>>> selector;

    public ConcurrentIteratorSelectManyAsync(
      ...
      if (flag)
      {
        do
        {
          if (!await this.baseEnumerator.MoveNextAsync(cancellationToken).ConfigureAwait(true))
            return false;
          this.currentEnumerator = await this.selector(this.baseEnumerator.Current, cancellationToken).ConfigureAwait(true);
        }
