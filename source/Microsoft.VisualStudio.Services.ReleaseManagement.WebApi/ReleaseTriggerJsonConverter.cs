...
// Type: Microsoft.VisualStudio.Services.ReleaseManagement.WebApi.ReleaseTriggerJsonConverter
...
{
  public class ReleaseTriggerJsonConverter : VssSecureJsonConverter
  {
    public override bool CanConvert(Type objectType) => typeof (ReleaseTriggerBase).GetTypeInfo().IsAssignableFrom(objectType.GetTypeInfo());
      ...
      JsonSerializer serializer)
    {
      ...
      if (!jobject.TryGetValue(closestMatchProperty.PropertyName, StringComparison.OrdinalIgnoreCase, out jtoken))
        return existingValue;
      ReleaseTriggerType result;
      if (jtoken.Type == JTokenType.Integer)
        result = (ReleaseTriggerType) (int) jtoken;
      else if (jtoken.Type != JTokenType.String || !Enum.TryParse<ReleaseTriggerType>((string) jtoken, true, out result))
        return existingValue;
      object target = (object) null;
      switch (result)
      {
        case ReleaseTriggerType.ArtifactSource:
          target = (object) new ArtifactSourceTrigger();
          break;
        case ReleaseTriggerType.Schedule:
          target = (object) new ScheduledReleaseTrigger();
          break;
        case ReleaseTriggerType.SourceRepo:
          target = (object) new SourceRepoTrigger();
          break;
        case ReleaseTriggerType.ContainerImage:
          target = (object) new ContainerImageTrigger();
          break;
        case ReleaseTriggerType.Package:
          target = (object) new PackageTrigger();
          break;
