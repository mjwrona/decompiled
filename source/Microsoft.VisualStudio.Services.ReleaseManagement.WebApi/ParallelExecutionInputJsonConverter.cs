...
// Type: Microsoft.VisualStudio.Services.ReleaseManagement.WebApi.ParallelExecutionInputJsonConverter
...
{
  public class ParallelExecutionInputJsonConverter : VssSecureJsonConverter
  {
      ...
      JsonSerializer serializer)
    {
      ...
      JsonProperty closestMatchProperty = jsonObjectContract.Properties.GetClosestMatchProperty("ParallelExecutionType");
      ...
      if (!jobject.TryGetValue(closestMatchProperty.PropertyName, StringComparison.OrdinalIgnoreCase, out jtoken))
        return existingValue;
      ParallelExecutionTypes result;
      if (jtoken.Type == JTokenType.Integer)
        result = (ParallelExecutionTypes) (int) jtoken;
      else if (jtoken.Type != JTokenType.String || !Enum.TryParse<ParallelExecutionTypes>((string) jtoken, true, out result))
        return existingValue;
      object target = (object) null;
      switch (result)
      {
        case ParallelExecutionTypes.None:
          target = (object) new NoneExecutionInput();
          break;
        case ParallelExecutionTypes.MultiConfiguration:
          target = (object) new MultiConfigInput(string.Empty);
          break;
        case ParallelExecutionTypes.MultiMachine:
          target = (object) new MultiMachineInput();
          break;
      }
      if (target != null)
      {
