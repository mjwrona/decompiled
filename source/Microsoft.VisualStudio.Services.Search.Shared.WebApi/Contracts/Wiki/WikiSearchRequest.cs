...
// Type: Microsoft.VisualStudio.Services.Search.Shared.WebApi.Contracts.Wiki.WikiSearchRequest
...
{
  [DataContract]
  public class WikiSearchRequest : EntitySearchRequest
  {
    ...
    public override bool IsSupportedFilter(string filterName) => WikiSearchRequest.s_validFilters.Contains(filterName);

    public void ValidateFilterHierarchy() => this.ValidateQuery(WikiSearchRequest.ParentOf);
    ...
    public override void ValidateQuery()
    {
      ...
      {
        if (!WikiSearchRequest.s_sortSupportedFields.Contains(sortOption.Field))
