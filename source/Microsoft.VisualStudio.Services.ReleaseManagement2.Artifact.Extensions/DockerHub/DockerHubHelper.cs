...
// Type: Microsoft.VisualStudio.Services.ReleaseManagement.Artifact.Extensions.DockerHub.DockerHubHelper
...
namespace Microsoft.VisualStudio.Services.ReleaseManagement.Artifact.Extensions.DockerHub
{
  public static class DockerHubHelper
  {
    public static IList<ContainerImageTrigger> GetDockerHubTriggers(ReleaseDefinition definition)
    {
      IList<ContainerImageTrigger> dockerHubTriggers = (IList<ContainerImageTrigger>) new List<ContainerImageTrigger>();
      if (definition != null && definition.Triggers != null)
      {
        foreach (ReleaseTriggerBase trigger in (IEnumerable<ReleaseTriggerBase>) definition.Triggers)
        {
          if (trigger.TriggerType == ReleaseTriggerType.ContainerImage)
          {
            ...
            if (artifactSource != null && artifactSource.IsDockerHubArtifact)
              dockerHubTriggers.Add(containerRepoTrigger);
          }
        }
      }
      return dockerHubTriggers;
    }
  }
}
