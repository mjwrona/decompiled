...
// Type: Microsoft.TeamFoundation.DistributedTask.Orchestration.Server.DataAccess.TimelineRecordBinder8
...
{
  internal class TimelineRecordBinder8 : ObjectBinder<TimelineRecord>
  {
    ...
    private SqlColumnBinder m_agentSpecification = new SqlColumnBinder("AgentSpecification");
    ...
    protected override TimelineRecord Bind()
    {
      ...
      timelineRecord1.QueueId = this.m_queueId.GetNullableInt32((IDataReader) this.Reader);
      timelineRecord1.AgentSpecification = JsonUtility.Deserialize<JObject>(this.m_agentSpecification.GetBytes((IDataReader) this.Reader, true));
      ...
      if (this.m_timelineId.ColumnExists((IDataReader) this.Reader))
        timelineRecord1.TimelineId = this.m_timelineId.GetNullableGuid((IDataReader) this.Reader);
