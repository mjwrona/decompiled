...
// Type: Microsoft.TeamFoundation.DistributedTask.Orchestration.Server.DataAccess.TaskTrackingComponent35
...
{
  internal class TaskTrackingComponent35 : TaskTrackingComponent34
  {
    public override int GetPlanStartedCount(DateTime startTime, DateTime endTime)
    {
      using (new TaskSqlComponentBase.SqlMethodScope((TaskSqlComponentBase) this, nameof (GetPlanStartedCount)))
      {
        this.PrepareStoredProcedure("Task.prc_GetPlanStartedCount");
        ...
        using (ResultCollection resultCollection = new ResultCollection((IDataReader) this.ExecuteReader(), this.ProcedureName, this.RequestContext))
        {
          ...
          return resultCollection.GetCurrent<int>().FirstOrDefault<int>();
        }
      }
    }
      ...
      bool createExpandedYaml)
    {
      using (new TaskSqlComponentBase.SqlMethodScope((TaskSqlComponentBase) this, nameof (CreatePlan)))
      {
        ...
        this.BindBoolean("@createInitLog", createInitializationLog);
        this.BindByte("@state", (byte) plan.State);
        using (ResultCollection resultCollection = new ResultCollection((IDataReader) this.ExecuteReader(), this.ProcedureName, this.RequestContext))
        {
