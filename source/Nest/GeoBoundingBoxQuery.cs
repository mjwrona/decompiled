// Decompiled with JetBrains decompiler
// Type: Nest.GeoBoundingBoxQuery
// Assembly: Nest, Version=7.0.0.0, Culture=neutral, PublicKeyToken=96c599bbe3e70f5d
// MVID: CCE7C15C-052B-4528-A6A5-137560B7864B
// Assembly location: C:\Program Files\Azure DevOps Server 2022\Application Tier\Web Services\bin\Nest.dll

namespace Nest
{
  public class GeoBoundingBoxQuery : 
    FieldNameQueryBase,
    IGeoBoundingBoxQuery,
    IFieldNameQuery,
    IQuery
  {
    public IBoundingBox BoundingBox { get; set; }

    public GeoExecution? Type { get; set; }

    public GeoValidationMethod? ValidationMethod { get; set; }

    public bool? IgnoreUnmapped { get; set; }

    protected override bool Conditionless => GeoBoundingBoxQuery.IsConditionless((IGeoBoundingBoxQuery) this);

    internal override void InternalWrapInContainer(IQueryContainer c) => c.GeoBoundingBox = (IGeoBoundingBoxQuery) this;

    internal static bool IsConditionless(IGeoBoundingBoxQuery q)
    {
      if (q.Field.IsConditionless())
        return true;
      return q.BoundingBox?.BottomRight == null && q.BoundingBox?.TopLeft == null && q.BoundingBox?.WellKnownText == null;
    }
  }
}
