...
// Type: Nest.UnfreezeIndexRequest
...
namespace Nest
{
  public class UnfreezeIndexRequest : 
    PlainRequestBase<UnfreezeIndexRequestParameters>,
    IUnfreezeIndexRequest,
    IRequest<UnfreezeIndexRequestParameters>,
    IRequest
  {
    protected IUnfreezeIndexRequest Self => (IUnfreezeIndexRequest) this;

    internal override ApiUrls ApiUrls => ApiUrlsLookups.IndicesUnfreeze;

    public UnfreezeIndexRequest(IndexName index)
      : base((Func<RouteValues, RouteValues>) (r => r.Required(nameof (index), (IUrlParameter) index)))
    {
    }

    [SerializationConstructor]
    protected UnfreezeIndexRequest()
    {
    }
