...
// Type: Nest.ResolvableDictionaryResponseFormatter`3
...
namespace Nest
{
  internal class ResolvableDictionaryResponseFormatter<TResponse, TKey, TValue> : 
    ...
    public TResponse Deserialize(ref JsonReader reader, IJsonFormatterResolver formatterResolver)
    {
      ...
      while (reader.ReadIsInObject(ref count))
      {
        ...
        else
        {
          ...
          backingDictionary.Add(key, obj);
        }
      }
      ResolvableDictionaryProxy<TKey, TValue> resolvableDictionaryProxy = new ResolvableDictionaryProxy<TKey, TValue>((IConnectionConfigurationValues) formatterResolver.GetConnectionSettings(), (IReadOnlyDictionary<TKey, TValue>) backingDictionary);
      response.BackingDictionary = (IReadOnlyDictionary<TKey, TValue>) resolvableDictionaryProxy;
      return response;
    }
