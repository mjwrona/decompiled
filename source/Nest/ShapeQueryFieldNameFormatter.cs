...
// Type: Nest.ShapeQueryFieldNameFormatter
...
namespace Nest
{
  internal class ShapeQueryFieldNameFormatter : IJsonFormatter<IShapeQuery>, IJsonFormatter
  {
    public IShapeQuery Deserialize(ref JsonReader reader, IJsonFormatterResolver formatterResolver) => throw new NotSupportedException();
      ...
      IJsonFormatterResolver formatterResolver)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          ShapeRelation? relation = value.Relation;
          if (relation.HasValue)
          {
            if (flag)
              writer.WriteValueSeparator();
            writer.WritePropertyName("relation");
            IJsonFormatter<ShapeRelation> formatter = formatterResolver.GetFormatter<ShapeRelation>();
            ref JsonWriter local = ref writer;
            relation = value.Relation;
            int num = (int) relation.Value;
            IJsonFormatterResolver formatterResolver1 = formatterResolver;
            formatter.Serialize(ref local, (ShapeRelation) num, formatterResolver1);
          }
