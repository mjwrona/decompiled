...
// Type: Microsoft.Azure.Pipelines.WebApi.RepositoryTypeJsonConverter`1
...
{
  public abstract class RepositoryTypeJsonConverter<T> : VssSecureJsonConverter where T : class
  {
    ...
    protected abstract T Create(RepositoryType type);
      ...
      JsonSerializer serializer)
    {
      ...
      if ((object) target == null)
      {
        ...
        if (closestMatchProperty == null || !jobject.TryGetValue(closestMatchProperty.PropertyName, StringComparison.OrdinalIgnoreCase, out jtoken))
          return existingValue;
        target = this.Create(UnknownEnum.Parse<RepositoryType>(jtoken.ToString()));
      }
      if ((object) target != null)
      {
