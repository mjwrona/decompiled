...
// Type: Microsoft.VisualStudio.Services.Search.WebServer.CollectionPackageCountHandler
...
{
  public class CollectionPackageCountHandler : AbstractSearchCountHandler
  {
    internal ISearchSecurityScopeFilterExpressionBuilder m_searchSecurityScopeFilterExpressionBuilder;

    public CollectionPackageCountHandler(ICountRequestForwarder forwarder)
      : base(forwarder)
    {
      ...
      this.EntityType = (IEntityType) PackageEntityType.GetInstance();
    ...
    public CollectionPackageCountHandler()
    {
      this.IndexMapper = (IIndexMapper) new Microsoft.VisualStudio.Services.Search.Query.IndexMapper((IEntityType) PackageEntityType.GetInstance());
      this.EntityType = (IEntityType) PackageEntityType.GetInstance();
      this.m_searchSecurityScopeFilterExpressionBuilder = (ISearchSecurityScopeFilterExpressionBuilder) new PackageSearchSecurityScopeFilterExpressionBuilder();
    ...
    protected override void InitializeForwarder(IVssRequestContext requestContext)
    {
      if (this.CountRequestForwarder == null)
        this.CountRequestForwarder = (ICountRequestForwarder) new CollectionPackageCountRequestForwarder(this.IndexMapper.GetESConnectionString(requestContext), requestContext.GetElasticsearchPlatformSettings("/Service/ALMSearch/Settings/ATSearchPlatformSettings"), requestContext.ExecutionEnvironment.IsOnPremisesDeployment);
    ...
    {
      return this.m_searchSecurityScopeFilterExpressionBuilder.GetScopeFilterExpression(requestContext, this.IsSecurityChecksEnabled, out noResultAccessible);
    }

    protected override void ValidateUserPermission(IVssRequestContext requestContext)
    {
    }
  }
}
