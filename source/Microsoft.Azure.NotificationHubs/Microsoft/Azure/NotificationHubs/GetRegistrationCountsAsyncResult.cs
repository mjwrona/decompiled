...
// Type: Microsoft.Azure.NotificationHubs.GetRegistrationCountsAsyncResult
...
{
  internal sealed class GetRegistrationCountsAsyncResult : 
    NotificationRequestAsyncResult<GetRegistrationCountsAsyncResult>
  {
    private readonly string tag;

    public GetRegistrationCountsAsyncResult(
    ...
    public RegistrationCounts Result { get; private set; }
    ...
    protected override void ProcessResponse()
    {
      try
      {
        ...
        using (XmlReader reader = XmlReader.Create(responseStream, new XmlReaderSettings()
        {
          ...
          this.Result = (RegistrationCounts) new DataContractSerializer(typeof (RegistrationCounts)).ReadObject(reader);
      }
    ...
    protected override void PrepareRequest()
    {
      ...
      uriBuilder.Path = string.Format((IFormatProvider) CultureInfo.InvariantCulture, "{0}{1}/registrations/counts", new object[2]
      {
        (object) uriBuilder.Path,
        (object) this.Manager.notificationHubPath
      });
      if (!string.IsNullOrWhiteSpace(this.tag))
        uriBuilder.Query = string.Format((IFormatProvider) CultureInfo.InvariantCulture, "{0}={1}&tags={2}", new object[3]
        {
          (object) "api-version",
          (object) "2016-07",
          (object) this.tag
