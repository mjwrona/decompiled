// Decompiled with JetBrains decompiler
// Type: Microsoft.Azure.NotificationHubs.Common.TransactionResult
// Assembly: Microsoft.Azure.NotificationHubs, Version=2.16.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35
// MVID: 1F43328A-44A2-48DE-9CBC-06F3C4A41C2A
// Assembly location: C:\Program Files\Azure DevOps Server 2022\Application Tier\Web Services\bin\Microsoft.Azure.NotificationHubs.dll

using Microsoft.Azure.NotificationHubs.Tracing;
using System;
using System.ServiceModel;

namespace Microsoft.Azure.NotificationHubs.Common
{
  internal sealed class TransactionResult : IExtensibleObject<TransactionResult>
  {
    public TransactionResult() => this.Extensions = (IExtensionCollection<TransactionResult>) new ExtensionCollection<TransactionResult>(this);

    public Exception CompletionException { get; set; }

    public int ReferenceCount { get; set; }

    public TrackingContext TrackingContext { get; set; }

    public IExtensionCollection<TransactionResult> Extensions { get; private set; }
  }
}
