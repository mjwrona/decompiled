// Decompiled with JetBrains decompiler
// Type: Microsoft.VisualStudio.Services.Feed.Common.ConcurrencyLimitingInvoker
// Assembly: Microsoft.VisualStudio.Services.Feed.Common, Version=19.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a
// MVID: AAC6BCA4-7F6C-4DFE-8058-1CCDD886477F
// Assembly location: C:\Program Files\Azure DevOps Server 2022\Application Tier\Web Services\bin\Microsoft.VisualStudio.Services.Feed.Common.dll

using Microsoft.VisualStudio.Services.Common;
using System;
using System.Threading;
using System.Threading.Tasks;

namespace Microsoft.VisualStudio.Services.Feed.Common
{
  public class ConcurrencyLimitingInvoker : IAsyncInvoker
  {
    public ConcurrencyLimitingInvoker(int maxConcurrency)
    {
      ArgumentUtility.CheckGreaterThanZero((float) maxConcurrency, nameof (maxConcurrency));
      this.Semaphore = new SemaphoreSlim(maxConcurrency);
    }

    private SemaphoreSlim Semaphore { get; }

    public async Task Invoke(Func<Task> toInvoke)
    {
      await this.Semaphore.WaitAsync();
      try
      {
        await toInvoke();
      }
      finally
      {
        this.Semaphore.Release();
      }
    }

    public async Task<T> Invoke<T>(Func<Task<T>> toInvoke)
    {
      await this.Semaphore.WaitAsync();
      T obj;
      try
      {
        obj = await toInvoke();
      }
      finally
      {
        this.Semaphore.Release();
      }
      return obj;
    }
  }
}
