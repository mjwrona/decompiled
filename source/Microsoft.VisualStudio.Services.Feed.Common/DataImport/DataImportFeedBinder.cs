// Decompiled with JetBrains decompiler
// Type: Microsoft.VisualStudio.Services.Feed.Common.DataImport.DataImportFeedBinder
// Assembly: Microsoft.VisualStudio.Services.Feed.Common, Version=19.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a
// MVID: AAC6BCA4-7F6C-4DFE-8058-1CCDD886477F
// Assembly location: C:\Program Files\Azure DevOps Server 2022\Application Tier\Web Services\bin\Microsoft.VisualStudio.Services.Feed.Common.dll

using Microsoft.TeamFoundation.Framework.Server;
using Microsoft.VisualStudio.Services.Feed.WebApi;
using System;
using System.Data;

namespace Microsoft.VisualStudio.Services.Feed.Common.DataImport
{
  internal class DataImportFeedBinder : ObjectBinder<FeedCore>
  {
    private SqlColumnBinder feedId = new SqlColumnBinder("FeedId");
    private SqlColumnBinder feedName = new SqlColumnBinder("FeedName");

    protected override FeedCore Bind() => DataImportFeedBinder.CreateElementalFeed(this.feedId.GetGuid((IDataReader) this.Reader), this.feedName.GetString((IDataReader) this.Reader, true));

    private static FeedCore CreateElementalFeed(Guid id, string name) => new FeedCore()
    {
      Id = id,
      Name = name
    };
  }
}
