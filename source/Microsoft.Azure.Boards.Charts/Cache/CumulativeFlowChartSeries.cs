// Decompiled with JetBrains decompiler
// Type: Microsoft.Azure.Boards.Charts.Cache.CumulativeFlowChartSeries
// Assembly: Microsoft.Azure.Boards.Charts, Version=19.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a
// MVID: EABADF19-3537-403E-8E3C-4185CE6D1F3B
// Assembly location: C:\Program Files\Azure DevOps Server 2022\Application Tier\Web Services\bin\Microsoft.Azure.Boards.Charts.dll

using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Runtime.Serialization;

namespace Microsoft.Azure.Boards.Charts.Cache
{
  [DataContract]
  [EditorBrowsable(EditorBrowsableState.Never)]
  public class CumulativeFlowChartSeries : ChartSeriesBase
  {
    public const int CurrentCacheVersion = 1;

    public CumulativeFlowChartSeries() => this.CacheVersion = 1;

    [DataMember(Name = "stateCounts", EmitDefaultValue = true)]
    public Dictionary<string, List<int>> StateTimeSeries { get; set; }

    [DataMember(Name = "filterState", EmitDefaultValue = false)]
    public string FilterState { get; set; }

    [DataMember(Name = "firstWorkDay", EmitDefaultValue = false)]
    public DayOfWeek FirstWorkDay { get; set; }
  }
}
