    ...
    <xsl:for-each select="planAndSuites">
      <div id="exported-data">
        ...
        <xsl:for-each select="testPlan">
          <div class="heading">
              Piano di test <xsl:call-template name="url-generator">
                  <xsl:with-param name="label">
                      Piano di test: <xsl:value-of select="@title"/> 
                  </xsl:with-param>
          ...
          <xsl:for-each select="configurations">
            ...
            <div class="sub-tab">
              CONFIGURAZIONI NEL PIANO DI TEST
            </div>
            <table class="table-style">
              <tr>
                <th width="5%">ID</th>
                <th width="45%">Nome</th>
                <th width="50%">Variabili di configurazione</th>
              </tr>
            ...
            <xsl:for-each select="additionalConfiguration">
              <div class="sub-tab">
                CONFIGURAZIONI AGGIUNTIVE A CUI FANNO RIFERIMENTO GRUPPI FIGLIO
              </div>
              <table class="table-style">
                <tr>
                  <th width="5%">ID</th>
        ...
        <xsl:for-each select="testSuites">
          <xsl:for-each select="testSuite">
            ...
            <div class="heading">
              Gruppo di test <xsl:call-template name="url-generator">
                <xsl:with-param name="label">
                    Gruppo di test: <xsl:value-of select="@title"/>
                </xsl:with-param>
              </xsl:call-template>: <xsl:value-of select="@title"/>
            </div>
                    ...
                    <tr>
                      ...
                      <td>
                          <xsl:call-template name="url-generator">
                              <xsl:with-param name="label">
                                  Requisito: <xsl:value-of select="@title"/>
                              </xsl:with-param>
                          </xsl:call-template>: <xsl:value-of select="@title"/>
                      </td>
                    </tr>
            ...
            <xsl:for-each select="testCases">
              ...
              <div class="tab">
                Test case (<xsl:value-of select="@count"/>)
                <hr/>
              </div>

              <xsl:for-each select="testCase">
                <div class="test-case-heading heading">
                  Test case <xsl:call-template name="url-generator">
                    <xsl:with-param name="label">
                        Test case: <xsl:value-of select="@title"/>
                       </xsl:with-param>
                    </xsl:call-template>: <xsl:value-of select="@title"/>
                </div>
                ...
                <xsl:comment>Riepilogo</xsl:comment>
                ...
                <xsl:comment>Passi del test</xsl:comment>
                ...
                <xsl:comment>Parametri</xsl:comment>
                ...
                <xsl:for-each select="parameters">
                  ...
                  <xsl:for-each select="sharedParameterDataSet">
                    <div class="sub-tab">
                      Parametro condiviso <xsl:call-template name="url-generator">
                          <xsl:with-param name="label">
                            Parametro condiviso: <xsl:value-of select="@title"/>
                          </xsl:with-param>
                ...
                <xsl:comment>Collegamenti e allegati</xsl:comment>
                ...
                <xsl:comment>Automazione</xsl:comment>

                <xsl:for-each select="automation">
                  <xsl:if test="*">
                    <div class="sub-tab">
                      AUTOMAZIONE ASSOCIATA
                    </div>
                    <xsl:apply-templates select="properties"/>
                  </xsl:if>
                </xsl:for-each>
                <xsl:comment>Risultato del test più recente</xsl:comment>
                <xsl:for-each select="latestTestOutcomes">
                  <div class="avoid-page-break">
                    <div class="sub-tab">
                      RISULTATO DEL TEST PIÙ RECENTE<br/>
                    </div>
                  </div>
