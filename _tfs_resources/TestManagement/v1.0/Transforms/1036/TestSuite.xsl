    ...
    <xsl:for-each select="planAndSuites">
      <div id="exported-data">
        ...
        <xsl:for-each select="testPlan">
          <div class="heading">
              Plan de test <xsl:call-template name="url-generator">
                  <xsl:with-param name="label">
                      Plan de test : <xsl:value-of select="@title"/> 
                  </xsl:with-param>
          ...
          <xsl:for-each select="configurations">
            ...
            <div class="sub-tab">
              CONFIGURATIONS DANS LE PLAN DE TEST
            </div>
            <table class="table-style">
              <tr>
                <th width="5%">ID</th>
                <th width="45%">Nom</th>
                <th width="50%">Variables de configuration</th>
              </tr>
            ...
            <xsl:for-each select="additionalConfiguration">
              <div class="sub-tab">
                CONFIGURATIONS SUPPLÉMENTAIRES RÉFÉRENCÉES PAR DES SUITES ENFANTS
              </div>
              <table class="table-style">
                <tr>
                  <th width="5%">ID</th>
        ...
        <xsl:for-each select="testSuites">
          <xsl:for-each select="testSuite">
            ...
            <div class="heading">
              Suite de tests <xsl:call-template name="url-generator">
                <xsl:with-param name="label">
                    Suite de tests : <xsl:value-of select="@title"/>
                </xsl:with-param>
              </xsl:call-template>: <xsl:value-of select="@title"/>
            </div>
                    ...
                    <tr>
                      ...
                      <td>
                          <xsl:call-template name="url-generator">
                              <xsl:with-param name="label">
                                  Exigence : <xsl:value-of select="@title"/>
                              </xsl:with-param>
                          </xsl:call-template>: <xsl:value-of select="@title"/>
                      </td>
                    </tr>
            ...
            <xsl:for-each select="testCases">
              ...
              <div class="tab">
                Cas de test (<xsl:value-of select="@count"/>)
                <hr/>
              </div>

              <xsl:for-each select="testCase">
                <div class="test-case-heading heading">
                  Cas de test <xsl:call-template name="url-generator">
                    <xsl:with-param name="label">
                        Cas de test : <xsl:value-of select="@title"/>
                       </xsl:with-param>
                    </xsl:call-template>: <xsl:value-of select="@title"/>
                </div>
                ...
                <xsl:comment>Résumé</xsl:comment>
                ...
                <xsl:comment>Paramètres</xsl:comment>
                ...
                <xsl:for-each select="parameters">
                  ...
                  <xsl:for-each select="sharedParameterDataSet">
                    <div class="sub-tab">
                      Paramètre partagé <xsl:call-template name="url-generator">
                          <xsl:with-param name="label">
                            Paramètre partagé : <xsl:value-of select="@title"/>
                          </xsl:with-param>
                ...
                <xsl:comment>Dernier résultat de test</xsl:comment>
                <xsl:for-each select="latestTestOutcomes">
                  <div class="avoid-page-break">
                    <div class="sub-tab">
                      DERNIER RÉSULTAT DE TEST<br/>
                    </div>
                  </div>
