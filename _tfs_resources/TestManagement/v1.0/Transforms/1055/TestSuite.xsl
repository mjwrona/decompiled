    ...
    <xsl:for-each select="planAndSuites">
      <div id="exported-data">
        ...
        <xsl:for-each select="testPlan">
          <div class="heading">
              Test planı <xsl:call-template name="url-generator">
                  <xsl:with-param name="label">
                      Test planı: <xsl:value-of select="@title"/> 
                  </xsl:with-param>
          ...
          <xsl:for-each select="configurations">
            ...
            <div class="sub-tab">
              TEST PLANINDAKİ YAPILANDIRMALAR
            </div>
            <table class="table-style">
              <tr>
                <th width="5%">Kimlik</th>
                <th width="45%">Ad</th>
                <th width="50%">Yapılandırma değişkenleri</th>
              </tr>
            ...
            <xsl:for-each select="additionalConfiguration">
              <div class="sub-tab">
                ALT PAKETLER TARAFINDAN BAŞVURULAN EK YAPILANDIRMALAR
              </div>
              <table class="table-style">
                <tr>
                  <th width="5%">Kimlik</th>
        ...
        <xsl:for-each select="testSuites">
          <xsl:for-each select="testSuite">
            ...
            <div class="heading">
              Test paketi <xsl:call-template name="url-generator">
                <xsl:with-param name="label">
                    Test paketi: <xsl:value-of select="@title"/>
                </xsl:with-param>
              </xsl:call-template>: <xsl:value-of select="@title"/>
            </div>
                    ...
                    <tr>
                      ...
                      <td>
                          <xsl:call-template name="url-generator">
                              <xsl:with-param name="label">
                                  Gereksinim: <xsl:value-of select="@title"/>
                              </xsl:with-param>
                          </xsl:call-template>: <xsl:value-of select="@title"/>
                      </td>
                    </tr>
            ...
            <xsl:for-each select="testCases">
              ...
              <div class="tab">
                Test çalışması (<xsl:value-of select="@count"/>)
                <hr/>
              </div>

              <xsl:for-each select="testCase">
                <div class="test-case-heading heading">
                  Test çalışması <xsl:call-template name="url-generator">
                    <xsl:with-param name="label">
                        Test çalışması: <xsl:value-of select="@title"/>
                       </xsl:with-param>
                    </xsl:call-template>: <xsl:value-of select="@title"/>
                </div>
                ...
                <xsl:comment>Parametreler</xsl:comment>
                ...
                <xsl:for-each select="parameters">
                  ...
                  <xsl:for-each select="sharedParameterDataSet">
                    <div class="sub-tab">
                      Paylaşılan parametre <xsl:call-template name="url-generator">
                          <xsl:with-param name="label">
                            Paylaşılan parametre : <xsl:value-of select="@title"/>
                          </xsl:with-param>
                ...
                <xsl:comment>Otomasyon</xsl:comment>
                ...
                <xsl:comment>En Son Test Sonucu</xsl:comment>
                <xsl:for-each select="latestTestOutcomes">
                  <div class="avoid-page-break">
                    <div class="sub-tab">
                      EN SON TEST SONUCU<br/>
                    </div>
                  </div>
  ...
  <xsl:template name="suiteHierarchy">
    ...
    <xsl:for-each select="suite">
      ...
      <xsl:value-of select="@type"/>: <span>
        <xsl:value-of select="@title"/> (Kimlik: 
          <xsl:call-template name="url-generator">
            <xsl:with-param name="label">
