...
<!-- Common strings -->
<xsl:variable name="morePrompt">Daha ...</xsl:variable>
<xsl:variable name="FileType">Dosya</xsl:variable>
<xsl:variable name="FolderType">Klasör</xsl:variable>
<xsl:variable name="datetime">- Tüm tarihler ve saatler UTC olarak gösterilir</xsl:variable>
...
<xsl:variable name="by">Sağlayan </xsl:variable>
<xsl:variable name="subscriberText"> oluşturan: </xsl:variable>
<xsl:variable name="subscriptionIdText"> alıyorsunuz </xsl:variable>
<xsl:variable name="Error">İsteğiniz işlenirken bir hata oluştu. Bu geçici bir hata olabilir ve isteği yeniden denemek yardımcı olabilir. Bu hata devam ederse veya ek dikkat gerektirdiğini düşünüyorsanız, lütfen bu iletiyi ve görünen hata iletilerini yönetici personelinize sağlayın</xsl:variable>
<xsl:variable name="DetailedErrorHeader">Ayrıntılı hata iletileri (yönetim personeli için):</xsl:variable>

  <!-- Item information webview title -->
<xsl:variable name="ChangesetViewTitle">Değişiklik Kümesi Bilgisi</xsl:variable>
<xsl:variable name="ShelvesetViewTitle">Raf Kümesi Bilgisi</xsl:variable>
<xsl:variable name="CheckinActionResolve">Çöz</xsl:variable>
<xsl:variable name="CheckinActionAssociate">İlişkilendir</xsl:variable>
  
<xsl:variable name="alerts-unsubscribe">Aboneliği kaldır</xsl:variable>
<xsl:variable name="alerts-view">Görünüm</xsl:variable>

<!-- Matcher types -->
<xsl:variable name="matcher-follows">FollowsMatcher</xsl:variable>

<xsl:variable name="follows-reason">- Bunu takip ettiğiniz için size bu bildirim gönderildi </xsl:variable>
...
<xsl:choose>
    ...
    <table>
        <tr>
            <td class="PropName">Sunucu Yolu:</td>
            <td class="PropValue"><xsl:value-of select="Item/@item"/></td>
        </tr>
        <tr>
            <td class="PropName">Değişiklik Kümesi:</td>
            <td class="PropValue">
        ...
        <tr>
            <td class="PropName">Üzerindeki Son Değişiklik:</td>
            <td class="PropValue"><xsl:value-of select="Item/@date"/></td>
        </tr>
        <tr>
            <td class="PropName">Tür:</td>
            <td class="PropValue">
        ...
        <xsl:if test="Item/@type = 'File'">
        <tr>
            <td class="PropName">Dosya Boyutu (bayt):</td>
...
<xsl:text>
</xsl:text><xsl:text>Özet</xsl:text>
<xsl:text>
</xsl:text><xsl:text>Sunucu Yolu:</xsl:text>       <xsl:value-of select="Item/@item"/>
<xsl:text>
</xsl:text><xsl:text>Değişiklik Kümesi:</xsl:text>         <xsl:if test="string-length(Item/@csurl) &gt; 0">
<xsl:call-template name="link">
...
<xsl:value-of select="Item/@cs"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>Üzerindeki Son Değişiklik:</xsl:text>  <xsl:value-of select="Item/@date"/>
<xsl:if test="Item/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>Tür:</xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
<xsl:if test="Item/@type = 'Folder'">
  <xsl:text>
</xsl:text><xsl:text>Tür:</xsl:text>              <xsl:value-of select="$FolderType"/>
</xsl:if>
<xsl:if test="@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>Dosya Boyutu (bayt):</xsl:text> <xsl:value-of select="Item/@len"/>
</xsl:if>
</xsl:when>
</xsl:choose>
...
<xsl:choose>
    ...
    <table>
        ...
        <tr>
            <td class="PropName">Oluşturma Tarihi:</td>
...
<xsl:text>
</xsl:text><xsl:text>Sunucu Yolu:</xsl:text>       <xsl:value-of select="PendingChange/@item"/>
<xsl:if test="string-length(PendingChange/@srcitem) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>Kaynak Sunucu Yolu:</xsl:text>       <xsl:value-of select="PendingChange/@srcitem"/>
</xsl:if>
<xsl:if test="string-length(PendingChange/@ssurl) &gt;0">
<xsl:text>
</xsl:text><xsl:text>Raf Kümesi:</xsl:text>
<xsl:call-template name="link">
<xsl:with-param name="format" select="'text'"/>
<xsl:with-param name="link" select="PendingChange/@ssurl"/>
<xsl:with-param name="addTracking" select="'true'"/>
<xsl:with-param name="displayText" select="PendingChange/@ss"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>Oluşturma Tarihi:</xsl:text>  <xsl:value-of select="PendingChange/@date"/>
<xsl:if test="PendingChange/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>Tür:</xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
...
<xsl:if test="string-length(PendingChange/@ssurl) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>Değiştir:</xsl:text>              <xsl:value-of select="PendingChange/@chg"/>
</xsl:if>
</xsl:when>
</xsl:choose>
<xsl:call-template name="footer">
...
<tr>
<td class="PropName">Takım Projesi:</td>
<td class="PropValue">
...
<xsl:choose>
    <xsl:when test="Owner != Committer">
        <td class="PropName">Tarafına iade edilen:</td>
    </xsl:when>
    <xsl:when test="Owner = Committer">
        <td class="PropName">İade eden:</td>
    </xsl:when>
</xsl:choose>
<td class="PropValue">
...
<tr>
<td class="PropName">Üzerinde iade edildi:</td>
<td class="PropValue">
...
<tr>
  ...
  <td class="PropValue">
    ...
    <xsl:if test="$valueLength = 0"><pre>Yok</pre></xsl:if>
  </td>
</tr>
</xsl:for-each>
<tr>
<td class="PropName">Açıklama:</td>
<td class="PropValue">
  ...
  <xsl:if test="$commentLength = 0"><pre>Yok</pre></xsl:if>
</td>
  </tr>
<!-- Optional policy override comment -->
<xsl:if test="string-length(PolicyOverrideComment) &gt; 0">
 <tr>
 <td class="PropName">İlkeyi Geçersiz Kılma Nedeni:</td>
<td class="PropValue">
...
<br/>
<b>Çözümlenen İş Öğeleri</b>
<table class="WithBorder">
<tr>
  <td class="ColHeadingSmall">Tür</td>
  <td class="ColHeadingXSmall">Kimlik</td>
  <td class="ColHeading">Başlık</td>
  <td class="ColHeadingSmall">Durum</td>
  <td class="ColHeadingSmall">Atanan</td>
</tr>
...
<table class="WithBorder">
<tr>
  <td class="ColHeading">Tür</td>
  <td class="ColHeading">Açıklama</td>
</tr>
...
<table class="WithBorder">
<tr>
<td class="ColHeading">Ad</td>
<td class="ColHeadingSmall">Değiştir</td>
<td class="ColHeading">Klasör</td>
</tr>
...
<tr>
  ...
  <td class="PropValue">
    <xsl:variable name="valueLength" select="string-length(@val)"/>
    <xsl:if test="$valueLength &gt; 0"><pre><xsl:value-of select="@val"/></pre></xsl:if>
    <xsl:if test="$valueLength = 0"><pre>Hiçbiri</pre></xsl:if>
  </td>
</tr>
</xsl:for-each>
<tr>
...
<tr>
  ...
  <td class="ColHeadingXSmall">Kimlik</td>
  <td class="ColHeading">Başlık</td>
  <td class="ColHeadingSmall">Eylem</td>
  <td class="ColHeadingSmall">Atanan</td>
</tr>
...
<br/>
<b>Rafa Kaldırılan Değişiklikler</b>
<table class="WithBorder">
<tr>
<td class="ColHeading">Ad</td>
<td class="ColHeadingSmall">Değiştir</td>
...
<xsl:template name="wiItem">
    ...
    <td class="ColDataSmall">
      ...
      <xsl:if test="$assignedToLength = 0">Yok</xsl:if>
    </td>
</xsl:template> <!-- wiItem -->
