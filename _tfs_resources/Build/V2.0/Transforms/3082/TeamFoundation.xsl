...
<!-- Common strings -->
<xsl:variable name="morePrompt">Más ...</xsl:variable>
<xsl:variable name="FileType">Archivo</xsl:variable>
<xsl:variable name="FolderType">Carpeta</xsl:variable>
<xsl:variable name="datetime">- Todas las fechas y horas se muestran en UTC</xsl:variable>
...
<xsl:variable name="by">Proporcionado por </xsl:variable>
<xsl:variable name="subscriberText"> creado por </xsl:variable>
<xsl:variable name="subscriptionIdText"> con identificador </xsl:variable>
<xsl:variable name="Error">Error mientras se procesaba la solicitud. Quizás sea un error transitorio que se resuelva volviendo a realizar la solicitud. Si el error persiste o cree que precisa mayor atención, proporcione este mensaje de error y todos los que aparezcan al personal administrativo</xsl:variable>
<xsl:variable name="DetailedErrorHeader">Mensajes de error detallados (para el personal administrativo):</xsl:variable>

  <!-- Item information webview title -->
<xsl:variable name="ChangesetViewTitle">Información del conjunto de cambios</xsl:variable>
<xsl:variable name="ShelvesetViewTitle">Información del conjunto de cambios agregados pendientes de confirmación</xsl:variable>
<xsl:variable name="CheckinActionResolve">Resolver</xsl:variable>
<xsl:variable name="CheckinActionAssociate">Asociar</xsl:variable>
  
<xsl:variable name="alerts-unsubscribe">Eliminar suscripción</xsl:variable>
<xsl:variable name="alerts-view">Ver</xsl:variable>

<!-- Matcher types -->
<xsl:variable name="matcher-follows">FollowsMatcher</xsl:variable>

<xsl:variable name="follows-reason">- Le enviamos esta notificación porque está siguiendo este </xsl:variable>
...
<xsl:choose>
    ...
    <table>
        <tr>
            <td class="PropName">Ruta de acceso del servidor:</td>
            <td class="PropValue"><xsl:value-of select="Item/@item"/></td>
        </tr>
        <tr>
            <td class="PropName">Conjunto de cambios:</td>
            <td class="PropValue">
        ...
        <tr>
            <td class="PropName">Última modificación:</td>
            <td class="PropValue"><xsl:value-of select="Item/@date"/></td>
        </tr>
        <tr>
            <td class="PropName">Tipo:</td>
            <td class="PropValue">
        ...
        <xsl:if test="Item/@type = 'File'">
        <tr>
            <td class="PropName">Tamaño del archivo (bytes):</td>
...
<xsl:text>
</xsl:text><xsl:text>Resumen</xsl:text>
<xsl:text>
</xsl:text><xsl:text>Ruta de acceso del servidor:</xsl:text>       <xsl:value-of select="Item/@item"/>
<xsl:text>
</xsl:text><xsl:text>Conjunto de cambios:</xsl:text>         <xsl:if test="string-length(Item/@csurl) &gt; 0">
<xsl:call-template name="link">
...
<xsl:value-of select="Item/@cs"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>Última modificación:</xsl:text>  <xsl:value-of select="Item/@date"/>
<xsl:if test="Item/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>Tipo:</xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
<xsl:if test="Item/@type = 'Folder'">
  <xsl:text>
</xsl:text><xsl:text>Tipo:</xsl:text>              <xsl:value-of select="$FolderType"/>
</xsl:if>
<xsl:if test="@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>Tamaño del archivo (bytes):</xsl:text> <xsl:value-of select="Item/@len"/>
</xsl:if>
</xsl:when>
</xsl:choose>
...
<xsl:choose>
    ...
    <table>
        ...
        <xsl:if test="string-length(PendingChange/@srcitem) &gt; 0">
            <tr>
                <td class="PropName">Ruta de acceso del servidor de origen:</td>
                <td class="PropValue"><xsl:value-of select="PendingChange/@srcitem"/></td>
            </tr>
        </xsl:if>
        <xsl:if test="string-length(PendingChange/@ssurl) &gt; 0">
            <tr>
                <td class="PropName">Conjunto de cambios agregados pendientes de confirmación:</td>
                <td class="PropValue">
                <xsl:call-template name="link">
                    <xsl:with-param name="format" select="'html'"/>
        ...
        <tr>
            <td class="PropName">Fecha de creación:</td>
...
<xsl:text>
</xsl:text><xsl:text>Ruta de acceso del servidor:</xsl:text>       <xsl:value-of select="PendingChange/@item"/>
<xsl:if test="string-length(PendingChange/@srcitem) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>Ruta de acceso del servidor de origen:</xsl:text>       <xsl:value-of select="PendingChange/@srcitem"/>
</xsl:if>
<xsl:if test="string-length(PendingChange/@ssurl) &gt;0">
<xsl:text>
</xsl:text><xsl:text>Conjunto de cambios agregados pendientes de confirmación:</xsl:text>
<xsl:call-template name="link">
<xsl:with-param name="format" select="'text'"/>
<xsl:with-param name="link" select="PendingChange/@ssurl"/>
<xsl:with-param name="addTracking" select="'true'"/>
<xsl:with-param name="displayText" select="PendingChange/@ss"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>Fecha de creación:</xsl:text>  <xsl:value-of select="PendingChange/@date"/>
<xsl:if test="PendingChange/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>Tipo:</xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
...
<xsl:if test="string-length(PendingChange/@ssurl) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>Cambio:</xsl:text>              <xsl:value-of select="PendingChange/@chg"/>
</xsl:if>
</xsl:when>
</xsl:choose>
<xsl:call-template name="footer">
...
<tr>
<td class="PropName">Proyectos de equipo:</td>
<td class="PropValue">
...
<xsl:choose>
    <xsl:when test="Owner != Committer">
        <td class="PropName">Protegido en nombre de:</td>
    </xsl:when>
    <xsl:when test="Owner = Committer">
        <td class="PropName">Protegido por:</td>
    </xsl:when>
</xsl:choose>
<td class="PropValue">
...
<tr>
<td class="PropName">Protegido el:</td>
<td class="PropValue">
...
<tr>
<td class="PropName">Comentario:</td>
<td class="PropValue">
...
<xsl:if test="string-length(PolicyOverrideComment) &gt; 0">
 <tr>
 <td class="PropName">Motivo de invalidación de la directiva:</td>
<td class="PropValue">
...
<br/>
<b>Elementos de trabajo resueltos</b>
<table class="WithBorder">
<tr>
  <td class="ColHeadingSmall">Tipo</td>
  <td class="ColHeadingXSmall">Id.</td>
  <td class="ColHeading">Título</td>
  <td class="ColHeadingSmall">Estado</td>
  <td class="ColHeadingSmall">Asignado a</td>
</tr>
...
<br/>
<b>Elementos de trabajo asociados</b>
<table class="WithBorder">
<tr>
  <td class="ColHeadingSmall">Tipo</td>
  <td class="ColHeadingXSmall">Id.</td>
...
<td class="ColHeadingSmall">Cambiar</td>
<td class="ColHeading">Carpeta</td>
</tr>
...
<tr>
<td class="PropName">Propietario:</td>
<td class="PropValue">
    <xsl:variable name="owner" select="OwnerDisplay"/>
    <xsl:if test="$owner=''">
        <xsl:value-of select="OwnerDisplay"/>
...
<br/>
<b>Elementos de trabajo</b>
<table class="WithBorder">
<tr>
  <td class="ColHeadingSmall">Tipo</td>
  <td class="ColHeadingXSmall">Id.</td>
  <td class="ColHeading">Título</td>
  <td class="ColHeadingSmall">Acción</td>
  <td class="ColHeadingSmall">Asignado a</td>
</tr>
...
<br/>
<b>Cambios aplazados</b>
<table class="WithBorder">
<tr>
<td class="ColHeading">Nombre</td>
<td class="ColHeadingSmall">Cambio</td>
<td class="ColHeading">Carpeta</td>
</tr>
...
<xsl:template name="wiItem">
    ...
    <td class="ColDataSmall">
      ...
      <xsl:if test="$assignedToLength = 0">N/D</xsl:if>
    </td>
</xsl:template> <!-- wiItem -->
