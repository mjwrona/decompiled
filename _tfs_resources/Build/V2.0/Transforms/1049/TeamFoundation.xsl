...
<!-- Common strings -->
<xsl:variable name="morePrompt">Дополнительно ...</xsl:variable>
<xsl:variable name="FileType">Файл</xsl:variable>
<xsl:variable name="FolderType">Папка</xsl:variable>
<xsl:variable name="datetime">- Все значения даты и времени указаны в формате UTC</xsl:variable>
...
<xsl:variable name="by">Кем предоставлено </xsl:variable>
<xsl:variable name="subscriberText"> созданной </xsl:variable>
<xsl:variable name="subscriptionIdText"> с идентификатором </xsl:variable>
<xsl:variable name="Error">Произошла ошибка в ходе обработки запроса. Это может быть временная ошибка. Попробуйте повторить запрос. Если эта ошибка повторяется, или если она является особенно важной, передайте это сообщение и отображающееся сообщение(сообщения) об ошибке административному персоналу</xsl:variable>
<xsl:variable name="DetailedErrorHeader">Подробные сообщения об ошибках (для административного персонала):</xsl:variable>

  <!-- Item information webview title -->
<xsl:variable name="ChangesetViewTitle">Сведения о наборе изменений</xsl:variable>
<xsl:variable name="ShelvesetViewTitle">Сведения о наборе отложенных изменений</xsl:variable>
<xsl:variable name="CheckinActionResolve">Обработать</xsl:variable>
<xsl:variable name="CheckinActionAssociate">Ассоциация</xsl:variable>
  
<xsl:variable name="alerts-unsubscribe">Отменить подписку</xsl:variable>
<xsl:variable name="alerts-view">Представление</xsl:variable>

<!-- Matcher types -->
<xsl:variable name="matcher-follows">FollowsMatcher</xsl:variable>

<xsl:variable name="follows-reason">— Мы отправили это уведомление, так как вы отслеживаете </xsl:variable>
...
<xsl:choose>
    ...
    <table>
        <tr>
            <td class="PropName">Путь к серверу:</td>
            <td class="PropValue"><xsl:value-of select="Item/@item"/></td>
        </tr>
        <tr>
            <td class="PropName">Набор изменений:</td>
            <td class="PropValue">
        ...
        <tr>
            <td class="PropName">Последнее изменение:</td>
            <td class="PropValue"><xsl:value-of select="Item/@date"/></td>
        </tr>
        <tr>
            <td class="PropName">Тип:</td>
            <td class="PropValue">
        ...
        <xsl:if test="Item/@type = 'File'">
        <tr>
            <td class="PropName">Размер файла (байт):</td>
...
<xsl:text>
</xsl:text><xsl:text>Сводка</xsl:text>
<xsl:text>
</xsl:text><xsl:text>Путь к серверу:</xsl:text>       <xsl:value-of select="Item/@item"/>
<xsl:text>
</xsl:text><xsl:text>Набор изменений:</xsl:text>         <xsl:if test="string-length(Item/@csurl) &gt; 0">
<xsl:call-template name="link">
...
<xsl:value-of select="Item/@cs"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>Последнее изменение:</xsl:text>  <xsl:value-of select="Item/@date"/>
<xsl:if test="Item/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>Тип:</xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
<xsl:if test="Item/@type = 'Folder'">
  <xsl:text>
</xsl:text><xsl:text>Тип:</xsl:text>              <xsl:value-of select="$FolderType"/>
</xsl:if>
<xsl:if test="@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>Размер файла (байт):</xsl:text> <xsl:value-of select="Item/@len"/>
</xsl:if>
</xsl:when>
</xsl:choose>
...
<xsl:choose>
    ...
    <table>
        ...
        <tr>
            <td class="PropName">Дата создания:</td>
        ...
        <xsl:if test="string-length(PendingChange/@ssurl) &gt; 0"><!-- @chg is only localized in webView -->
            <tr>
                <td class="PropName">Изменение:</td>
...
<xsl:text>
</xsl:text><xsl:text>Путь к серверу:</xsl:text>       <xsl:value-of select="PendingChange/@item"/>
<xsl:if test="string-length(PendingChange/@srcitem) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>Исходный путь на сервере:</xsl:text>       <xsl:value-of select="PendingChange/@srcitem"/>
</xsl:if>
<xsl:if test="string-length(PendingChange/@ssurl) &gt;0">
<xsl:text>
</xsl:text><xsl:text>Набор отложенных изменений:</xsl:text>
<xsl:call-template name="link">
<xsl:with-param name="format" select="'text'"/>
<xsl:with-param name="link" select="PendingChange/@ssurl"/>
<xsl:with-param name="addTracking" select="'true'"/>
<xsl:with-param name="displayText" select="PendingChange/@ss"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>Дата создания:</xsl:text>  <xsl:value-of select="PendingChange/@date"/>
<xsl:if test="PendingChange/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>Тип:</xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
...
<xsl:if test="string-length(PendingChange/@ssurl) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>Изменение:</xsl:text>              <xsl:value-of select="PendingChange/@chg"/>
</xsl:if>
</xsl:when>
</xsl:choose>
<xsl:call-template name="footer">
...
<tr>
<td class="PropName">Командные проекты:</td>
<td class="PropValue">
...
<xsl:choose>
    <xsl:when test="Owner != Committer">
        <td class="PropName">Возвращено от имени:</td>
    </xsl:when>
    <xsl:when test="Owner = Committer">
        <td class="PropName">Кем возвращено:</td>
    </xsl:when>
</xsl:choose>
<td class="PropValue">
...
<tr>
<td class="PropName">Когда возвращено:</td>
<td class="PropValue">
...
<tr>
  ...
  <td class="PropValue">
    ...
    <xsl:if test="$valueLength = 0"><pre>Нет</pre></xsl:if>
  </td>
</tr>
</xsl:for-each>
<tr>
<td class="PropName">Примечание:</td>
<td class="PropValue">
  ...
  <xsl:if test="$commentLength = 0"><pre>Нет</pre></xsl:if>
</td>
  </tr>
<!-- Optional policy override comment -->
<xsl:if test="string-length(PolicyOverrideComment) &gt; 0">
 <tr>
 <td class="PropName">Причина переопределения политики:</td>
<td class="PropValue">
...
<br/>
<b>Разрешенные рабочие элементы</b>
<table class="WithBorder">
<tr>
  <td class="ColHeadingSmall">Тип</td>
  <td class="ColHeadingXSmall">Идентификатор</td>
  <td class="ColHeading">Название</td>
  <td class="ColHeadingSmall">Состояние</td>
  <td class="ColHeadingSmall">Кому назначено</td>
</tr>
...
<br/>
<b>Связанные рабочие элементы</b>
<table class="WithBorder">
<tr>
  <td class="ColHeadingSmall">Тип</td>
  <td class="ColHeadingXSmall">Идентификатор</td>
...
<table class="WithBorder">
<tr>
  <td class="ColHeading">Тип</td>
  <td class="ColHeading">Описание</td>
</tr>
...
<table class="WithBorder">
<tr>
<td class="ColHeading">Имя</td>
<td class="ColHeadingSmall">Изменение</td>
<td class="ColHeading">Папка</td>
</tr>
...
<br/>
<b>Рабочие элементы</b>
<table class="WithBorder">
<tr>
  <td class="ColHeadingSmall">Тип</td>
  <td class="ColHeadingXSmall">Идентификатор</td>
  <td class="ColHeading">Название</td>
  <td class="ColHeadingSmall">Действие</td>
  <td class="ColHeadingSmall">Кому назначено</td>
</tr>
...
<br/>
<b>Изменения, включенные в набор отложенных изменений</b>
<table class="WithBorder">
<tr>
<td class="ColHeading">Имя</td>
<td class="ColHeadingSmall">Изменение</td>
...
<xsl:template name="wiItem">
    ...
    <td class="ColDataSmall">
      ...
      <xsl:if test="$assignedToLength = 0">Н/Д</xsl:if>
    </td>
</xsl:template> <!-- wiItem -->
