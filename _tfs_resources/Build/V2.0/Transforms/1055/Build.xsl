...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="/Report">
    <html>
      <head>
        ...
        <title>Derleme Ayrıntıları</title>
        <style type="text/css">
          .title {
          }
          .title td {
        ...
        <script type="text/javascript" language="javascript">
            ...
            location.href=elem.innerHTML;
              ...
              alert("Geçerli bölgeden dosya URI'si açılamıyor.");
            }
            return false;
          }
        </script>
  ...
  <xsl:template match="BuildDetail">
    ...
    <div class="buildNumber">
        Derleme <xsl:value-of select="@BuildNumber"/>
    </div>
    ...
    <table class="heading" cellspacing="0" cellpadding="0" width="100%">
      ...
      <tr>
        <td>Son değiştiren:</td>
        <td>
          <xsl:value-of select="@LastChangedBy"/>
        </td>
      </tr>
      <tr>
        <td>Üzerindeki son değişme:</td>
        <td>
      ...
      <tr>
        <td>Kaynak denetimi sürümü:</td>
        <td>
          <xsl:value-of select="@SourceGetVersion"/>
        </td>
      </tr>
      <tr>
        <td>Raf kümesi adı:</td>
        <td>
  ...
  <xsl:template match="BuildInformationNode[@Type='AgentScopeActivityTracking']" name="vstfbuild_agentScopeActivityTrackingNode">
    <div class="information">
      ...
      <div class="property">
        <span>
            Rezervasyon Durumu: <xsl:value-of select="InformationField[Name='ReservationStatus']/Value"/>
        </span>
      </div>
      <div class="property">
        <span>
            Ayrılmış Aracı: <xsl:value-of select="InformationField[Name='ReservedAgentName']/Value"/> (<xsl:value-of select="InformationField[Name='ReservedAgentUri']/Value"/>)
        </span>
      </div>
  ...
  <xsl:template match="BuildInformationNode[@Type='BuildError']" name="vstfbuild_errorNode">
    <div class="information">
      ...
      <b>Hata</b>: <xsl:value-of select="InformationField[Name='File']/Value"/>(<xsl:value-of select="InformationField[Name='LineNumber']/Value"/>): <xsl:value-of select="InformationField[Name='Code']/Value"/>: <xsl:value-of select="InformationField[Name='Message']/Value"/>
      <xsl:apply-templates select="BuildInformationNode[@Type!='ActivityProperties']"/>
    </div>
  </xsl:template>
  ...
  <xsl:template match="BuildInformationNode[@Type='BuildWarning']" name="vstfbuild_warningNode">
    <div class="information">
      ...
      <b>Uyarı</b>: <xsl:value-of select="InformationField[Name='File']/Value"/>(<xsl:value-of select="InformationField[Name='LineNumber']/Value"/>): <xsl:value-of select="InformationField[Name='Code']/Value"/>: <xsl:value-of select="InformationField[Name='Message']/Value"/>
      <xsl:apply-templates select="BuildInformationNode[@Type!='ActivityProperties']"/>
    </div>
  </xsl:template>
      ...
      <span>
        <xsl:choose>
          <xsl:when test="string-length(InformationField[Name='ServerPath']/Value) &gt; 0">
            Derlendi <xsl:value-of select="InformationField[Name='ServerPath']/Value"/>
          </xsl:when>
          <xsl:otherwise>
            Derlendi <xsl:value-of select="InformationField[Name='LocalPath']/Value"/>
          </xsl:otherwise>
        </xsl:choose>
      </span>
      <xsl:apply-templates select="BuildInformationNode[@Type!='ActivityProperties']"/>
