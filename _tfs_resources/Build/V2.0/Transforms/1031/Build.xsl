...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
      ...
      <head>
        ...
        <script type="text/javascript" language="javascript">
            ...
            location.href=elem.innerHTML;
              ...
              alert("Der Datei-URI kann nicht aus der aktuellen Zone geöffnet werden.");
            }
            return false;
          }
        </script>
  ...
  <xsl:template match="BuildDetail">
    ...
    <table class="heading" cellspacing="0" cellpadding="0" width="100%">
      <tr>
        <th width="25%">Zusammenfassung</th>
      ...
      <tr>
        <td>Geöffnete Arbeitselemente:</td>
        <xsl:choose>
      ...
      <tr>
        <td>Quellcodeverwaltungsversion:</td>
        <td>
  ...
  <xsl:template match="BuildInformationNode[@Type='AgentScopeActivityTracking']" name="vstfbuild_agentScopeActivityTrackingNode">
    <div class="information">
      ...
      <div class="property">
        <span>
            Reservierungsstatus: <xsl:value-of select="InformationField[Name='ReservationStatus']/Value"/>
        </span>
      </div>
      <div class="property">
        <span>
            Reservierter Agent: <xsl:value-of select="InformationField[Name='ReservedAgentName']/Value"/> (<xsl:value-of select="InformationField[Name='ReservedAgentUri']/Value"/>)
        </span>
      </div>
  ...
  <xsl:template match="BuildInformationNode[@Type='BuildError']" name="vstfbuild_errorNode">
    <div class="information">
      ...
      <b>Fehler</b>: <xsl:value-of select="InformationField[Name='File']/Value"/>(<xsl:value-of select="InformationField[Name='LineNumber']/Value"/>): <xsl:value-of select="InformationField[Name='Code']/Value"/>: <xsl:value-of select="InformationField[Name='Message']/Value"/>
      <xsl:apply-templates select="BuildInformationNode[@Type!='ActivityProperties']"/>
    </div>
  </xsl:template>
  ...
  <xsl:template match="BuildInformationNode[@Type='BuildWarning']" name="vstfbuild_warningNode">
    <div class="information">
      ...
      <b>Warnung</b>: <xsl:value-of select="InformationField[Name='File']/Value"/>(<xsl:value-of select="InformationField[Name='LineNumber']/Value"/>): <xsl:value-of select="InformationField[Name='Code']/Value"/>: <xsl:value-of select="InformationField[Name='Message']/Value"/>
      <xsl:apply-templates select="BuildInformationNode[@Type!='ActivityProperties']"/>
    </div>
  </xsl:template>
      ...
      <span>
        <xsl:choose>
          <xsl:when test="string-length(InformationField[Name='ServerPath']/Value) &gt; 0">
            Erstellt <xsl:value-of select="InformationField[Name='ServerPath']/Value"/>
          </xsl:when>
          <xsl:otherwise>
            Erstellt <xsl:value-of select="InformationField[Name='LocalPath']/Value"/>
          </xsl:otherwise>
        </xsl:choose>
      </span>
      <xsl:apply-templates select="BuildInformationNode[@Type!='ActivityProperties']"/>
