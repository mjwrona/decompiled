...
<!-- Common strings -->
<xsl:variable name="morePrompt">Mehr...</xsl:variable>
<xsl:variable name="FileType">Datei</xsl:variable>
<xsl:variable name="FolderType">Ordner</xsl:variable>
<xsl:variable name="datetime">- Alle Datums- und Uhrzeitwerte werden in UTC angezeigt.</xsl:variable>
...
<xsl:variable name="by">Bereitgestellt von </xsl:variable>
<xsl:variable name="subscriberText"> Erstellt von </xsl:variable>
<xsl:variable name="subscriptionIdText"> mit ID </xsl:variable>
<xsl:variable name="Error">Fehler beim Verarbeiten der Anforderung. Möglicherweise handelt es sich nur um einen vorübergehenden Fehler, der bei einer Wiederholung der Anforderung nicht mehr auftritt. Sollte der Fehler weiterhin auftreten oder Ihnen eine nähere Untersuchung des Fehlers sinnvoll erscheinen, leiten Sie diese Meldung und die angezeigten Fehlermeldungen an die Administrationsmitarbeiter weiter.</xsl:variable>
<xsl:variable name="DetailedErrorHeader">Ausführliche Fehlermeldungen (für die Administrationsmitarbeiter):</xsl:variable>

  <!-- Item information webview title -->
<xsl:variable name="ChangesetViewTitle">Changeset-Informationen</xsl:variable>
<xsl:variable name="ShelvesetViewTitle">Shelvesetinformationen</xsl:variable>
<xsl:variable name="CheckinActionResolve">Lösen</xsl:variable>
<xsl:variable name="CheckinActionAssociate">Zuordnen</xsl:variable>
  
<xsl:variable name="alerts-unsubscribe">Abonnement kündigen</xsl:variable>
<xsl:variable name="alerts-view">Ansicht</xsl:variable>

<!-- Matcher types -->
<xsl:variable name="matcher-follows">FollowsMatcher</xsl:variable>

<xsl:variable name="follows-reason">– Wir haben Ihnen diese Benachrichtigung gesendet, weil Sie diesem Artefakttyp folgen: </xsl:variable>
...
<xsl:choose>
    ...
    <table>
        <tr>
            <td class="PropName">Serverpfad:</td>
            <td class="PropValue"><xsl:value-of select="Item/@item"/></td>
        </tr>
        <tr>
            <td class="PropName">Changeset:</td>
            <td class="PropValue">
        ...
        <tr>
            <td class="PropName">Letzte Änderung am:</td>
            <td class="PropValue"><xsl:value-of select="Item/@date"/></td>
        </tr>
        <tr>
            <td class="PropName">Typ:</td>
            <td class="PropValue">
        ...
        <xsl:if test="Item/@type = 'File'">
        <tr>
            <td class="PropName">Dateigröße (Bytes):</td>
...
<xsl:text>
</xsl:text><xsl:text>Zusammenfassung</xsl:text>
<xsl:text>
</xsl:text><xsl:text>Serverpfad:</xsl:text>       <xsl:value-of select="Item/@item"/>
<xsl:text>
</xsl:text><xsl:text>Changeset:</xsl:text>         <xsl:if test="string-length(Item/@csurl) &gt; 0">
<xsl:call-template name="link">
...
<xsl:value-of select="Item/@cs"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>Letzte Änderung am:</xsl:text>  <xsl:value-of select="Item/@date"/>
<xsl:if test="Item/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>Typ:</xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
<xsl:if test="Item/@type = 'Folder'">
  <xsl:text>
</xsl:text><xsl:text>Typ:</xsl:text>              <xsl:value-of select="$FolderType"/>
</xsl:if>
<xsl:if test="@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>Dateigröße (Bytes):</xsl:text> <xsl:value-of select="Item/@len"/>
</xsl:if>
</xsl:when>
</xsl:choose>
...
<xsl:choose>
    ...
    <table>
        ...
        <xsl:if test="string-length(PendingChange/@srcitem) &gt; 0">
            <tr>
                <td class="PropName">Pfad des Quellservers:</td>
        ...
        <tr>
            <td class="PropName">Erstellungsdatum:</td>
...
<xsl:text>
</xsl:text><xsl:text>Serverpfad:</xsl:text>       <xsl:value-of select="PendingChange/@item"/>
<xsl:if test="string-length(PendingChange/@srcitem) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>Pfad des Quellservers:</xsl:text>       <xsl:value-of select="PendingChange/@srcitem"/>
</xsl:if>
<xsl:if test="string-length(PendingChange/@ssurl) &gt;0">
<xsl:text>
...
<xsl:with-param name="displayText" select="PendingChange/@ss"/>
  ...
  <xsl:text>
</xsl:text><xsl:text>Erstellt am:</xsl:text>  <xsl:value-of select="PendingChange/@date"/>
<xsl:if test="PendingChange/@type = 'File'">
  <xsl:text>
</xsl:text><xsl:text>Typ:</xsl:text>              <xsl:value-of select="$FileType"/>
</xsl:if>
...
<xsl:if test="string-length(PendingChange/@ssurl) &gt; 0">
<xsl:text>
</xsl:text><xsl:text>Änderung:</xsl:text>              <xsl:value-of select="PendingChange/@chg"/>
</xsl:if>
</xsl:when>
</xsl:choose>
<xsl:call-template name="footer">
...
<tr>
<td class="PropName">Teamprojekt(e):</td>
<td class="PropValue">
...
<xsl:choose>
    <xsl:when test="Owner != Committer">
        <td class="PropName">Eingecheckt für:</td>
    </xsl:when>
    <xsl:when test="Owner = Committer">
        <td class="PropName">Eingecheckt von:</td>
    </xsl:when>
</xsl:choose>
<td class="PropValue">
...
<tr>
<td class="PropName">Eingecheckt am:</td>
<td class="PropValue">
...
<tr>
  ...
  <td class="PropValue">
    ...
    <xsl:if test="$valueLength = 0"><pre>Keiner</pre></xsl:if>
  </td>
</tr>
</xsl:for-each>
<tr>
<td class="PropName">Kommentar:</td>
<td class="PropValue">
...
<xsl:if test="string-length(PolicyOverrideComment) &gt; 0">
 <tr>
 <td class="PropName">Grund für das Überschreiben der Richtlinie:</td>
<td class="PropValue">
...
<br/>
<b>Gelöste Arbeitselemente</b>
<table class="WithBorder">
<tr>
  <td class="ColHeadingSmall">Typ</td>
  <td class="ColHeadingXSmall">ID</td>
  <td class="ColHeading">Titel</td>
  <td class="ColHeadingSmall">Status</td>
  <td class="ColHeadingSmall">Zugewiesen an</td>
</tr>
...
<br/>
<b>Zugeordnete Arbeitselemente</b>
<table class="WithBorder">
<tr>
  <td class="ColHeadingSmall">Typ</td>
  <td class="ColHeadingXSmall">ID</td>
...
<td class="ColHeadingSmall">Ändern</td>
<td class="ColHeading">Ordner</td>
</tr>
...
<tr>
<td class="PropName">Erstellt am:</td>
<td class="PropValue">
  <xsl:value-of select="CreationDate"/>
</td>
</tr>
...
<tr>
  ...
  <td class="ColHeadingXSmall">ID</td>
  <td class="ColHeading">Titel</td>
  <td class="ColHeadingSmall">Aktion</td>
  <td class="ColHeadingSmall">Zugewiesen an</td>
</tr>
...
<xsl:template name="wiItem">
    ...
    <td class="ColDataSmall">
      ...
      <xsl:if test="$assignedToLength = 0">N/V</xsl:if>
    </td>
</xsl:template> <!-- wiItem -->
